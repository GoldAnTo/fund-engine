...
function completedView(overrides: Partial<AutomaticResearchView> = {}): AutomaticResearchView {
  return {
    ...
    activities: [{
      label: "资料处理",
      count: 3,
      technicalDetails: [{ occurredAt: "2026-08-17T01:00:20Z", workItem: "筛选供应链公告", internalStatus: "succeeded" }],
    }],
    exceptions: [{
      reason: "来源许可不满足",
      count: 1,
      impact: "该来源未通过许可核验，未纳入结论。",
      systemAction: "系统已跳过并继续处理。",
    }],
    factors: [{
      statement: "供应链需求变化",
      classification: "key",
      rankingReason: "与提问高度相关，支持证据充足。",
      supportCount: 2,
...
function activeView(status: "queued" | "running", runId = `run-${status}`): AutomaticResearchView {
  return completedView({
    ...
    stages: completedView().stages.map((stage, index) => ({
      ...
      completedAt: null,
      startedAt: index === 0 && status === "running" ? "2026-08-17T01:00:00Z" : null,
      summary: index === 0 && status === "running" ? "正在查找允许来源" : "等待中",
    })),
    narrative: {
      currentAction: status === "running" ? "正在执行关键步骤" : "任务已排队，等待启动",
      completedCount: 0,
      totalCount: 5,
      nextAction: status === "running" ? "继续执行后续阶段" : "开始自动分析",
      elapsedSeconds: status === "running" ? 6 : 0,
      estimatedRemainingSecondsMin: status === "running" ? 30 : null,
      estimatedRemainingSecondsMax: status === "running" ? 50 : null,
    },
    activities: [{
      label: "系统执行",
      count: 1,
      technicalDetails: [{
        occurredAt: "2026-08-17T01:00:10Z",
        workItem: "关键来源识别",
        internalStatus: status === "running" ? "running" : "queued",
      }],
    }],
    exceptions: status === "running" ? [] : [{
      reason: "来源许可不满足",
      count: 1,
      impact: "该来源未通过许可核验，未纳入结论。",
      systemAction: "系统已跳过并继续处理。",
...
function failedView(): AutomaticResearchView {
  return completedView({
    ...
    narrative: {
      currentAction: "自动研究未能完成",
      completedCount: 1,
      totalCount: 5,
      nextAction: "请查看失败信息并重试。",
      elapsedSeconds: 10,
      estimatedRemainingSecondsMin: null,
      estimatedRemainingSecondsMax: null,
    },
    ...
    factors: [{
      ...
      rankingReason: "证据不足，暂不纳入关键因素。",
      supportCount: 0,
      counterEvidenceCount: 0,
      evidenceGap: "缺少可比财报区间。",
    }],
...
describe("AutomaticResearchPage", () => {
  ...
  it("loads and shows the completed result with five textual stages and trace details", async () => {
    ...
    expect(details).toHaveTextContent("资料处理");
    expect(details).toHaveTextContent("来源筛选");
    expect(details).toHaveTextContent("来源许可不满足");
    expect(details).toHaveTextContent("未通过许可核验");
    expect(details).toHaveTextContent("该来源未通过许可核验，未纳入结论。");
    expect(details).toHaveTextContent("可查看关键因素并验证证据细节。");
    ...
    const liveStage = screen.getByText(/当前动作：研究结论已生成/);

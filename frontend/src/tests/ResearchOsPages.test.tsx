...
import {
  ...
  CaseWikiPage,
  MechanismProtocolPanel,
  MechanismRuleConfig,
  MechanismRuleHistory,
  ProtocolContent,
...
type OutcomeBindingFixture = Awaited<
  ReturnType<MockResearchOsApi["createOutcomeBinding"]>
>;

function outcomeBindingFixture(
  state: OutcomeBindingFixture["state"],
  id = "binding-pending",
): OutcomeBindingFixture {
  return {
    id,
    thesis_id: "00000000-0000-4000-8000-000000000001",
    ...
    direction: "increase",
    baseline: {
      source_ref: "document:doc-event-tsm-q2",
      value: "100",
      unit: "CNY m",
...
function WorkflowNoticeProbe() {
  const location = useLocation();
  const notice = (location.state as { workflowNotice?: unknown } | null)?.workflowNotice;
  return <output data-testid="workflow-notice">{typeof notice === "string" ? notice : ""}</output>;
}

function ProtocolContentRouteHarness({
  workbenches,
}: {
  workbenches: Record<string, EventWorkbench>;
}) {
  const { caseId = "" } = useParams();
  return (
    <>
      <Link to="/events/event-alphabet/protocol">切换到 Alphabet</Link>
      <ProtocolContent caseId={caseId} data={workbenches[caseId]} />
      <CaseLocationProbe />
    </>
  );
}

function renderSwitchableProtocolPage(
  workbenches: Record<string, EventWorkbench>,
) {
  return render(
    <MemoryRouter initialEntries={["/events/event-tsm/protocol"]}>
      <Routes>
        <Route
          ...
          element={<ProtocolContentRouteHarness workbenches={workbenches} />}
        />
      </Routes>
    </MemoryRouter>,
  );
...
async function switchProtocolPageToAlphabet(
  user: ReturnType<typeof userEvent.setup>,
) {
  await user.click(screen.getByRole("link", { name: "切换到 Alphabet" }));
  expect(await screen.findByTestId("case-location")).toHaveTextContent(
    "/events/event-alphabet/protocol",
  );
  expect(await screen.findByText("选中命题 · Alphabet 资本开支验证")).toBeVisible();
}

async function fillOutcomeBindingForm(
  user: ReturnType<typeof userEvent.setup>,
) {
  await user.click(
    await screen.findByRole("button", { name: "设定结果指标与验证窗口" }),
  );
  await user.type(screen.getByLabelText("公司 ID"), "company:tsmc");
  await user.type(screen.getByLabelText("company 范围"), "先进封装业务线");
  await user.selectOptions(
    await screen.findByLabelText("选择冻结基线资料"),
    "doc-event-tsm-q2",
  );
  await user.type(screen.getByLabelText("基线数值"), "100");
  await user.type(screen.getByLabelText("基线观察期"), "2026-06-30");
  await user.type(screen.getByLabelText("验证窗口起点"), "2026-07-01");
  await user.type(screen.getByLabelText("验证窗口终点"), "2026-12-31");
  await user.type(
    screen.getByPlaceholderText("说明为何选择该指标、基线与窗口"),
    "固定可回放的验证基线",
  );
}

async function protocolWorkbenches(adapter: MockResearchAdapter) {
  const tsm = await adapter.getEventWorkbench("event-tsm");
  const alphabet = await adapter.getEventWorkbench("event-alphabet");
  return {
    "event-tsm": tsm,
    "event-alphabet": {
      ...alphabet,
      factors: alphabet.factors.map((factor, index) =>
        index === 0
          ? { ...factor, statement: "Alphabet 资本开支验证" }
          : factor,
      ),
    },
  };
...
type MetricFixture = Awaited<ReturnType<MockResearchOsApi["metrics"]>>[number];

function metricFixture(id: string, displayName: string): MetricFixture {
  return {
    id,
    metric_id: id,
    version: 1,
...
describe("Research OS event entry", () => {
  it("renders the root workflow and event switcher without calling the legacy workbench", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    const legacyWorkbench = vi.spyOn(adapter, "getEventWorkbench").mockRejectedValue(
      new Error("legacy workbench unavailable"),
    );
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-tsm"]}>
        <Routes>
          <Route path="/events/:caseId" element={<CaseWorkflowOverviewPage />} />
        </Routes>
      </MemoryRouter>,
    );

    expect(await screen.findByLabelText("当前事件研究流程")).toBeVisible();
    expect(screen.getByRole("button", { name: /当前事件研究/ })).toBeVisible();
    await user.click(screen.getByText("更多研究内容"));
    expect(
      within(screen.getByRole("group", { name: "更多研究内容" })).getByRole(
        "link",
        { name: "证据工作台" },
      ),
    ).toBeVisible();
    expect(legacyWorkbench).not.toHaveBeenCalled();
  });
  ...
  it("restores event-switcher focus when Escape closes the menu", async () => {
    const user = userEvent.setup();
    render(
      <MemoryRouter initialEntries={["/events/event-tsm"]}>
    ...
    const switcher = await screen.findByRole("button", { name: /当前事件研究/ });
    await user.click(switcher);
    const menu = screen.getByRole("group", { name: "切换事件研究" }).closest(".ros-event-menu");
    expect(switcher).toHaveAttribute("aria-controls", menu?.id);
    await user.keyboard("{Escape}");

    expect(screen.queryByRole("group", { name: "切换事件研究" })).not.toBeInTheDocument();
    expect(switcher).toHaveFocus();
  });

  it("shows the server-owned six-stage workflow below the current event switcher", async () => {
    render(
      <MemoryRouter initialEntries={["/events/event-tsm"]}>
        <Routes><Route path="/events/:caseId" element={<CaseConclusionPage />} /></Routes>
      </MemoryRouter>,
    ...
    const switcher = await screen.findByRole("button", { name: /当前事件研究/ });
    const progress = await screen.findByRole("list", { name: "事件研究六阶段" });
    expect(switcher.compareDocumentPosition(progress) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(progress).toHaveTextContent("建立事件");
    expect(progress).toHaveTextContent("确认命题");
    expect(progress).toHaveTextContent("主动补证");
    expect(progress).toHaveTextContent("证据归并");
    expect(progress).toHaveTextContent("命题判定");
    expect(progress).toHaveTextContent("报告与监测");
    expect(screen.getByText("系统正在做什么")).toBeVisible();
    expect(screen.getByText("你现在应该做什么")).toBeVisible();
  ...
  it("routes a revised scope to required protocol work instead of claiming acquisition resumed", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-exhausted");
    vi.spyOn(adapter, "getEventWorkflow").mockResolvedValue({
      ...blocked,
      scope: { ...blocked.scope, id: "scope-revised", version: 2 },
      ...
      userAction: {
        ...
        alternatives: [],
        impact: "协议完成后才能继续补证",
        payload: { scope_version_id: "scope-revised", scope_version: 2 },
      },
      userActionSummary: "需要完成研究协议",
    });
    setResearchClient(adapter);
    setResearchOsApi(new MockResearchOsApi(adapter));
    const user = userEvent.setup();
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-exhausted/scope"]}>
        <Routes>
          <Route path="/events/:caseId/scope" element={<CaseScopePage />} />
          <Route path="/events/:caseId/protocol" element={<><p>协议编辑区</p><WorkflowNoticeProbe /></>} />
        </Routes>
      </MemoryRouter>,
    );

    await user.click(await screen.findByRole("button", { name: "保存新的研究范围" }));
    expect(await screen.findByText("协议编辑区")).toBeVisible();
    expect(screen.getByTestId("workflow-notice")).toHaveTextContent("范围已更新，需完成研究协议");
    expect(screen.queryByText(/系统已按新范围继续补证/)).not.toBeInTheDocument();
  });

  it("does not append another scope version when the write succeeded but projection refresh failed", async () => {
    const adapter = new MockResearchAdapter();
    const update = vi.spyOn(adapter, "updateEventResearchScope");
    vi.spyOn(adapter, "getEventWorkflow").mockRejectedValue(new Error("projection unavailable"));
    setResearchClient(adapter);
    setResearchOsApi(new MockResearchOsApi(adapter));
    const user = userEvent.setup();
    render(
      <MemoryRouter initialEntries={["/events/event-exhausted/scope"]}>
        <Routes><Route path="/events/:caseId/scope" element={<CaseScopePage />} /></Routes>
      </MemoryRouter>,
    );

    const save = await screen.findByRole("button", { name: "保存新的研究范围" });
    await user.click(save);
    expect(await screen.findByText(/范围已更新，最新状态暂不可读/)).toBeVisible();
  ...
  it("resumes the workflow only after every required protocol is ready", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-tsm");
    vi.spyOn(adapter, "getEventWorkflow").mockResolvedValue({
      ...blocked,
      scope: { ...blocked.scope, id: "scope-2", version: 2 },
      state: "needs_scope_decision",
      userAction: {
        kind: "protocol_completion",
        label: "完成研究协议",
    ...
    const resume = vi.spyOn(adapter, "resumeProtocolWorkflow");
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "researchability").mockResolvedValue({
      status: "ready",
      reason_codes: [],
      effective_binding_id: "binding-ready",
      next_action: "可以继续自动补证",
    });
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-tsm/protocol?focus=required"]}>
        <Routes>
          <Route path="/events/:caseId/protocol" element={<CaseProtocolPage />} />
          <Route path="/events/:caseId" element={<WorkflowNoticeProbe />} />
        </Routes>
      </MemoryRouter>,
    );

    await user.click(await screen.findByRole("button", { name: "继续自动补证" }));
    expect(resume).toHaveBeenCalledWith("event-tsm", "scope-2", {
      scopeVersion: 2,
      idempotencyKey: "workflow:resume-protocol:event-tsm:scope-2",
    });
    expect(await screen.findByTestId("workflow-notice")).toHaveTextContent("系统已按新范围继续补证");
  });

  it("keeps resume blocked when a current scope factor has no protocol-bound thesis", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-tsm");
    vi.spyOn(adapter, "getEventWorkflow").mockResolvedValue({
      ...blocked,
    ...
    vi.spyOn(adapter, "getEventWorkbench").mockResolvedValue({
      ...
      factors: [
        ...
        {
          ...workbench.factors[0],
          position: workbench.factors.length + 1,
          statement: "尚未绑定命题的新增因素",
          thesisId: undefined,
        },
      ],
    });
    ...
    expect(await screen.findByText(/当前还有 1 项未完成：尚未绑定命题的新增因素/)).toBeVisible();
    expect(screen.queryByRole("button", { name: "继续自动补证" })).not.toBeInTheDocument();
  });

  it("keeps the protocol page open and reports a truthful recovery when resume fails", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-tsm");
    vi.spyOn(adapter, "getEventWorkflow").mockResolvedValue({
      ...blocked,
    ...
    vi.spyOn(adapter, "resumeProtocolWorkflow").mockRejectedValue(new Error("resume unavailable"));
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "researchability").mockResolvedValue({ status: "ready", reason_codes: [], effective_binding_id: "binding-ready", next_action: "可以继续自动补证" });
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    ...
    await user.click(await screen.findByRole("button", { name: "继续自动补证" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("协议已满足，但系统尚未恢复补证");
    expect(screen.getByRole("button", { name: "重新尝试继续自动补证" })).toBeEnabled();
  });

  it("does not let a late protocol resume response navigate back to the previous event", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-tsm");
    vi.spyOn(adapter, "getEventWorkflow").mockImplementation(async (caseId) => ({
      ...blocked,
      event: { ...blocked.event, caseId, title: caseId === "event-tsm" ? "台积电事件" : "Alphabet 事件" },
      scope: { ...blocked.scope, id: `scope-${caseId}`, version: 2 },
      state: "needs_scope_decision",
      userAction: {
        kind: "protocol_completion",
        label: "完成研究协议",
    ...
    const pendingResume = deferred<Awaited<ReturnType<typeof adapter.resumeProtocolWorkflow>>>();
    const resume = vi.spyOn(adapter, "resumeProtocolWorkflow").mockReturnValue(pendingResume.promise);
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "researchability").mockResolvedValue({
      status: "ready",
      reason_codes: [],
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-tsm/protocol?focus=required"]}>
        <Routes>
          <Route
            path="/events/:caseId/protocol"
            element={<><Link to="/events/event-alphabet/protocol">切换到 Alphabet</Link><CaseProtocolPage /><CaseLocationProbe /></>}
          />
          <Route path="/events/:caseId" element={<CaseLocationProbe />} />
        </Routes>
      </MemoryRouter>,
    );
    ...
    await user.click(screen.getByRole("link", { name: "切换到 Alphabet" }));
    expect(await screen.findByTestId("case-location")).toHaveTextContent("/events/event-alphabet/protocol");

    await act(async () => pendingResume.resolve({
      orchestrationId: "orch-tsm",
      caseId: "event-tsm",
      scopeVersionId: "scope-event-tsm",
      researchRunId: "run-tsm",
      state: "planning_acquisition",
      userStage: "acquisition",
      currentSystemAction: "准备资料获取",
      systemActionReason: "协议已完成",
      nextAction: null,
      version: 5,
    }));

    expect(screen.getByTestId("case-location")).toHaveTextContent("/events/event-alphabet/protocol");
    expect(screen.queryByText("系统已按新范围继续补证。")).not.toBeInTheDocument();
  });

  it("ignores a rejected protocol resume after switching events", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-tsm");
    vi.spyOn(adapter, "getEventWorkflow").mockImplementation(async (caseId) => ({
      ...blocked,
      event: {
        ...blocked.event,
        caseId,
        title: caseId === "event-tsm" ? "台积电事件" : "Alphabet 事件",
      },
      scope: { ...blocked.scope, id: `scope-${caseId}`, version: 2 },
      state: "needs_scope_decision",
      userAction: {
    ...
    const pendingResume = deferred<
      Awaited<ReturnType<typeof adapter.resumeProtocolWorkflow>>
    >();
    const resume = vi
      .spyOn(adapter, "resumeProtocolWorkflow")
      .mockReturnValue(pendingResume.promise);
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "researchability").mockResolvedValue({
      status: "ready",
      reason_codes: [],
    ...
    renderSwitchableProtocolPage(await protocolWorkbenches(adapter));
    await user.click(await screen.findByRole("button", { name: "继续自动补证" }));
    await waitFor(() => expect(resume).toHaveBeenCalledTimes(1));
    await switchProtocolPageToAlphabet(user);

    await act(async () => {
      pendingResume.reject(new Error("late resume failed"));
    ...
    expect(screen.queryByText(/协议已满足，但系统尚未恢复补证/)).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "重新尝试继续自动补证" }),
    ).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "正在继续…" })).not.toBeInTheDocument();
    expect(screen.getByText("选中命题 · Alphabet 资本开支验证")).toBeVisible();
  });

  it("ignores a resolved protocol refresh after switching events", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    const pendingResearchability = deferred<
      Awaited<ReturnType<typeof api.researchability>>
    >();
    let deferRefresh = false;
    const researchability = vi
      .spyOn(api, "researchability")
      .mockImplementation((thesisId) =>
        deferRefresh
          ? pendingResearchability.promise
          : Promise.resolve({
              status: "blocked",
              reason_codes: ["missing_outcome_binding"],
              effective_binding_id: null,
              next_action: `补齐 ${thesisId} 的当前协议`,
            }),
      );
    const workflow = vi.spyOn(adapter, "getEventWorkflow");
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    renderSwitchableProtocolPage(await protocolWorkbenches(adapter));

    await screen.findByText(/补齐 00000000-0000-4000-8000-000000000001 的当前协议/);
    const callsBeforeRefresh = researchability.mock.calls.length;
    deferRefresh = true;
    window.dispatchEvent(new Event("research-os-protocol-refresh"));
    await waitFor(() =>
      expect(researchability.mock.calls.length).toBeGreaterThan(callsBeforeRefresh),
    );
    deferRefresh = false;
    await switchProtocolPageToAlphabet(user);
    const workflowCasesBeforeSettlement = workflow.mock.calls.map(([caseId]) => caseId);

    await act(async () => {
      pendingResearchability.resolve({
        status: "ready",
        reason_codes: [],
        effective_binding_id: "binding-event-tsm-stale",
        next_action: "TSM 旧刷新结果不得写回",
      });
      await pendingResearchability.promise;
    ...
    expect(screen.queryByText("TSM 旧刷新结果不得写回")).not.toBeInTheDocument();
    expect(screen.queryByText("协议已保存，但最新可研究性状态暂不可读")).not.toBeInTheDocument();
    expect(workflow.mock.calls.map(([caseId]) => caseId)).toEqual(
      workflowCasesBeforeSettlement,
    );
  });

  it.each(["resolve", "reject"] as const)(
    "ignores protocol metrics that %s after switching events",
    async (settlement) => {
      const adapter = new MockResearchAdapter();
      const api = new MockResearchOsApi(adapter);
      const pendingMetrics = deferred<Awaited<ReturnType<typeof api.metrics>>>();
      let metricsCall = 0;
      vi.spyOn(api, "metrics").mockImplementation(() => {
        ...
        return metricsCall === 1
          ...
          : Promise.resolve([
              metricFixture("metric-alphabet-current", "Alphabet 当前指标"),
            ]);
      });
      setResearchClient(adapter);
      setResearchOsApi(api);
      ...
      await screen.findByText("选中命题 · 资本开支 / 自由现金流担忧");
      await switchProtocolPageToAlphabet(user);
      await user.click(screen.getByRole("button", { name: "设定结果指标与验证窗口" }));
      expect(
        await screen.findByRole("option", { name: /Alphabet 当前指标/ }),
      ).toBeVisible();

      await act(async () => {
        if (settlement === "resolve") {
          pendingMetrics.resolve([
            metricFixture("metric-tsm-stale", "TSM 旧指标不得写回"),
          ]);
          await pendingMetrics.promise;
        } else {
          pendingMetrics.reject(new Error("late metrics failed"));
          await pendingMetrics.promise.catch(() => undefined);
        }
      });

      expect(
        screen.getByRole("option", { name: /Alphabet 当前指标/ }),
      ).toBeVisible();
      expect(screen.queryByRole("option", { name: /TSM 旧指标不得写回/ })).not.toBeInTheDocument();
      expect(screen.queryByText(/指标库暂不可读/)).not.toBeInTheDocument();
      expect(screen.getByText("选中命题 · Alphabet 资本开支验证")).toBeVisible();
    },
  );

  it("does not repeat an outcome-binding write after its projection refresh fails", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    let projectionUnavailable = false;
    vi.spyOn(api, "researchability").mockImplementation(() => projectionUnavailable
      ? Promise.reject(new Error("projection unavailable"))
      : Promise.resolve({
          status: "blocked",
          reason_codes: ["missing_outcome_binding"],
          effective_binding_id: null,
    ...
    const create = vi.spyOn(api, "createOutcomeBinding").mockImplementation(async (...args) => {
      projectionUnavailable = true;
      return MockResearchOsApi.prototype.createOutcomeBinding.apply(api, args);
    });
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    ...
    await fillOutcomeBindingForm(user);
    const save = screen.getByRole("button", { name: "登记为待审核结果绑定" });
    await user.click(save);

    expect(await screen.findByText(/结果绑定已登记，最新状态暂不可读/)).toBeVisible();
    expect(save).toBeDisabled();
    await user.click(save);
    expect(create).toHaveBeenCalledTimes(1);
  });

  it("scopes outcome-binding receipts to one Case factor without locking the next factor", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    const create = vi.spyOn(api, "createOutcomeBinding");
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    render(
    ...
    await screen.findByText(/结果绑定已登记为草案/);
    expect(save).toBeDisabled();

    await user.click(screen.getByRole("button", { name: /盈利预期变化/ }));
    expect(screen.getByText("选中命题 · 盈利预期变化")).toBeVisible();
    expect(save).toBeEnabled();
    await user.click(save);
    await waitFor(() => expect(create).toHaveBeenCalledTimes(2));
    await waitFor(() =>
      expect(screen.queryByRole("button", { name: "正在登记…" }))
        .not.toBeInTheDocument(),
    );
    expect(create.mock.calls.map(([thesisId]) => thesisId)).toEqual([
      "00000000-0000-4000-8000-000000000001",
      "00000000-0000-4000-8000-000000000002",
    ]);

    await user.click(screen.getByRole("button", { name: /资本开支 \/ 自由现金流担忧/ }));
    expect(save).toBeDisabled();
    await user.click(screen.getByRole("button", { name: "重新读取协议状态" }));
    await screen.findByText(/已读取最新协议状态/);
    expect(save).toBeDisabled();
  });

  it("scopes outcome-binding approval receipts to one Case factor", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "researchability").mockImplementation(async (thesisId) => ({
      status: "blocked",
      reason_codes: ["binding_not_approved"],
      effective_binding_id: `binding-${thesisId}`,
      next_action: "审核并固定结果绑定",
    }));
    const approve = vi
      .spyOn(api, "approveOutcomeBinding")
      .mockImplementation(async (bindingId) => ({
        ...outcomeBindingFixture("approved", bindingId),
        thesis_id: bindingId.replace("binding-", ""),
      }));
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    ...
    const reason = await screen.findByLabelText("结果绑定审核理由");
    await user.type(reason, "已核对冻结基线");
    const approveButton = screen.getByRole("button", { name: "审核并固定结果绑定" });
    await user.click(approveButton);
    await waitFor(() => expect(approve).toHaveBeenCalledTimes(1));
    await screen.findByText(/结果绑定已审核并固定/);
    ...
    expect(approve.mock.calls.map(([bindingId]) => bindingId)).toEqual([
      "binding-00000000-0000-4000-8000-000000000001",
      "binding-00000000-0000-4000-8000-000000000002",
    ]);
  });

  it("settles pending protocol requests after unmount without state updates or navigation", async () => {
    const adapter = new MockResearchAdapter();
    const blocked = await adapter.getEventWorkflow("event-tsm");
    vi.spyOn(adapter, "getEventWorkflow").mockResolvedValue({
      ...blocked,
      scope: { ...blocked.scope, id: "scope-event-tsm", version: 2 },
      state: "needs_scope_decision",
      userAction: {
        kind: "protocol_completion",
        label: "完成研究协议",
    ...
    const api = new MockResearchOsApi(adapter);
    const pendingMetrics = deferred<Awaited<ReturnType<typeof api.metrics>>>();
    vi.spyOn(api, "metrics").mockReturnValue(pendingMetrics.promise);
    const pendingRefresh = deferred<Awaited<ReturnType<typeof api.researchability>>>();
    let deferRefresh = false;
    const researchability = vi
      .spyOn(api, "researchability")
      .mockImplementation(() =>
    ...
    const view = renderSwitchableProtocolPage(await protocolWorkbenches(adapter));
    ...
    await act(async () => {
      pendingMetrics.resolve([
        metricFixture("metric-after-unmount", "卸载后指标"),
      ]);
      pendingRefresh.resolve({
        status: "ready",
        reason_codes: [],
        effective_binding_id: "binding-after-unmount",
        next_action: "卸载后协议状态",
      });
      pendingResume.reject(new Error("resume rejected after unmount"));
    ...
    const unmountedUpdateWarnings = consoleError.mock.calls.filter((args) =>
      args.some((value) =>
        /unmounted component|state update on an unmounted/i.test(String(value)),
      ),
    );
  ...
  it("returns a rejected Case to active research immediately after saving a new scope", async () => {
    ...
    expect(
      within(await screen.findByRole("list", { name: "事件研究六阶段" }))
        .getByText("主动补证")
        .closest("li"),
    ).toHaveAttribute("aria-current", "step");
    expect(screen.getByText("当前无需操作")).toBeVisible();
  ...
  it("does not let a late scope receipt lock the newly selected Case", async () => {
    const adapter = new MockResearchAdapter();
    const updateScope = adapter.updateEventResearchScope.bind(adapter);
    const pendingScope = deferred<Awaited<ReturnType<typeof updateScope>>>();
    vi.spyOn(adapter, "updateEventResearchScope").mockImplementationOnce(
      ...
      <MemoryRouter initialEntries={["/events/event-exhausted/scope"]}>
        <Routes>
          <Route
            ...
            element={(
              <>
                <Link to="/events/event-alphabet/scope">切换到 Alphabet 范围</Link>
                <CaseScopePage />
                <CaseLocationProbe />
              </>
            )}
    ...
    await screen.findByRole("heading", { name: "调整关键因素，创建新的研究范围" });
    await user.click(screen.getByRole("button", { name: "保存新的研究范围" }));
    await user.click(screen.getByRole("link", { name: "切换到 Alphabet 范围" }));
    expect(await screen.findByTestId("case-location")).toHaveTextContent(
      "/events/event-alphabet/scope",
    );
    expect(
      await screen.findByRole("button", { name: "保存新的研究范围" }),
    ).toBeEnabled();
    ...
    expect(screen.getByRole("button", { name: "保存新的研究范围" })).toBeEnabled();
    expect(screen.queryByText(/范围已更新/)).not.toBeInTheDocument();
  });
  ...
  it("keeps the research subject, question and factors editable before a Case is created", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    const createEventResearch = vi.spyOn(adapter, "createEventResearch");
    setResearchClient(adapter);
    render(
    ...
    const subject = screen.getByLabelText("研究主体（公司、机构或行业）");
    ...
    await user.type(subject, "Alphabet");
    await user.click(
      screen.getByRole("button", { name: "建立 Case，进入资料核验" }),
    );
    await waitFor(() =>
      expect(createEventResearch).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  it("initializes the created Case from the typed workflow safe action before navigating", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    const getWorkflow = vi.spyOn(adapter, "getEventWorkflow").mockRejectedValue(
      new EventWorkflowError(
        ...
        "research workflow is not initialized",
        {
          ...
          label: "确认命题并开始研究",
          method: "POST",
          href: "/api/v1/event-research/event-created-1/workflow/confirm",
          payload: {
            scope_version_id: "scope-created-1",
            scope_version: 1,
            idempotency_key: "workflow:initialize:scope-created-1:v1",
          },
        },
      ),
    );
    const confirm = vi.spyOn(adapter, "confirmEventWorkflow").mockResolvedValue({
      orchestrationId: "orch-created-1",
      caseId: "event-created-1",
      scopeVersionId: "scope-created-1",
      researchRunId: "run-created-1",
      state: "planning_acquisition",
      userStage: "acquisition",
      currentSystemAction: "正在根据已确认范围规划资料获取",
      systemActionReason: "研究范围已确认",
      nextAction: null,
      version: 1,
    });
    setResearchClient(adapter);
    render(
      <MemoryRouter initialEntries={["/events/new"]}>
        <Routes>
          <Route path="/events/new" element={<EventCreatePage />} />
          <Route path="/events/:caseId" element={<><p>新 Case 已建立</p><WorkflowNoticeProbe /></>} />
        </Routes>
      </MemoryRouter>,
    );

    await user.type(screen.getByLabelText("事件原始输入"), "公司上调产能指引，需验证订单传导。");
    ...
    expect(getWorkflow.mock.calls[0][0]).toBe("event-created-1");
    expect(confirm).toHaveBeenCalledWith("event-created-1", "scope-created-1", {
      scopeVersion: 1,
      idempotencyKey: "workflow:initialize:scope-created-1:v1",
    });
    expect(await screen.findByText("新 Case 已建立")).toBeVisible();
    expect(screen.getByTestId("workflow-notice")).toHaveTextContent("系统已开始规划资料获取");
  });

  it("keeps a recovery link when workflow initialization has no real scope version", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    vi.spyOn(adapter, "getEventWorkflow").mockRejectedValue(
      new EventWorkflowError(
        "workflow_not_initialized",
        "research workflow is not initialized",
        {
          ...
          method: "POST",
          href: "/api/v1/event-research/event-created-1/workflow/confirm",
          payload: { brief_id: "brief-created-1", idempotency_key: "workflow:initialize:brief-created-1" },
        },
      ),
    );
    const confirm = vi.spyOn(adapter, "confirmEventWorkflow");
    setResearchClient(adapter);
    render(
      <MemoryRouter initialEntries={["/events/new"]}>
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent("缺少可验证的范围版本");
    expect(screen.getByRole("link", { name: "打开已建立的事件研究" })).toHaveAttribute(
      "href",
      "/events/event-created-1",
    );
  ...
  it("returns an attached material to the event overview with a truthful workflow notice", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    const attach = vi.spyOn(adapter, "attachEventMaterial");
    setResearchClient(adapter);
    render(
      <MemoryRouter initialEntries={["/events/new"]}>
        <Routes>
    ...
    await user.type(screen.getByLabelText("事件原始输入"), "公司补充订单交付节奏，归入已有研究。");
    await user.click(screen.getByRole("button", { name: "识别事件与研究问题" }));
    await user.click(await screen.findByRole("radio", { name: "归入已有 Case" }));
    await user.selectOptions(screen.getByLabelText("选择目标 Case"), "event-tsm");
    await user.click(screen.getByRole("button", { name: "冻结并归入当前 Case" }));

    await waitFor(() => expect(attach).toHaveBeenCalledWith(expect.not.objectContaining({ actor: expect.anything() })));
    expect(await screen.findByTestId("workflow-notice")).toHaveTextContent(
      "材料已冻结，统一流程状态将在事件总览读取",
    );
  });
  ...
  it("keeps Case creation usable and transparent when model framing is unavailable", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    vi.spyOn(adapter, "extractEventResearch").mockRejectedValue(
    ...
    await user.upload(
      screen.getByLabelText("选择上传原件文件"),
      new File(
        ["皇马科技（603181.SH）披露营业收入、净利润与供应链风险。"],
        "event-note.txt",
        { type: "text/plain" },
      ),
    );
    ...
    expect(screen.getByLabelText("研究主体（公司、机构或行业）")).toHaveValue("皇马科技");
    expect(screen.getByLabelText("证券代码（可选）")).toHaveValue("603181.SH");
    expect(screen.getByLabelText("关键因素 1")).toHaveValue("营业收入");
    expect(screen.getByLabelText("关键因素 2")).toHaveValue("净利润");
    expect(screen.getByLabelText("关键因素 3")).toHaveValue("供应链");
    expect(screen.getByRole("button", { name: "建立 Case，进入资料核验" })).toBeEnabled();
  });

  it("extracts an explicit leading subject when fallback text has no ticker", async () => {
    const user = userEvent.setup();
    const adapter = new MockResearchAdapter();
    vi.spyOn(adapter, "extractEventResearch").mockRejectedValue(
      new Error("provider unavailable"),
    ...
    await user.type(
      screen.getByLabelText("事件原始输入"),
      "台积电说明先进封装产能将继续扩张。请核验产能、客户需求、设备交付以及可能的替代解释。",
    );
    await user.click(screen.getByRole("button", { name: "识别事件与研究问题" }));
    ...
    expect(screen.getByLabelText("研究主体（公司、机构或行业）")).toHaveValue("台积电");
    expect(screen.getByLabelText("证券代码（可选）")).toHaveValue("");
    expect(screen.getByRole("button", { name: "建立 Case，进入资料核验" })).toBeEnabled();
  });
  ...
  it.each([
    ...
    expect(screen.getByRole("region", { name: "来源资料台账" })).toBeVisible();
  ...
  it("refreshes global workflow state without redirecting when review succeeds after leaving the page", async () => {
    ...
    expect(reviewProposal.mock.calls[0]?.[1]).not.toHaveProperty("reviewer_id");
  ...
  it("does not render a duplicate global run strip on an event workflow overview", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "activeRuns").mockResolvedValue({
      items: [{
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-tsm"]}>
        <Routes><Route element={<AppShell />}><Route path="/events/:caseId" element={<p>统一 workflow 主操作</p>} /></Route></Routes>
      </MemoryRouter>,
    );

    expect(await screen.findByText("统一 workflow 主操作")).toBeVisible();
    await waitFor(() => expect(api.activeRuns).toHaveBeenCalled());
    expect(screen.queryByRole("region", { name: "系统正在运行" })).not.toBeInTheDocument();
    expect(screen.queryByText("台积电研究")).not.toBeInTheDocument();
  });

  it("scopes child-page run strips to the selected event and removes the previous event", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "activeRuns").mockResolvedValue({
      items: [
        { run_id: "run-tsm", case_id: "event-tsm", case_title: "台积电旧事件", status: "running", stage: "fetching", updated_at: "2026-08-16T08:00:00Z", processed_count: 3, next_action: "查看台积电运行", scope: { allowed_source_types: ["company_disclosure"] } },
        { run_id: "run-alphabet", case_id: "event-alphabet", case_title: "Alphabet 当前事件", status: "running", stage: "fetching", updated_at: "2026-08-16T08:00:00Z", processed_count: 2, next_action: "查看 Alphabet 运行", scope: { allowed_source_types: ["company_disclosure"] } },
      ],
      next_cursor: null,
      has_more: false,
    });
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-alphabet/monitor"]}>
        <Routes><Route element={<AppShell />}><Route path="/events/:caseId/monitor" element={<p>事件子页</p>} /></Route></Routes>
      </MemoryRouter>,
    );

    const strip = await screen.findByRole("region", { name: "系统正在运行" });
    expect(strip).toHaveTextContent("Alphabet 当前事件");
    expect(screen.queryByText("台积电旧事件")).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "查看台积电运行 →" })).not.toBeInTheDocument();
  });

  it("closes an old Case run drawer when the selected event changes", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    vi.spyOn(api, "activeRuns").mockResolvedValue({
      items: [
    ...
    vi.spyOn(api, "runEvents").mockResolvedValue({
      run_id: "run-tsm",
      items: [{ seq: 1, stage: "fetching", status: "running", message: "TSM 专属运行详情", details: {}, created_at: "2026-08-16T08:00:00Z" }],
      next_cursor: null,
      has_more: false,
    });
    setResearchClient(adapter);
    ...
    render(
      <MemoryRouter initialEntries={["/events/event-tsm/monitor"]}>
        <Routes>
          <Route element={<AppShell />}>
            <Route path="/events/:caseId/monitor" element={<Link to="/events/event-alphabet/monitor">切换到 Alphabet</Link>} />
          </Route>
        </Routes>
      </MemoryRouter>,
    );
    ...
    expect(await screen.findByRole("complementary", { name: "全局运行详情" })).toHaveTextContent("台积电旧事件");

    await user.click(screen.getByRole("link", { name: "切换到 Alphabet" }));

    expect(await screen.findByRole("region", { name: "系统正在运行" })).toHaveTextContent("Alphabet 当前事件");
    expect(screen.queryByRole("complementary", { name: "全局运行详情" })).not.toBeInTheDocument();
    expect(screen.queryByText("TSM 专属运行详情")).not.toBeInTheDocument();
  });
  ...
  it("shows the researchability gate as an explicit Case workflow, never a hidden worker state", async () => {
    const user = userEvent.setup();
    let mechanismWriteSaved = false;
    vi.stubGlobal(
      "fetch",
      vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
        const url = String(input);
        if (url.endsWith("/research-cases/event-tsm/mechanism-selection") && init?.method === "POST") {
          mechanismWriteSaved = true;
          return Promise.resolve(new Response(JSON.stringify({ id: "selection-new" }), { status: 201, headers: { "content-type": "application/json" } }));
        }
        if (mechanismWriteSaved && url.includes("/mechanism-protocol")) {
    ...
    expect(
      await screen.findByText("确认结果指标、范围、基线和时间窗"),
    ).toBeVisible();
    expect(
    ...
    const selectTemplate = screen.getByRole("button", { name: "选择此模板" });
    await user.click(selectTemplate);
    expect(await screen.findByText(/机制模板已保存，最新状态暂不可读/)).toBeVisible();
    expect(selectTemplate).toBeDisabled();
    await user.click(selectTemplate);
    const mechanismWrites = (fetch as ReturnType<typeof vi.fn>).mock.calls.filter(
      ([url, init]) => String(url).endsWith("/mechanism-selection") && init?.method === "POST",
    );
    expect(mechanismWrites).toHaveLength(1);
  });

  it.each(["resolve", "reject"] as const)(
    "keeps old mechanism content and writes unavailable when Case A reads %s after switching to pending Case B",
    async (settlement) => {
      const adapter = new MockResearchAdapter();
      const api = new MockResearchOsApi(adapter);
      const template = (await api.mechanismTemplates())[0];
      const ruleInput = (support: string) => ({
        metric_definition_id: "metric-capex",
        expected_direction: "increase" as const,
        support_predicate: support,
        contradiction_predicate: `${support} 的反证`,
      ...
      await api.createVerificationRule(
        "case-a",
        template.edges[0].id,
        ruleInput("旧 Case 支持条件不得写回"),
      );
      await api.createVerificationRule(
        "case-b",
        template.edges[0].id,
        ruleInput("当前 Case 支持条件"),
      );
      const oldProtocol = await api.caseMechanismProtocol("case-a");
      const currentProtocol = await api.caseMechanismProtocol("case-b");
      const pendingOldProtocol = deferred<typeof oldProtocol>();
      const pendingCurrentProtocol = deferred<typeof currentProtocol>();
      const protocolRead = vi
        .spyOn(api, "caseMechanismProtocol")
        .mockImplementation((caseId) =>
          caseId === "case-a"
            ? pendingOldProtocol.promise
            : pendingCurrentProtocol.promise,
        );
      setResearchClient(adapter);
      setResearchOsApi(api);
      ...
      const view = render(
        <>
          <MechanismProtocolPanel caseId="case-a" />
          <MechanismRuleConfig caseId="case-a" />
          <MechanismRuleHistory caseId="case-a" />
        </>,
      );
      await waitFor(() =>
        expect(protocolRead.mock.calls.filter(([caseId]) => caseId === "case-a"))
          .toHaveLength(3),
      );
      view.rerender(
        <>
          <MechanismProtocolPanel caseId="case-b" />
          <MechanismRuleConfig caseId="case-b" />
          <MechanismRuleHistory caseId="case-b" />
        </>,
      );
      expect(screen.queryByDisplayValue("旧 Case 支持条件不得写回")).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: "保存为新的验证规则版本" })).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: "选择此模板" })).not.toBeInTheDocument();

      await act(async () => {
        if (settlement === "resolve") {
          pendingOldProtocol.resolve(oldProtocol);
          await pendingOldProtocol.promise;
        } else {
          pendingOldProtocol.reject(new Error("old Case protocol failed"));
          await pendingOldProtocol.promise.catch(() => undefined);
        }
      });

      expect(screen.queryByDisplayValue("旧 Case 支持条件不得写回")).not.toBeInTheDocument();
      expect(screen.queryByText(/机制模板暂不可读|规则配置暂不可读/)).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: "保存为新的验证规则版本" })).not.toBeInTheDocument();
      expect(protocolRead.mock.calls.filter(
        ([requestedCaseId, options]) => requestedCaseId === "case-b"
          && options?.signal instanceof AbortSignal,
      )).toHaveLength(3);

      await act(async () => {
        pendingCurrentProtocol.resolve(currentProtocol);
        await pendingCurrentProtocol.promise;
      });
      expect(await screen.findByLabelText("支持条件")).toHaveValue("当前 Case 支持条件");
    },
  );

  it("clears visible Case A mechanism forms immediately while Case B is still loading", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    const template = (await api.mechanismTemplates())[0];
    await api.createVerificationRule("case-a", template.edges[0].id, {
      metric_definition_id: "metric-capex",
      expected_direction: "increase",
      support_predicate: "只属于旧 Case 的支持条件",
      contradiction_predicate: "只属于旧 Case 的反证条件",
      allowed_source_roles: ["primary_disclosure"],
      observed_period_start: "2026-01-01",
      observed_period_end: "2026-03-31",
      available_at_deadline: "2026-05-31",
      next_verification_event: "旧 Case 下一次财报",
      reason: "验证切换时立即清空",
    });
    const oldProtocol = await api.caseMechanismProtocol("case-a");
    const pendingCurrentProtocol = deferred<typeof oldProtocol>();
    vi.spyOn(api, "caseMechanismProtocol").mockImplementation((caseId) =>
      caseId === "case-a" ? Promise.resolve(oldProtocol) : pendingCurrentProtocol.promise,
    );
    setResearchClient(adapter);
    setResearchOsApi(api);
    ...
    expect(await screen.findByLabelText("支持条件")).toHaveValue("只属于旧 Case 的支持条件");
    expect(screen.getByRole("button", { name: "保存为新的验证规则版本" })).toBeInTheDocument();
    ...
    expect(screen.queryByDisplayValue("只属于旧 Case 的支持条件")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "保存为新的验证规则版本" })).not.toBeInTheDocument();
    expect(screen.queryByText("旧 Case 下一次财报")).not.toBeInTheDocument();
  });
  ...
  it.each(["resolve", "reject"] as const)(
    "ignores a late outcome-binding save that %s after switching events",
    async (settlement) => {
      const adapter = new MockResearchAdapter();
      const api = new MockResearchOsApi(adapter);
      const pending = deferred<
        Awaited<ReturnType<typeof api.createOutcomeBinding>>
      >();
      const save = vi
        .spyOn(api, "createOutcomeBinding")
      ...
      await act(async () => {
        if (settlement === "resolve") {
          pending.resolve(
            outcomeBindingFixture("pending_review", "binding-event-tsm-late"),
          );
        } else {
          pending.reject(new Error("late save failed"));
        }
        await Promise.resolve();
      });
      ...
      expect(screen.queryByText(/结果绑定已登记为草案/)).not.toBeInTheDocument();
      expect(screen.queryByText(/未能登记结果绑定/)).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: "正在登记…" })).not.toBeInTheDocument();
      expect(screen.getByText("选中命题 · Alphabet 资本开支验证")).toBeVisible();
    },
  );

  it("does not repeat an outcome-binding approval after its projection refresh fails", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    let projectionUnavailable = false;
    vi.spyOn(api, "researchability").mockImplementation(() => projectionUnavailable
    ...
    const approve = vi.spyOn(api, "approveOutcomeBinding").mockImplementation(async () => {
      projectionUnavailable = true;
      return outcomeBindingFixture("approved");
    });
    setResearchClient(adapter);
    setResearchOsApi(api);
    const user = userEvent.setup();
    ...
    await user.type(await screen.findByLabelText("结果绑定审核理由"), "已核对冻结基线");
    const button = screen.getByRole("button", { name: "审核并固定结果绑定" });
    await user.click(button);

    expect(await screen.findByText(/结果绑定已审核，最新状态暂不可读/)).toBeVisible();
    expect(button).toBeDisabled();
    await user.click(button);
  ...
  it.each(["resolve", "reject"] as const)(
    "ignores a late outcome-binding approval that %s after switching events",
    async (settlement) => {
      const adapter = new MockResearchAdapter();
      const api = new MockResearchOsApi(adapter);
      vi.spyOn(api, "researchability").mockResolvedValue({
      ...
      const pending = deferred<
        Awaited<ReturnType<typeof api.approveOutcomeBinding>>
      >();
      const approve = vi
        .spyOn(api, "approveOutcomeBinding")
        .mockReturnValue(pending.promise);
      ...
      await user.type(
        await screen.findByLabelText("结果绑定审核理由"),
        "已核对冻结基线",
      );
      await user.click(
        screen.getByRole("button", { name: "审核并固定结果绑定" }),
      );
      await waitFor(() => expect(approve).toHaveBeenCalledTimes(1));
      await switchProtocolPageToAlphabet(user);
      ...
      await act(async () => {
        if (settlement === "resolve") {
          pending.resolve(outcomeBindingFixture("approved"));
        } else {
          pending.reject(new Error("late approval failed"));
        }
        await Promise.resolve();
      });
      ...
      expect(screen.queryByText(/结果绑定已审核并固定/)).not.toBeInTheDocument();
      expect(screen.queryByText(/审核未完成/)).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: "正在审核…" })).not.toBeInTheDocument();
      expect(screen.getByText("选中命题 · Alphabet 资本开支验证")).toBeVisible();
    },
  );

  it("ignores a failed protocol refresh event after switching events", async () => {
    const adapter = new MockResearchAdapter();
    const api = new MockResearchOsApi(adapter);
    const lateResearchability = deferred<
      Awaited<ReturnType<typeof api.researchability>>
    >();
    let requestCount = 0;
    vi.spyOn(api, "researchability").mockImplementation((thesisId) => {
      requestCount += 1;
      if (requestCount >= 4 && requestCount <= 6) {
        return lateResearchability.promise;
      }
      return Promise.resolve({
        status: "blocked",
        reason_codes: ["missing_outcome_binding"],
        effective_binding_id: null,
        next_action: `补齐 ${thesisId} 的协议`,
      });
    });
    setResearchClient(adapter);
    setResearchOsApi(api);
    ...
    await screen.findByText(/补齐 00000000-0000-4000-8000-000000000001 的协议/);
    window.dispatchEvent(new Event("research-os-protocol-refresh"));
    await switchProtocolPageToAlphabet(user);
    await act(async () => {
      lateResearchability.reject(new Error("late refresh failed"));
      await Promise.resolve();
    });
  ...
  it("keeps the newly saved monitor version and scope visible in the configuration form", async () => {
    ...
    const monitorSave = fetchMock.mock.calls.find(
      ([url, init]) => String(url).endsWith("/research-cases/event-tsm/monitor") && init?.method === "PUT",
    );
    expect(JSON.parse(String(monitorSave?.[1]?.body))).not.toHaveProperty("actor");
  });
  ...
  it("edits a rule from the current Case version and never posts to a global template edge", async () => {
    ...
    let ruleWriteSaved = false;
    const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (url.includes("/verification-rules") && init?.method === "POST") {
        ruleWriteSaved = true;
        return Promise.resolve(new Response(JSON.stringify(rule), { status: 201, headers: { "content-type": "application/json" } }));
      }
      if (ruleWriteSaved && url.includes("/mechanism-protocol")) {
        return Promise.reject(new Error("projection unavailable"));
      }
      const body = url.includes("/researchability")
        ? {
    ...
    const ruleSave = fetchMock.mock.calls.find(
      ([url, init]) => String(url).includes("/verification-rules") && init?.method === "POST",
    );
    expect(JSON.parse(String(ruleSave?.[1]?.body))).not.toHaveProperty("reviewer");
    expect(await screen.findByText(/验证规则已保存，最新状态暂不可读/)).toBeVisible();
    const saveRule = screen.getByRole("button", { name: "保存为新的验证规则版本" });
    expect(saveRule).toBeDisabled();
    await user.click(saveRule);

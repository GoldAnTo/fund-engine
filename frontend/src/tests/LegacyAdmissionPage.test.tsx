...
describe("legacy Case admission", () => {
  ...
  it("explains admission requirements before appending the explicit admission", async () => {
    ...
    const api = {
      session: vi.fn().mockResolvedValue({ tenant_id: "team-a", roles: ["case_administrator"] }),
    ...
    expect(admitLegacyCase).toHaveBeenCalledWith("case-legacy", expect.objectContaining({
      tenant_id: "team-a",
      initial_document_version_id: "doc-legacy",
      reason: "迁移记录已核验",
    }));

...
describe("EventCreatePage automatic research entry", () => {
  ...
  it("starts automatic research from one supported uploaded original", async () => {
    const user = userEvent.setup();
    const start = vi.spyOn(adapter, "startAutomaticResearchFromUpload");
    renderCreatePage();
    const file = new File(["营收增速：30%"], "quarterly-update.md", {
      type: "text/markdown",
    });

    await user.upload(screen.getByLabelText("上传原始文件"), file);

    expect(screen.getByText("已选择：quarterly-update.md")).toBeVisible();
    expect(screen.getByRole("button", { name: "开始自动研究" })).toBeEnabled();
    await user.click(screen.getByRole("button", { name: "开始自动研究" }));

    await waitFor(() => expect(start).toHaveBeenCalledWith({ file, input: "" }));
    expect(await screen.findByText("自动研究过程")).toBeVisible();
  });

  it("rejects unsupported or oversized uploads before starting research", async () => {
    const user = userEvent.setup();
    const start = vi.spyOn(adapter, "startAutomaticResearchFromUpload");
    renderCreatePage();

    await user.upload(
      screen.getByLabelText("上传原始文件"),
      new File(["data"], "data.xlsx", { type: "application/vnd.ms-excel" }),
    );

    expect(await screen.findByRole("alert")).toHaveTextContent(
      "仅支持 PDF、TXT、Markdown 或 CSV 文件。",
    );

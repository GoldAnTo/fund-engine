...
describe("HttpResearchAdapter", () => {
  it("resumes a completed protocol with the frozen scope contract and no client actor", async () => {
    let requestUrl = "";
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return jsonResponse({
        ...
        case_id: "case-a",
        scope_version_id: "scope-2",
        research_run_id: "run-2",
        state: "planning_acquisition",
        user_stage: "acquisition",
        current_system_action: "正在按新范围规划资料获取",
        system_action_reason: "研究协议已完成",
        next_action: null,
        version: 5,
      }, true, 202);
    }));
    ...
    const result = await adapter.resumeProtocolWorkflow("case-a", "scope-2", {
      scopeVersion: 2,
      idempotencyKey: "resume-protocol-case-a-v2",
    });

    expect(requestUrl).toBe("http://api.test/api/v1/event-research/case-a/workflow/resume-protocol");
    expect(requestBody).toEqual({
      scope_version_id: "scope-2",
      scope_version: 2,
      idempotency_key: "resume-protocol-case-a-v2",
    });
    expect(requestBody).not.toHaveProperty("actor");
    expect(result).toMatchObject({ state: "planning_acquisition", version: 5 });
  });

  it("starts a published material continuation without sending a client identity", async () => {
    let requestBody: Record<string, unknown> | null = null;
    vi.stubGlobal("fetch", vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      requestBody = JSON.parse(String(init?.body)) as Record<string, unknown>;
      return jsonResponse({
        run_id: "run-next",
        lifecycle: { status: "researching", active_run_id: "run-next", current_round: 0, status_summary: "继续研究", current_gap: null, next_human_action: null },
      }, true, 201);
    }));

    const adapter = new HttpResearchAdapter({ baseUrl: "http://api.test/api/v1" });
    await adapter.continueEventResearch({
      caseId: "case-a",
    ...
    expect(requestBody).not.toHaveProperty("triggered_by");
  });
  ...
  it("freezes an original upload through multipart without inventing a JSON text snapshot", async () => {
    ...
    expect(form.has("actor")).toBe(false);
  ...
  it("serializes an omitted new-event research protocol flag as false", async () => {
    let requestBody: Record<string, unknown> | null = null;
    vi.stubGlobal("fetch", vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const adapter = new HttpResearchAdapter({ baseUrl: "http://api.test/api/v1" });
    await adapter.createEventResearch({ rawInput: "公司更新资本开支指引。", eventTitle: "资本开支更新", companyName: null, ticker: null, eventAt: null, marketReaction: null, summary: null, researchQuestion: "影响是什么？", candidateFactors: ["因素一", "因素二", "因素三"], confirmationRequired: true });

    expect((requestBody as Record<string, unknown> | null)?.research_protocol_required).toBe(false);
    expect(requestBody).not.toHaveProperty("created_by");
  });

  it("preserves an explicit policy-required new-event research protocol flag", async () => {
    let requestBody: Record<string, unknown> | null = null;
    vi.stubGlobal("fetch", vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      requestBody = JSON.parse(String(init?.body));
      return jsonResponse({ case_id: "event-1", brief_id: "brief-1", lifecycle: { status: "awaiting_key_review", active_run_id: null, current_round: 0, status_summary: "等待核验", current_gap: null, next_human_action: "核验原文资料并完成研究协议" } });
    ...
    const adapter = new HttpResearchAdapter({ baseUrl: "http://api.test/api/v1" });
    await adapter.createEventResearch({ rawInput: "公司更新资本开支指引。", eventTitle: "资本开支更新", companyName: null, ticker: null, eventAt: null, marketReaction: null, summary: null, researchQuestion: "影响是什么？", candidateFactors: ["因素一", "因素二", "因素三"], confirmationRequired: true, researchProtocolRequired: true });
  ...
  it("maps conclusion-first event workbench progress, current scope, and edit action", async () => {
    vi.stubGlobal(
      ...
      vi.fn(async () => jsonResponse({
        ...
        factors: [{
          ...
          reviewed_contradiction_count: 1, automatically_admitted_support_count: 3,
          automatically_admitted_contradiction_count: 2, pending_proposal_count: 1, current_gap: "缺少反证",
        }],
        evidence: [],
        progress: { reviewed_count: 3, automatically_admitted_count: 2, pending: 2, invalid_source: 1, current_gap: "缺少反证" },
    ...
    expect(view.progress).toEqual({ reviewedCount: 3, automaticallyAdmittedCount: 2, pending: 2, invalidSource: 1, currentGap: "缺少反证" });
    ...
    expect(view.factors[0]).toMatchObject({
      ...
      automaticallyAdmittedSupportCount: 3,
      automaticallyAdmittedContradictionCount: 2,
  ...
  it("maps event evidence to a Case-owned frozen-document action", async () => {
    vi.stubGlobal(
      ...
      vi.fn(async () => jsonResponse({
        ...
        progress: { reviewed_count: 1, automatically_admitted_count: 0, pending: 0, invalid_source: 0, current_gap: null },

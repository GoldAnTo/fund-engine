...
describe("HttpResearchAdapter", () => {
  ...
  it("maps report intake once and includes a governed source contract in every write", async () => {
    const fetchMock = vi.fn(async (url: string) => {
      if (url.includes("/pdf?")) return jsonResponse({ case: { id: "case-pdf", title: "PDF 研报" }, document: { id: "doc-pdf", title: "PDF 研报" }, state: "pending_candidate_extraction", next_action: "extract_candidates", blocking_reason: null, needs_text_or_pages: false });
      if (url.endsWith("/supplement")) return jsonResponse({ case: { id: "case-text", title: "测试研报" }, document: { id: "doc-text", title: "补充快照" }, state: "artifact_extraction_unavailable", next_action: "view_saved_snapshot", blocking_reason: "已保存为 Case 内快照。", needs_text_or_pages: false, recovery_snapshot: { id: "artifact-1", input_kind: "recovery_text", source_identity: "case:case-text", content_sha256: "sha256", claimed_page_reference: "第 3 页", parser: "manual-v1" } });
      if (url.endsWith("/intake")) return jsonResponse({ case: { id: "case-text", title: "测试研报" }, primary_document: { id: "doc-text", title: "测试研报", input_kind: "web_content" }, supplement_documents: [], supplement_artifacts: [{ id: "artifact-1", claimed_page_reference: "第 3 页" }], state: "artifact_extraction_unavailable", blocking_reason: "已保存为 Case 内快照。", next_action: "view_saved_snapshot" });
      ...
      return jsonResponse({ case: { id: "case-text", title: "测试研报" }, document: { id: "doc-text", title: "测试研报" }, state: "pending_candidate_extraction", next_action: "extract_candidates", blocking_reason: null, needs_text_or_pages: false });
    });
    vi.stubGlobal("fetch", fetchMock);
    const adapter = new HttpResearchAdapter({ baseUrl: "http://api.test/api/v1" });

    const pasted = await adapter.createReportResearch({ inputKind: "pasted_text", title: "测试研报", content: "观点", sourceContractId: "contract-1" });
    const pdfFile = new File(["%PDF-1.4"], "report.pdf", { type: "application/pdf" });
    const pdf = await adapter.createReportResearchPdf({ title: "PDF 研报", file: pdfFile, sourceContractId: "contract-1" });
    const scopes = await adapter.listReportResearchScopes("case-text");
    const supplemented = await adapter.supplementReportResearch({ caseId: "case-text", documentId: "doc-text", content: "补充观点", pageReference: "第 3 页", sourceContractId: "contract-1" });
    const intake = await adapter.getReportResearchIntake("case-text");

    expect(pasted).toMatchObject({ caseId: "case-text", documentId: "doc-text", state: "pending_candidate_extraction", nextAction: { kind: "extract_candidates" } });
    expect(pdf).toMatchObject({ caseId: "case-pdf", documentId: "doc-pdf", state: "pending_candidate_extraction" });
    expect(supplemented).toMatchObject({ caseId: "case-text", documentId: "doc-text", state: "artifact_extraction_unavailable", recoverySnapshot: { id: "artifact-1" } });
    expect(intake).toMatchObject({ caseId: "case-text", caseTitle: "测试研报", primaryDocument: { inputKind: "web_content" }, state: "artifact_extraction_unavailable", nextAction: { kind: "view_saved_snapshot" }, supplementArtifacts: [{ id: "artifact-1", claimedPageReference: "第 3 页" }] });
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(2, expect.stringContaining("/report-research/pdf?title=PDF+%E7%A0%94%E6%8A%A5&filename=report.pdf&source_contract_id=contract-1"), expect.objectContaining({ method: "POST", headers: expect.objectContaining({ "Content-Type": "application/pdf" }), body: pdfFile }));
    expect(fetchMock).toHaveBeenNthCalledWith(3, "http://api.test/api/v1/report-research/case-text/scopes", expect.anything());
    expect(fetchMock).toHaveBeenNthCalledWith(4, "http://api.test/api/v1/report-research/case-text/documents/doc-text/supplement", expect.objectContaining({ method: "POST", body: JSON.stringify({ content: "补充观点", page_reference: "第 3 页", source_contract_id: "contract-1" }) }));
    expect(fetchMock).toHaveBeenNthCalledWith(5, "http://api.test/api/v1/report-research/case-text/intake", expect.anything());
  });

  it("fails closed with a typed error when report intake next_action is unknown", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({
      case: { id: "case-unknown", title: "未知动作研报" },
      primary_document: { id: "doc-unknown", title: "未知动作研报", input_kind: "pdf_upload" },
      supplement_documents: [], supplement_artifacts: [], state: "pending_candidate_extraction",
      blocking_reason: null, next_action: "auto_publish",
    })));
    const adapter = new HttpResearchAdapter({ baseUrl: "http://api.test/api/v1" });

    await expect(adapter.getReportResearchIntake("case-unknown")).rejects.toMatchObject({
      name: "PageStateError", kind: "backend_unavailable", message: "资料接入状态不兼容，请重新读取。",
    });
  });

  it("fails closed with a typed error when a create response has an unknown intake state", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({
      case: { id: "case-unknown", title: "未知状态研报" },
      document: { id: "doc-unknown", title: "未知状态研报" },
      state: "auto_published",
      next_action: "extract_candidates",
      blocking_reason: null,
      needs_text_or_pages: false,
    })));
    const adapter = new HttpResearchAdapter({ baseUrl: "http://api.test/api/v1" });

    await expect(adapter.createReportResearch({
  ...
  it("maps unlisted relations, complete partial-fund fields, observations and reviews", async () => {
    ...
    expect(trace.factors[0].relations[0]).toMatchObject({ companyType: "unlisted_supplier", stocks: [], effectiveStatus: "verified", isHighImpact: false, isReviewable: false });

...
export function EventResearchWorkbenchScreen() {
  ...
  return <main className="prototype-screen event-workbench">
    ...
    <section className="event-workbench-grid"><div className="prototype-paper event-factors"><p className="section-kicker">竞争性因素</p><h2>系统正在区分什么</h2>{view.factors.map((factor) => { const evidence = view.evidence.filter((item) => item.factorStatement === factor.statement); const confidence = factorConfidence(factor.reviewedSupportCount, factor.reviewedContradictionCount, view.progress.pending); return <article className="event-factor-card" key={factor.position}><strong>{factor.position}. {factor.statement}</strong>{factor.description ? <p className="event-factor-card__description">{factor.description}</p> : null}<span>支持 {factor.reviewedSupportCount} · 反证 {factor.reviewedContradictionCount}</span><small>判断：{confidence}</small>{factor.currentGap ? <small>缺口：{factor.currentGap}</small> : null}{view.conclusion.state === "ai_draft" ? <div className="event-factor-card__evidence"><b>结论置信度：{confidence}</b><p>支持证据：{evidence.filter((item) => item.role === "supports").map((item) => item.excerpt).slice(0, 2).join("；") || "暂无已展示的支持证据"}</p><p>反驳证据：{evidence.filter((item) => item.role === "contradicts").map((item) => item.excerpt).slice(0, 2).join("；") || "暂无已展示的反驳证据"}</p></div> : null}</article>; })}{view.factors.length === 0 || view.lifecycle.status === "exhausted" ? <section className="event-evidence-boundary" aria-label="证据边界"><h3>当前证据边界</h3><p>当前缺什么：{view.progress.currentGap ?? "尚无可确认的范围缺口。"}</p><p>已检查：已核验 {view.progress.verified} 条证据；已排除 {view.progress.invalidSource} 条无效来源，它们不计入结论。</p><p>下一次系统会尝试：{nextResearchAttempt(view)}</p></section> : null}</div><aside className="prototype-paper event-next-action"><p className="section-kicker">下一步</p><h2>{view.nextAction.label}</h2><p>{view.lifecycle.summary}</p>{view.nextAction.kind === "edit_factors" ? <button ref={factorEditTrigger} className="prototype-button primary" type="button" onClick={() => setIsEditingFactors(true)}>{view.nextAction.label}</button> : view.nextAction.kind === "review_evidence" ? <Link className="prototype-button primary" to={`/events/${caseId}/review`}>{view.nextAction.label}</Link> : view.nextAction.kind === "review_conclusion" ? <Link className="prototype-button primary" to={`/events/${caseId}/conclusion`}>{view.nextAction.label}</Link> : view.nextAction.kind === "view_conclusion_change" ? <Link className="prototype-button primary" to={`/events/${caseId}/conclusion`}>{view.nextAction.label}</Link> : <Link className="prototype-button primary" to={`/events/${caseId}/basis`}>查看研究依据</Link>}</aside></section>

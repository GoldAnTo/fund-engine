...
function GraphCanvas({ graph, onSelect }: { graph: Pick<ReportWikiGraph, "nodes" | "edges">; onSelect: (node: ReportWikiNode) => void }) {
  ...
  return <svg className="report-wiki-svg" viewBox="0 0 820 330" role="img" aria-label="当前研究范围的关系图谱，另有下方结构化路径列表">
...
export function ReportWikiGraphScreen() {
  ...
  useEffect(() => { void (async () => { if (!caseId) return; try { const base = await researchClient.getReportWikiGraph!(caseId); const relationId = base.factors.find((factor) => factor.relationId)?.relationId ?? null; if (relationId) { const selected = await researchClient.getReportWikiGraph!(caseId, { relationId }); setGraph(selected); setSelectedRelationId(relationId); } else setGraph(base); } catch (reason) { setError(reason instanceof Error ? reason.message : "无法加载图谱。请稍后重试。"); } })(); }, [caseId]);
  if (error) return <main className="prototype-screen report-wiki-screen"><section className="report-state report-state--error" role="alert"><h1>无法加载图谱</h1><p>{error}</p><button className="prototype-button primary" type="button" onClick={() => void load(mode, selectedRelationId)}>重新加载图谱</button></section></main>;
  ...
  const nodeHref = safeExternalHref(selectedNode?.sourceLocator ?? null);
  return <main className="prototype-screen report-wiki-screen">
    ...
    <div className="report-wiki-layout"><section className="report-wiki-canvas" aria-labelledby="report-wiki-canvas-title"><header><p className="section-kicker">关系概览</p><h2 id="report-wiki-canvas-title">从主张到资产映射</h2><p>图谱用于定位路径，不替代来源与证据判断。</p></header><GraphCanvas graph={graph} onSelect={setSelectedNode} /></section><aside className="report-wiki-inspector" aria-live="polite"><p className="section-kicker">路径定位</p><h2>{selectedNode ? selectedNode.label : "选择一个节点或路径"}</h2>{selectedNode ? <>{nodeHref ? <a href={nodeHref} target="_blank" rel="noopener noreferrer">查看安全来源</a> : <p role="status">{selectedNode.sourceLocator ?? `当前状态：${selectedNode.status === "verified" ? "已验证" : "仍待核验"}`}</p>}</> : <p>点击图谱节点或下方结构化路径，查看其安全来源定位或当前状态。</p>}</aside></div>
...
export function ReportEmbedScreen() {
  const { caseId = "" } = useParams();
  const [graph, setGraph] = useState<ReportEmbedWikiGraph | null>(null);
  const [error, setError] = useState<string | null>(null);
  useEffect(() => { const token = readHashToken(); if (!token || !caseId) { setError("嵌入访问未获授权或已失效。请向提供方获取新的只读链接。"); return; } window.history.replaceState(null, document.title, window.location.pathname + window.location.search); researchClient.getReportEmbedWiki!(caseId, token).then(setGraph).catch(() => setError("嵌入访问未获授权或已失效。请向提供方获取新的只读链接。")); }, [caseId]);

...
function GraphCanvas({ graph }: { graph: Pick<ReportWikiGraph, "nodes" | "edges"> }) {
  ...
  return <svg className="report-wiki-svg" viewBox={`0 0 820 ${height}`} role="group" aria-label="当前研究范围的关系图谱，另有下方结构化路径列表">
    ...
    {layout.map(({ node, x, y }) => <g key={node.id} className={`report-wiki-svg__node report-wiki-svg__node--${node.status}`} aria-hidden="true"><rect x={x - 54} y={y - 25} width="108" height="50" rx="6" /><text x={x} y={y - 3} textAnchor="middle">{node.kind === "report_claim" ? "研报主张" : node.kind === "market_window" ? "市场窗口" : node.kind === "fund" ? "中国基金" : node.kind === "evidence" ? "证据/反证" : "公司"}</text><text x={x} y={y + 14} textAnchor="middle">{node.label.length > 13 ? `${node.label.slice(0, 12)}…` : node.label}</text></g>)}
  </svg>;
}
...
export function ReportWikiGraphScreen() {
  ...
  const availableRelationId = graph.factors.find((factor) => factor.relationId)?.relationId ?? null;
  return <main className="prototype-screen report-wiki-screen">
    ...
    <section className="report-wiki-toolbar" aria-label="图谱范围"><div role="group" aria-label="图谱范围"><button type="button" className="report-toggle" aria-pressed={mode === "path" && Boolean(availableRelationId)} disabled={!availableRelationId} onClick={() => { if (availableRelationId) void load("path", selectedRelationId ?? availableRelationId); }}>当前路径</button><button type="button" className="report-toggle" aria-pressed={mode === "all"} onClick={() => void load("all")}>所有关系</button></div><span>{!availableRelationId ? "当前范围没有可选关系路径，已显示全部关系" : mode === "path" ? "只显示选中因素的可核验路径" : "显示当前不可变研究范围的全部关系"}</span></section>
    <div className="report-wiki-layout"><section className="report-wiki-canvas" aria-labelledby="report-wiki-canvas-title"><header><p className="section-kicker">关系概览</p><h2 id="report-wiki-canvas-title">从主张到资产映射</h2><p>图谱用于定位路径，不替代来源与证据判断。</p></header><GraphCanvas graph={graph} /></section><aside className="report-wiki-inspector" aria-live="polite"><p className="section-kicker">路径定位</p><h2>{selectedNode ? selectedNode.label : "选择一个节点或路径"}</h2>{selectedNode ? <>{source.href ? <a href={source.href} target="_blank" rel="noopener noreferrer">{source.label}</a> : <p role="status">{source.label}</p>}</> : <p>通过下方结构化路径，查看其安全来源定位或当前状态。</p>}</aside></div>
...
export function ReportEmbedScreen({ embedClient }: { embedClient?: Pick<ReturnType<typeof configuredEmbedResearchClient>, "getReportEmbedWiki"> } = {}) {
  ...
  if (tokenRef.current === undefined) tokenRef.current = readHashToken();
  useEffect(() => {
    ...
    setGraph(null);
    setError(null);
    if (!token || !caseId) { setError("嵌入访问未获授权或已失效。请向提供方获取新的只读链接。"); return () => { cancelled = true; }; }
    ...
    try {
      ...
      if (!cancelled && requestSequence.current === sequence) setError(reason instanceof Error && reason.message === EMBED_API_CONFIGURATION_ERROR ? EMBED_API_CONFIGURATION_ERROR : "嵌入访问未获授权或已失效。请向提供方获取新的只读链接。");
      return () => { cancelled = true; };
    }
    client.getReportEmbedWiki(caseId, token).then((nextGraph) => { if (!cancelled && requestSequence.current === sequence) { setGraph(nextGraph); setError(null); } }).catch((reason) => { if (!cancelled && requestSequence.current === sequence) setError(reason instanceof Error && reason.message === EMBED_API_CONFIGURATION_ERROR ? EMBED_API_CONFIGURATION_ERROR : "嵌入访问未获授权或已失效。请向提供方获取新的只读链接。"); });
    return () => { cancelled = true; };
  }, [caseId, embedClient]);

...
export class HttpResearchAdapter implements ActiveResearchClient {
  ...
  async startAutomaticResearchFromUpload(
    input: StartAutomaticResearchUpload,
  ): Promise<AutomaticResearchStart> {
    const form = new FormData();
    form.append("file", input.file);
    if (input.input?.trim()) form.append("input", input.input.trim());
    const dto = await this.postForm<Schemas["AutomaticResearchStartResponse"]>(
      "/automatic-research/uploaded",
      form,
    );
    return this.mapAutomaticResearchStart(dto);
  }
  ...
  async getAutomaticResearch(caseId: string): Promise<AutomaticResearchView> {
    ...
    return {
      ...
      narrative: {
        currentAction: dto.narrative.current_action,
        completedCount: dto.narrative.completed_count,
        totalCount: dto.narrative.total_count,
        nextAction: dto.narrative.next_action,
        elapsedSeconds: dto.narrative.elapsed_seconds,
        estimatedRemainingSecondsMin: dto.narrative.estimated_remaining_seconds_min,
        estimatedRemainingSecondsMax: dto.narrative.estimated_remaining_seconds_max,
      },
      activities: dto.activities.map((activity) => ({
        label: activity.label,
        count: activity.count,
        technicalDetails: activity.technical_details.map((detail) => ({
          occurredAt: detail.occurred_at,
          workItem: detail.work_item,
          internalStatus: detail.internal_status,
        })),
      })),
      exceptions: dto.exceptions.map((exception) => ({
        ...
        systemAction: exception.system_action,
      })),
      factors: dto.factors.map((factor) => ({
        statement: factor.statement,
        classification: factor.classification,
        rankingReason: factor.ranking_reason,
        supportCount: factor.support_count,
        counterEvidenceCount: factor.counter_evidence_count,
        evidenceGap: factor.evidence_gap,
      })),

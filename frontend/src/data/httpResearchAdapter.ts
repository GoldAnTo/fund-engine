...
export class HttpResearchAdapter implements ResearchClient {
  ...
  async getEventImpactTrace(caseId: string): Promise<EventImpactTrace> {
    ...
    const mapRelation = (relation: any) => ({ relationId: relation.relation_id, companyId: relation.company_id, companyName: relation.company_name, companyType: relation.company_type, relationKind: relation.relation_kind, direction: relation.direction, mechanism: relation.mechanism, status: relation.status, effectiveStatus: relation.effective_status, sourceStatementId: relation.source_statement_id ?? null, review: relation.review ? { outcome: relation.review.outcome, reason: relation.review.reason, reviewer: relation.review.reviewer, createdAt: relation.review.created_at } : null, stocks: (relation.stocks ?? []).map((stock: any) => ({ stockId: stock.stock_id, code: stock.code, name: stock.name, market: stock.market })), observations: (relation.observations ?? []).map(mapObservation), fundExposure: (relation.fund_exposure ?? []).map(mapFund) });

...
import { fetchWithOidc, type OidcTokenProvider } from "../auth/oidc";
import {
  EventWorkflowError,
  mapEventWorkflow,
  mapWorkflowCommand,
  mapWorkflowLedgerPage,
  type EventWorkflow,
...
type WorkflowNotInitializedEnvelopeDTO = components["schemas"]["WorkflowNotInitializedEnvelopeDTO"];
...
export class HttpResearchAdapter implements ActiveResearchClient {
  constructor(
    private readonly options: { baseUrl: string; tokenProvider?: OidcTokenProvider },
  ) {}

  private async get<T>(path: string, options?: { signal?: AbortSignal }): Promise<T> {
    let response: Response;
    try {
      response = await fetchWithOidc(`${this.options.baseUrl}${path}`, {
        credentials: "include",
    ...
    if (!response.ok) {
      const payload = (await response.json().catch(
        () => null,
      )) as ErrorEnvelopeDTO | WorkflowNotInitializedEnvelopeDTO | null;
      const code = payload?.error?.code;
      if (code === "workflow_not_initialized") {
        const workflowError = payload as WorkflowNotInitializedEnvelopeDTO;
        throw new EventWorkflowError(
          code,
          workflowError.error.message,
          workflowError.error.details.safe_action,
        );
      }
  ...
  private mapDocument(
    ...
    return {
      ...
      source_contract: dto.source_contract ? {
        ...
        provider_record: dto.source_contract.provider_record ? {
          provider_name: dto.source_contract.provider_record.provider_name,
  ...
  async cancelResearchRun(runId: string): Promise<ResearchRunSummary> {
    return this.post<ResearchRunSummary>(`/research-runs/${encodeURIComponent(runId)}/cancel`, { change_reason: "用户停止本次运行" });
  }
  ...
  async getEventWorkflow(
    ...
    const dto = await this.get<Schemas["ResearchWorkflowResponse"]>(
      `/event-research/${encodeURIComponent(caseId)}/workflow`,
      options,
    );
    return mapEventWorkflow(dto);
  }

  async getCaseRuntimeStatus(
    caseId: string,
    options?: { signal?: AbortSignal },
  ): Promise<CaseRuntimeStatus> {
    const dto = await this.get<Schemas["CaseRuntimeStatusDTO"]>(
      `/event-research/${encodeURIComponent(caseId)}/runtime-status`,
      options,
    );
    return {
      runtimeStatus: dto.runtime_status,
      requiredServices: dto.required_services.map((service) => ({
        name: service.name,
      ...
      durableCheckpoint: {
        workflowState: dto.durable_checkpoint.workflow_state,
        userStage: dto.durable_checkpoint.user_stage,
        version: dto.durable_checkpoint.version,
        systemAction: dto.durable_checkpoint.system_action,
        savedAt: dto.durable_checkpoint.saved_at,
        lastTransition: dto.durable_checkpoint.last_transition,
        lastTransitionAt: dto.durable_checkpoint.last_transition_at,
      },
  ...
  async getEventWorkflowLedger(
    ...
    const dto = await this.get<Schemas["WorkflowLedgerPageDTO"]>(
      `/event-research/${encodeURIComponent(caseId)}/workflow/ledger?${params.toString()}`,
      { signal: options?.signal },
    );
    return mapWorkflowLedgerPage(dto);
  }
  ...
  async confirmEventWorkflow(
    ...
    const dto = await this.post<Schemas["ConfirmResearchWorkflowResponse"]>(
      `/event-research/${encodeURIComponent(caseId)}/workflow/confirm`,
      {
        scope_version_id: scopeVersionId,
        ...(options?.scopeVersion ? { scope_version: options.scopeVersion } : {}),
        idempotency_key: options?.idempotencyKey
          ?? `workflow:confirm:${caseId}:${scopeVersionId}`,
      },
    );
    return mapWorkflowCommand(dto);
  }
  ...
  async decideEventScope(
    ...
    const idempotencyKey = decision.idempotencyKey
      ?? `workflow:decision:${caseId}:${decision.kind}:${globalThis.crypto.randomUUID()}`;
    const dto = await this.post<Schemas["ConfirmResearchWorkflowResponse"]>(
      `/event-research/${encodeURIComponent(caseId)}/workflow/decision`,
      {
        kind: decision.kind,
        reason: decision.reason,
        expected_version: decision.expectedVersion,
        idempotency_key: idempotencyKey,
      } satisfies Schemas["DecideResearchWorkflowRequest"],
      options,
    );
    return mapWorkflowCommand(dto);
  }
  ...
  async resumeProtocolWorkflow(
    ...
    const dto = await this.post<Schemas["ConfirmResearchWorkflowResponse"]>(
      `/event-research/${encodeURIComponent(caseId)}/workflow/resume-protocol`,
      {
        scope_version_id: scopeVersionId,
        ...(options?.scopeVersion ? { scope_version: options.scopeVersion } : {}),
        idempotency_key: options?.idempotencyKey
          ?? `workflow:resume-protocol:${caseId}:${scopeVersionId}`,
      },
    );
    return mapWorkflowCommand(dto);
  }
  ...
  async getEventWorkbench(caseId: string): Promise<EventWorkbench> {
    const dto = await this.get<{
      ...
      factors: Array<{ thesis_id: string; statement: string; description?: string | null; position: number; reviewed_support_count: number; reviewed_contradiction_count: number; automatically_admitted_support_count: number; automatically_admitted_contradiction_count: number; pending_proposal_count?: number; current_gap: string | null }>;
      evidence: Array<{
      ...
      progress: { reviewed_count: number; automatically_admitted_count: number; pending: number; invalid_source: number; current_gap: string | null };
    ...
    return {
      ...
      progress: { reviewedCount: dto.progress.reviewed_count, automaticallyAdmittedCount: dto.progress.automatically_admitted_count, pending: dto.progress.pending, invalidSource: dto.progress.invalid_source, currentGap: dto.progress.current_gap },
  ...
  async updateEventResearchScope(input: { caseId: string; factors: Array<EventResearchScope["factors"][number] | string>; changeReason: string }): Promise<EventResearchScope & { reclassifiedEvidenceCount: number }> {
    const dto = await this.requestJson<{
      ...
      method: "PUT",
      body: JSON.stringify({ factors: input.factors, change_reason: input.changeReason }),
    });
    return {
      version: dto.version,

...
export class HttpResearchAdapter implements ResearchClient {
  ...
  async getEventWorkbench(caseId: string): Promise<EventWorkbench> {
    const dto = await this.get<{
      ...
      factors: Array<{ statement: string; description?: string | null; position: number; reviewed_support_count: number; reviewed_contradiction_count: number; current_gap: string | null }>;
      evidence: unknown[];
    ...
    return {
      ...
      factors: dto.factors.map((factor) => ({ statement: factor.statement, description: factor.description, position: factor.position, reviewedSupportCount: factor.reviewed_support_count, reviewedContradictionCount: factor.reviewed_contradiction_count, currentGap: factor.current_gap })),
      evidence,

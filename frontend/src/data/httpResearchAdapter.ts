...
import type {
  ...
  ReportIntakeActionKind,
  ReportIntakeDocument,
  ReportIntakeNextAction,
  ReportIntakeState,
  ReportResearchIntake,
...
type CausalStepDTO = Schemas["app__schemas__v1__cases__CausalStepDTO"];
type ReviewQueueResponseDTO = Schemas["app__schemas__v1__commands__ReviewQueueResponse"];

type ReportResearchCreatedResponseDTO = Schemas["ReportResearchCreatedResponse"];
type WireReportAction = ReportResearchCreatedResponseDTO["next_action"];
type WireReportState = ReportResearchCreatedResponseDTO["state"];

const reportIntakeActionCopy: Record<WireReportAction, { label: string; unlockMessage: string }> = {
  supplement_text: { label: "补充正文并标注页码", unlockMessage: "冻结独立补充快照后，才可进入候选陈述抽取。" },
  extract_candidates: { label: "开始抽取待审核陈述", unlockMessage: "完成后可审核关键陈述，尚不会生成正式研究结论。" },
  view_saved_snapshot: { label: "查看已保存快照", unlockMessage: "可确认该 Case 内快照，候选抽取仍需受控适配器。" },
};

const REPORT_INTAKE_STATES: readonly WireReportState[] = [
  "needs_supplement",
  "pending_candidate_extraction",
  "artifact_extraction_unavailable",
];
const REPORT_DOCUMENT_INPUT_KINDS: readonly ReportIntakeDocument["inputKind"][] = [
  "pdf_upload",
  "pasted_text",
  "web_content",
  "recovery_text",
...
function reportIntakeContractError(): PageStateError {
  return new PageStateError("backend_unavailable", "资料接入状态不兼容，请重新读取。");
}

function asRecord(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw reportIntakeContractError();
  return value as Record<string, unknown>;
}

function requiredString(value: unknown): string {
  if (typeof value !== "string" || !value) throw reportIntakeContractError();
  return value;
}

function optionalString(value: unknown): string | null {
  if (value === undefined || value === null) return null;
  if (typeof value !== "string") throw reportIntakeContractError();
  return value;
}
...
function requiredBoolean(value: unknown): boolean {
  if (typeof value !== "boolean") throw reportIntakeContractError();
  return value;
}

function mapReportIntakeState(value: unknown): ReportIntakeState {
  if (typeof value !== "string" || !(REPORT_INTAKE_STATES as readonly string[]).includes(value)) throw reportIntakeContractError();
  return value as ReportIntakeState;
}

function mapReportIntakeAction(kind: unknown): ReportIntakeNextAction {
  if (typeof kind !== "string" || !(kind in reportIntakeActionCopy)) throw reportIntakeContractError();
  const knownKind = kind as ReportIntakeActionKind;
  return { kind: knownKind, ...reportIntakeActionCopy[knownKind] };
}

function mapReportIntakeDocument(value: unknown): ReportIntakeDocument {
  const document = asRecord(value);
  const inputKind = requiredString(document.input_kind);
  if (!(REPORT_DOCUMENT_INPUT_KINDS as readonly string[]).includes(inputKind)) throw reportIntakeContractError();
  return {
    id: requiredString(document.id),
    title: requiredString(document.title),
    inputKind: inputKind as ReportIntakeDocument["inputKind"],
  };
}

function mapReportIntakeArtifact(value: unknown): import("../domain/eventResearch").ReportIntakeSupplementArtifact {
  const artifact = asRecord(value);
  return { id: requiredString(artifact.id), claimedPageReference: optionalString(artifact.claimed_page_reference) };
}

function mapCreatedReportRecoverySnapshot(value: unknown): import("../domain/eventResearch").ReportIntakeSupplementArtifact {
  const snapshot = asRecord(value);
  if (snapshot.input_kind !== "recovery_text") throw reportIntakeContractError();
  requiredString(snapshot.source_identity);
  requiredString(snapshot.content_sha256);
  requiredString(snapshot.parser);
  return {
    id: requiredString(snapshot.id),
    claimedPageReference: nullableString(snapshot.claimed_page_reference),
  };
}

/** The only mapping of the report-intake read DTO into the UI domain. */
function mapReportIntake(dto: unknown): ReportResearchIntake {
  const intake = asRecord(dto);
  const caseDto = asRecord(intake.case);
  const state = mapReportIntakeState(intake.state);
  const supplementDocuments = intake.supplement_documents ?? [];
  const supplementArtifacts = intake.supplement_artifacts ?? [];
  if (!Array.isArray(supplementDocuments) || !Array.isArray(supplementArtifacts)) throw reportIntakeContractError();
  return {
    caseId: requiredString(caseDto.id),
    caseTitle: requiredString(caseDto.title),
    primaryDocument: mapReportIntakeDocument(intake.primary_document),
    supplementDocuments: supplementDocuments.map((item) => mapReportIntakeDocument(asRecord(item).document)),
    supplementArtifacts: supplementArtifacts.map(mapReportIntakeArtifact),
    state,
    blockingReason: optionalString(intake.blocking_reason),
    nextAction: mapReportIntakeAction(intake.next_action),
  };
}

function mapCreatedReportResearch(dto: unknown): CreatedReportResearch {
  const created = asRecord(dto);
  const caseDto = asRecord(created.case);
  const document = asRecord(created.document);
  requiredBoolean(created.needs_text_or_pages);
  return {
    caseId: requiredString(caseDto.id),
    documentId: requiredString(document.id),
    documentTitle: requiredString(document.title),
    state: mapReportIntakeState(created.state),
    blockingReason: optionalString(created.blocking_reason),
    nextAction: mapReportIntakeAction(created.next_action),
    recoverySnapshot: created.recovery_snapshot === undefined || created.recovery_snapshot === null
      ? null
      : mapCreatedReportRecoverySnapshot(created.recovery_snapshot),
  };
}
...
export class HttpResearchAdapter implements ResearchClient {
  ...
  private mapCausalStep(dto: CausalStepDTO): {
    id: string;
    sequence: number;
    title: string;
    description: string;
  ...
  async getResearchPlanView(caseId?: string): Promise<ResearchPlanView> {
    ...
    const [gapsDto, documents, queue, runs] = await Promise.all([
      ...
      this.get<Schemas["DocumentListResponse"]>(`/documents`),
      this.get<ReviewQueueResponseDTO>(
  ...
  async getLibraryView(): Promise<LibraryView> {
    ...
    const firstSpanByDoc = new Map<string, Schemas["SourceSpanDTO"] | undefined>(
  ...
  async getReviewQueueView(caseId?: string): Promise<ReviewQueueView> {
    ...
    const dto = await this.get<ReviewQueueResponseDTO>(
      `/review-queue${query}`,
    );
  ...
  async listReviewProposals(caseId?: string): Promise<ProposalReviewItem[]> {
    ...
    const dto = await this.get<{ items: ProposalReviewItem[] }>(`/review-proposals${qs}`);
    return dto.items;
  }
  ...
  async getEventImpactTrace(caseId: string): Promise<EventImpactTrace> {
    ...
    const mapObservation = (observation: any) => ({ kind: observation.kind, status: observation.status, sourceStatementId: observation.source_statement_id ?? null, valuationSnapshotId: observation.valuation_snapshot_id ?? null, summary: observation.summary, asOfDate: observation.as_of_date ?? null });
  ...
  async createReportResearch(input: import("../domain/eventResearch").CreateReportResearchInput): Promise<CreatedReportResearch> {
    const dto = await this.post<unknown>("/report-research", {
    ...
    return mapCreatedReportResearch(dto);
  }

  async createReportResearchPdf(input: import("../domain/eventResearch").CreateReportResearchPdfInput): Promise<CreatedReportResearch> {
    ...
    query.set("source_contract_id", input.sourceContractId);
    let response: Response;
    try {
  ...
  async supplementReportResearch(input: import("../domain/eventResearch").SupplementReportResearchInput): Promise<CreatedReportResearch> {
    const dto = await this.post<unknown>(`/report-research/${encodeURIComponent(input.caseId)}/documents/${encodeURIComponent(input.documentId)}/supplement`, {
      content: input.content,
  ...
  async getReportResearchIntake(caseId: string): Promise<ReportResearchIntake> {
    const dto = await this.get<unknown>(`/report-research/${encodeURIComponent(caseId)}/intake`);
    return mapReportIntake(dto);
  }
  ...
  async updateEventResearchScope(input: { caseId: string; factors: Array<EventResearchScope["factors"][number] | string>; changedBy: string }): Promise<EventResearchScope & { reclassifiedEvidenceCount: number }> {
    ...
    return {
      version: dto.version,
      factors: dto.factors.map((factor) => typeof factor === "string" ? { statement: factor, description: null } : factor),

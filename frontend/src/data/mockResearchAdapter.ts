...
import {
  ...
  type WorkflowLedgerPage,
  type WorkflowLedgerStatus,
...
const MOCK_SERVER_ACTOR = "user:mock-researcher";
...
export class MockResearchAdapter implements ResearchClient {
  ...
  private initializedWorkflowCases = new Set<string>();
  ...
  setScenario(scenario: MockScenario): void {
    ...
    this.initializedWorkflowCases.clear();
  }
  ...
  async createDocumentSupplement(input: {
    ...
    const document: SourceDocumentView = {
      ...
      publisher: "user:mock-researcher",
      ...
      source_contract: contract ? { ...contract, provider_or_tenant: "user:mock-researcher" } : contract,
    };
      ...
      switch (payload.outcome) {
        ...
        case "rejected": {
          ...
          this.eventTsmReviewDecision = {
            ...
            reviewerId: "user:mock-researcher",
          };
          if (previous) {
  ...
  async extractEventResearch(input: { rawInput: string; sourceUrl?: string; sourceType?: EventSourceType; sourceMetadata?: Record<string, unknown> }): Promise<EventExtraction> {
    ...
    return simulateLatency({
      ...
      companyName: "示例主体", ticker: null, eventAt: null, marketReaction: "盘后下跌",
  ...
  async createEventResearch(input: CreateEventResearchInput): Promise<{ caseId: string; briefId: string; lifecycle: EventLifecycle }> {
    ...
    const document: SourceDocumentView = {
      ...
      publisher: typeof sourceMetadata.provider_name === "string" ? sourceMetadata.provider_name : MOCK_SERVER_ACTOR,
      ...
      source_contract: {
        ...
        provider_or_tenant: typeof sourceMetadata.provider_name === "string" ? sourceMetadata.provider_name : MOCK_SERVER_ACTOR,
  ...
  async attachEventMaterial(input: { caseId: string; rawInput: string; sourceUrl?: string; sourceType: EventSourceType; sourceMetadata: Record<string, unknown> }): Promise<{ documentVersionId: string }> {
    this.throwIfOffline();
    ...
    this.createdDocuments.set(documentVersionId, {
      document: {
        ...
        source_contract: {
          ...
          provider_or_tenant: MOCK_SERVER_ACTOR,
          permissions: { ai_processing: aiProcessing, display, export: typeof permissions.export === "boolean" ? permissions.export : false, api: typeof permissions.api === "boolean" ? permissions.api : false },
  ...
  async uploadEventMaterial(input: { caseId: string; file: File; sourceMetadata: Record<string, unknown> }): Promise<{ documentVersionId: string; parseState: "parsed" | "partial" | "failed"; nextAction: "review_original" | "supplement_original" }> {
    this.throwIfOffline();
    const caseItem = this.eventResearchItems().find((item) => item.id === input.caseId);
    if (!caseItem) throw new Error("event research case not found");
    if (caseItem.status === "published") throw new Error("published Case requires an explicit material decision");
  ...
  async getEventWorkflow(
    caseId: string,
    options?: { signal?: AbortSignal },
  ): Promise<EventWorkflow> {
    this.throwIfOffline();
    if (options?.signal?.aborted) throw new DOMException("Aborted", "AbortError");
    const created = /^event-created-(\d+)$/.exec(caseId);
    if (created && !this.initializedWorkflowCases.has(caseId)) {
      const scopeId = `scope-created-${created[1]}`;
      throw new EventWorkflowError(
        ...
        "research workflow is not initialized",
        {
          ...
          label: "确认命题并开始研究",
          method: "POST",
          href: `/api/v1/event-research/${caseId}/workflow/confirm`,
          payload: {
            scope_version_id: scopeId,
            scope_version: 1,
            idempotency_key: `workflow:initialize:${scopeId}:v1`,
          },
        },
      );
    }
    const workbench = await this.projectEventWorkbench(caseId);
    const state: EventWorkflow["state"] = workbench.lifecycle.status === "published"
      ? "monitoring"
      : workbench.lifecycle.status === "exhausted"
        ? "exhausted"
        : workbench.lifecycle.status === "researching" || workbench.lifecycle.status === "continuing"
          ? "acquiring"
          : workbench.lifecycle.status === "draft_ready"
            ? "generating_report"
            : "needs_scope_decision";
    const activeIndex = state === "monitoring" ? 5 : state === "generating_report" ? 5 : state === "acquiring" ? 2 : 1;
    const names = ["建立事件", "确认命题", "主动补证", "证据归并", "命题判定", "报告与监测"];
    const codes = ["event_intake", "scope_confirmation", "source_acquisition", "evidence_synthesis", "thesis_adjudication", "report_monitoring"] as const;
    const now = "2026-08-16T08:00:00Z";
    return simulateLatency({
      orchestrationId: `mock-orchestration-${caseId}`,
      researchRunId: `mock-run-${caseId}`,
      researchExecution: null,
      event: { caseId, title: workbench.event.eventTitle, researchQuestion: "当前事件的可验证研究问题" },
      scope: {
        id: `mock-scope-${caseId}`,
        version: workbench.scope.version,
        factors: workbench.scope.factors.map((factor, index) => ({
      ...
      stages: codes.map((code, index) => ({
        ...
        reason: index < activeIndex ? "该阶段已完成" : index === activeIndex ? workbench.lifecycle.summary : "等待前序阶段完成",
      })),
      systemAction: {
        label: workbench.lifecycle.summary,
        reason: workbench.lifecycle.currentGap,
        startedAt: now,
        heartbeatAt: now,
        leaseExpiresAt: null,
        retryAt: null,
      ...
      userAction: state === "needs_scope_decision" ? {
        kind: "scope_decision",
        label: "确认或调整研究范围",
        reason: workbench.lifecycle.currentGap ?? "当前范围需要研究员确认",
        recommendation: { kind: "revise_scope", label: "调整研究范围" },
        alternatives: [{ kind: "keep_scope", label: "保留当前范围并形成证据不足结论" }, { kind: "stop", label: "停止研究" }],
        impact: "决定会追加到当前事件的过程记录。",
        payload: {},
      } : null,
      userActionSummary: state === "needs_scope_decision" ? "确认或调整研究范围" : "当前无需操作",
      events: [{ id: `mock-event-${caseId}`, sequence: 1, transition: "mock_projection", actor: "system:mock", message: workbench.lifecycle.summary, payload: {}, createdAt: now }],
      acquisition: { seriesCount: 0, rounds: [] },
      sourceLedger: { counts: { total: 0, reviewed: 0, automaticallyAdmitted: 0, deduplicated: 0, quarantined: 0, byStatus: {} }, items: [], total: 0, hasMore: false },
      coverage: workbench.factors.map((factor) => ({
        goalId: factor.thesisId ?? `goal-${factor.position}`,
        thesisId: factor.thesisId ?? null,
        objective: factor.statement,
        status: factor.currentGap ? "continue" : "ready",
        reasonCodes: factor.currentGap ? ["mock_gap"] : [],
        requiredAuthorityCount: 1,
        observedAuthorityCount: factor.reviewedSupportCount,
        requiredIndependentSourceCount: 1,
        observedIndependentSourceCount: factor.reviewedSupportCount,
        contrarySearchCompleted: factor.reviewedContradictionCount > 0,
        evidenceLinkIds: [],
        unresolved: factor.currentGap ? [factor.currentGap] : [],
        unknown: [],
        evaluationRound: workbench.lifecycle.currentRound,
      })),
      conclusion: workbench.conclusion.state === "cannot_conclude" ? null : {
        id: `mock-conclusion-${caseId}`,
        state: workbench.conclusion.state,
        text: workbench.conclusion.text,
        primaryFactor: null,
        evidenceLinkIds: [],
        citations: [],
        reviewer: workbench.conclusion.state === "published" ? "human:researcher" : null,
        systemGenerated: workbench.conclusion.state !== "published",
        humanReviewed: workbench.conclusion.state === "published",
        reviewLabel: workbench.conclusion.state === "published" ? "已人工发布" : "系统生成，未经人工审核",
        createdAt: now,
      },
      monitor: state === "monitoring" ? { id: `mock-monitor-${caseId}`, version: 1, status: "active", frequency: "daily", factorIds: [], sourceTypes: [], nextVerificationEvent: "下一次公开披露", createdAt: now } : null,
      recovery: { status: "healthy", reason: null, source: "worker", attempt: 1, evaluatedAt: now, orchestrationHeartbeatAt: now, workerHeartbeatAt: now, leaseExpiresAt: null, retryAt: null, failedAt: null, lastCheckpoint: {}, decisionDiagnostic: null },
      version: 1,
      updatedAt: now,
    });
  }

  async getEventWorkflowLedger(
    _caseId: string,
    _options?: { status?: WorkflowLedgerStatus; cursor?: string; limit?: number; signal?: AbortSignal },
  ): Promise<WorkflowLedgerPage> {
    return simulateLatency({ items: [], total: 0, hasMore: false, nextCursor: null });
  }

  async confirmEventWorkflow(
    caseId: string,
    scopeVersionId: string,
    _options?: { scopeVersion?: number; idempotencyKey?: string },
  ): Promise<WorkflowCommandResult> {
    this.initializedWorkflowCases.add(caseId);
    return simulateLatency({
      orchestrationId: `mock-orchestration-${caseId}`,
      caseId,
      scopeVersionId,
  ...
  async decideEventScope(
    ...
    return simulateLatency({
      orchestrationId: `mock-orchestration-${caseId}`,
      caseId,
      scopeVersionId: `mock-scope-${caseId}`,
      researchRunId: `mock-run-${caseId}`,
      state: decision.kind === "stop" ? "cancelled" : "synthesizing_evidence",
      userStage: decision.kind === "stop" ? "cancelled" : "evidence_synthesis",
      currentSystemAction: decision.kind === "stop" ? "研究已明确停止" : "正在形成证据不足的诚实结论",
      systemActionReason: decision.reason,
      nextAction: null,
      version: 2,
    });
  }

  async resumeProtocolWorkflow(
    caseId: string,
    scopeVersionId: string,
    _options?: { scopeVersion?: number; idempotencyKey?: string },
  ): Promise<WorkflowCommandResult> {
    return simulateLatency({
      ...
      state: "planning_acquisition",
      userStage: "acquisition",
      currentSystemAction: "正在按已完成协议的新范围规划资料获取",
      systemActionReason: "研究协议已完成",
      nextAction: null,
      version: 3,
    });
  }
  ...
  private async projectEventWorkbench(caseId: string): Promise<EventWorkbench> {
    ...
    const conclusionCitations = publishedConclusion && caseId === "event-tsm"
      ...
      : evidence;
    const reviewedCount = tsmProjection?.reviewedCount ?? (["draft_ready", "published"].includes(event.status) ? 3 : 0);
    ...
    return simulateLatency({
      ...
      factors: activeFactors.map((factor, index) => { const pendingProposalCount = tsmProjection && index === 0 ? tsmProjection.pending : 0; const reviewedSupportCount = tsmProjection ? index === 0 ? tsmProjection.reviewedCount : 0 : reviewedCount ? 1 : 0; return { thesisId: `00000000-0000-4000-8000-${String(index + 1).padStart(12, "0")}`, statement: factor.statement, description: factor.description, position: index + 1, reviewedSupportCount, reviewedContradictionCount: 0, automaticallyAdmittedSupportCount: 0, automaticallyAdmittedContradictionCount: 0, pendingProposalCount, currentGap: tsmProjectionOwnsLifecycle && index === 0 && lifecycle.currentGap ? lifecycle.currentGap : pendingProposalCount ? "有关键证据待审核" : reviewedSupportCount ? null : "尚缺少可采纳证据" }; }),
      evidence,
      progress: { reviewedCount, automaticallyAdmittedCount: 0, pending: tsmProjection?.pending ?? 0, invalidSource: caseId === "event-tsm" ? 1 : 0, currentGap: lifecycle.currentGap },
  ...
  async getEventWorkbench(caseId: string): Promise<EventWorkbench> {
    return this.projectEventWorkbench(caseId);
  }
  ...
  async continueEventResearch(input: { caseId: string; documentVersionId: string; reason: string }): Promise<import("../domain/eventResearch").EventResearchContinuation> {
    this.throwIfOffline();
    ...
    void input.documentVersionId;
    return simulateLatency({ runId, lifecycle });
  }

  async decidePublishedMaterial(input: { caseId: string; rawInput: string; sourceUrl?: string; sourceType: EventSourceType; sourceMetadata: Record<string, unknown>; decision: "reopen" | "no_change"; reason: string }): Promise<import("../domain/eventResearch").PublishedMaterialDecision> {
    this.throwIfOffline();
    ...
    this.createdDocuments.set(documentVersionId, {
      document: {
        ...
        publisher: typeof input.sourceMetadata.provider_name === "string" ? input.sourceMetadata.provider_name : MOCK_SERVER_ACTOR,
        document_type: input.sourceType,
        publish_date: null,
        available_at: "2026-08-09T12:00:00Z",
        acquired_at: "2026-08-09T12:00:00Z",
        ...
        source_contract: {
          source_type: input.sourceType,
          provider_or_tenant: typeof input.sourceMetadata.provider_name === "string" ? input.sourceMetadata.provider_name : MOCK_SERVER_ACTOR,
          permissions: {
    ...
    if (input.decision === "reopen") {
      const next = await this.continueEventResearch({ caseId: input.caseId, documentVersionId, reason: input.reason });
  ...
  async decidePublishedUploadedMaterial(input: { caseId: string; file: File; sourceMetadata: Record<string, unknown>; decision: "reopen" | "no_change"; reason: string }): Promise<import("../domain/eventResearch").PublishedMaterialDecision> {
    this.throwIfOffline();
    const isPdf = input.file.type === "application/pdf";
  ...
  async updateEventResearchScope(input: { caseId: string; factors: EventResearchScopeFactorInput[]; changeReason: string }): Promise<{ version: number; factors: EventResearchScopeFactor[]; reclassifiedEvidenceCount: number; unmappedEvidenceCount: number }> {
    this.throwIfOffline();
  ...
  async publishEventConclusion(input: { caseId: string; text: string }): Promise<{ conclusionId: string; state: "published" }> {
    this.throwIfOffline();
    if (input.caseId === "event-tsm") {
      ...
      const published: EventConclusionVersion = {
        ...
        reviewer: "user:mock-authenticated-researcher",
    ...
    try {
      ...
      const draft: EventConclusionVersion = {
        ...
        basedOnConclusionId: null,
        reviewer: null,
        evidenceCount: workbench.progress.reviewedCount + workbench.progress.automaticallyAdmittedCount,

...
export class MockResearchAdapter implements ResearchClient {
  ...
  private reportIntakes = new Map<string, import("../domain/eventResearch").ReportResearchIntake>();
  ...
  async createReportResearch(input: import("../domain/eventResearch").CreateReportResearchInput): Promise<import("../domain/eventResearch").CreatedReportResearch> {
    ...
    const nextAction = { kind: "extract_candidates" as const, label: "开始抽取待审核陈述", unlockMessage: "完成后可审核关键陈述，尚不会生成正式研究结论。" };
    this.reportIntakes.set(caseId, { caseId, caseTitle: input.title, primaryDocument: { id: documentId, title: input.title, inputKind: input.inputKind }, supplementDocuments: [], supplementArtifacts: [], state: "pending_candidate_extraction", blockingReason: null, nextAction });
    return simulateLatency({ caseId, documentId, documentTitle: input.title, state: "pending_candidate_extraction", blockingReason: null, nextAction, recoverySnapshot: null });
  }
  ...
  async createReportResearchPdf(input: import("../domain/eventResearch").CreateReportResearchPdfInput): Promise<import("../domain/eventResearch").CreatedReportResearch> {
    ...
    const nextAction = { kind: "extract_candidates" as const, label: "开始抽取待审核陈述", unlockMessage: "完成后可审核关键陈述，尚不会生成正式研究结论。" };
    this.reportIntakes.set("report-pdf-mock", { caseId: "report-pdf-mock", caseTitle: input.title, primaryDocument: { id: "document-pdf-mock", title: input.title, inputKind: "pdf_upload" }, supplementDocuments: [], supplementArtifacts: [], state: "pending_candidate_extraction", blockingReason: null, nextAction });
    return simulateLatency({ caseId: "report-pdf-mock", documentId: "document-pdf-mock", documentTitle: input.title, state: "pending_candidate_extraction", blockingReason: null, nextAction, recoverySnapshot: null });
  }
  ...
  async supplementReportResearch(input: import("../domain/eventResearch").SupplementReportResearchInput): Promise<import("../domain/eventResearch").CreatedReportResearch> {
    ...
    const previous = this.reportIntakes.get(input.caseId);
    const nextAction = { kind: "extract_candidates" as const, label: "开始抽取待审核陈述", unlockMessage: "完成后可审核关键陈述，尚不会生成正式研究结论。" };
    const caseTitle = previous?.caseTitle ?? "恢复资料";
    this.reportIntakes.set(input.caseId, { caseId: input.caseId, caseTitle, primaryDocument: previous?.primaryDocument ?? { id: input.documentId, title: caseTitle, inputKind: "pdf_upload" }, supplementDocuments: [{ id: `supplement-${input.documentId}`, title: "补充内容快照", inputKind: "recovery_text" }], supplementArtifacts: [], state: "pending_candidate_extraction", blockingReason: null, nextAction });
    return simulateLatency({ caseId: input.caseId, documentId: input.documentId, documentTitle: "补充内容快照", state: "pending_candidate_extraction", blockingReason: null, nextAction, recoverySnapshot: null });
  }

  async getReportResearchIntake(caseId: string): Promise<import("../domain/eventResearch").ReportResearchIntake> {
    this.throwIfOffline();
    const intake = this.reportIntakes.get(caseId) ?? {
      caseId,
      caseTitle: "待审核研报",
      primaryDocument: { id: "report-document-mock", title: "冻结资料", inputKind: "pdf_upload" },
      supplementDocuments: [],
      supplementArtifacts: [],
      state: "pending_candidate_extraction" as const,
      blockingReason: null,
      nextAction: { kind: "extract_candidates" as const, label: "开始抽取待审核陈述", unlockMessage: "完成后可审核关键陈述，尚不会生成正式研究结论。" },
    };
    return simulateLatency(intake);
  }
  ...
  async reviewEventImpactRelation(input: { relationId: string; outcome: "accepted" | "rejected" | "needs_more"; reason: string; reviewer: string }): Promise<{ reviewId: string; relationId: string; outcome: string }> {
    return simulateLatency({ reviewId: `review-${input.relationId}`, relationId: input.relationId, outcome: input.outcome });
  }

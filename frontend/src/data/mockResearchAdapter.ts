...
export class MockResearchAdapter implements ResearchClient {
  ...
  async getEventImpactTrace(_caseId: string): Promise<EventImpactTrace> {
    return simulateLatency({ scopeVersion: 1, asOf: "2026-08-08", progress: { hypotheses: 2, relations: 2 }, alternatives: [{ hypothesisId: "impact-alt", statement: "市场替代解释", rank: 2, classification: "alternative", scoreComponents: { market: 1 }, explanation: "市场证据尚不足", relations: [], funds: [] }], factors: [{ hypothesisId: "impact-1", statement: "资本开支传导", rank: 1, classification: "candidate", scoreComponents: { event: 1 }, explanation: "等待传导审核", relations: [{ relationId: "impact-listed", companyId: "company-listed", companyName: "示例 A 股公司", companyType: "listed", relationKind: "supplier", direction: "benefits", mechanism: "订单传导", status: "verified", effectiveStatus: "verified", sourceStatementId: "statement-listed", review: { outcome: "accepted", reason: "公告披露", reviewer: "researcher", createdAt: "2026-08-08T00:00:00Z" }, stocks: [{ stockId: "stock-1", code: "600001.SH", name: "示例 A 股公司", market: "SSE" }], observations: [], fundExposure: [] }, { relationId: "impact-unlisted", companyId: "company-unlisted", companyName: "未上市供应商", companyType: "unlisted_supplier", relationKind: "supplier", direction: "benefits", mechanism: "产能传导", status: "candidate", effectiveStatus: "candidate", sourceStatementId: null, review: null, stocks: [], observations: [], fundExposure: [] }], funds: [{ fundId: "fund-complete", coverageStatus: "complete", computable: true, exposure: "0.12", reportPeriod: "2026-06-30", publishedAt: "2026-08-01T00:00:00Z", source: "https://example.invalid/fund-complete" }, { coverageStatus: "partial", computable: false, exposure: null, reportPeriod: "2026-06-30", publishedAt: "2026-08-01T00:00:00Z", source: "https://example.invalid/fund-disclosure" }] }] });
  }

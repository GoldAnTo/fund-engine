  ...
  return {
    ...
    narrative: completed
      ? {
          ...
          completedCount: 3,
          totalCount: 3,
          nextAction: "可查看关键因素、证据与局限",
          elapsedSeconds: 65,
          estimatedRemainingSecondsMin: null,
          estimatedRemainingSecondsMax: null,
        }
      : {
          currentAction: failed ? "自动研究未能完成" : "正在查找可信来源",
          completedCount: 0,
          totalCount: 3,
          nextAction: failed ? "可检查已处理内容后重新开始" : "随后获取已找到的来源",
          elapsedSeconds: 0,
          estimatedRemainingSecondsMin: null,
          estimatedRemainingSecondsMax: null,
        },
    activities: completed
      ...
      : failed ? [{
          ...
          technicalDetails: [{
            ...
            workItem: "核验供应链影响",
            internalStatus: "searching",
          }],
        }],
    exceptions: completed
      ? [{ reason: "来源许可不满足", count: 1, impact: "该来源未纳入结论。", systemAction: "系统已跳过受限来源。" }]
      : failed ? [{ reason: "必要来源暂时不可读取", count: 1, impact: "该来源未纳入结论。", systemAction: "系统已停止本轮并保留已处理内容。" }] : [],
    factors: [{
      statement: "供应链需求变化",
      classification: completed ? "key" : "pending",
      rankingReason: completed
        ? "该因素直接影响供应链需求，且现有证据充分并无实质反证。"
        : "尚未确认关键因素：等待资料与证据校验完成。",
      supportCount: completed ? 2 : 0,
      counterEvidenceCount: completed ? 1 : 0,
      evidenceGap: completed ? null : "等待更多可用资料。",
    }],
...
export class MockResearchAdapter implements ResearchClient {
  ...
  async startAutomaticResearchFromUpload(
    ...
    const title = input.input?.trim() || input.file.name;
    return this.startAutomaticResearch(title);
  }

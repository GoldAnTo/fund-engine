...
export class MockResearchOsApi implements ResearchOsApi {
  ...
  private marketBindings = new Map<string, Schemas["MarketInstrumentBindingDTO"][]>();
  private fundamentalImpacts = new Map<string, Schemas["FundamentalImpactDTO"][]>();
  private marketObservations = new Map<string, Schemas["MarketObservationDTO"][]>();
  ...
  async startFactorMonitorRun(caseId: string, keyFactorId: string): ReturnType<ResearchOsApi["startFactorMonitorRun"]> {
    const monitor = this.monitors.get(caseId) ?? monitorFor(caseId);
    const factorId = keyFactorId === "factor-capex" ? "factor-capex" : keyFactorId;
    const scopedMonitor = { ...monitor, factor_ids: [factorId], allowed_source_types: monitor.allowed_source_types.filter((source) => source === "company_disclosure") };
    const id = `run-${caseId}-factor-${keyFactorId}`;
    this.manualRuns.set(id, { caseId, monitor: scopedMonitor });
    return { id, case_id: caseId, status: "queued", stage: "queued", round: 0, max_rounds: 3, budget: scopedMonitor.budget, budget_used: 0, stop_reason: null, scope_thesis_ids: [factorId], progress: {}, evidence: {}, by_thesis: {}, gaps: [], gap_tasks: [], failed_tasks: [], assessments: [], pending_proposals: [], review_tasks: [], next_action: "查看运行详情", tasks: [] };
  }
  ...
  async marketExpression(caseId: string): ReturnType<ResearchOsApi["marketExpression"]> {
    const baseClaims: Schemas["ReportClaimDTO"][] = [{ id: "report-claim-demo", text: "公司预计资本开支将高于此前指引。", claim_kind: "forecast", asserted_period: "2026H2", asserted_by: "研究报告", reviewed_by: "human:reviewer", review_reason: "原文定位及许可已核对", reviewed_at: now, source }];
    const baseFactors: Schemas["KeyFactorDTO"][] = [{ id: "factor-capex", thesis_id: "factor-capex", report_claim_id: "report-claim-demo", name: factors[0].statement, expected_direction: "positive", metric_name: "资本开支同比增速", allowed_source_types: ["company_disclosure"], verification_window_start: "2026-07-01", verification_window_end: "2026-09-30", support_condition: "一手披露实际增长并明确投向", refutation_condition: "下调或投向不符", next_verification_event: "2026Q3 季报", reviewed_by: "human:reviewer", review_reason: "已定义可验证指标", reviewed_at: now, verification: { outcome: "not_due", rationale: "尚未到下一财报验证时点", reviewed_by: "human:reviewer", reviewed_at: now, source } }];
    const allFactors = [...baseFactors, ...(this.keyFactors.get(caseId) ?? [])].map((factor) => ({ ...factor, verification: this.verifications.get(`${caseId}:${factor.id}`) ?? factor.verification }));
    return { case_id: caseId, as_of: "2026-08-09", cutoff: now, claims: [...baseClaims, ...(this.reportClaims.get(caseId) ?? [])], factors: allFactors, fundamentals: [{ id: "impact-demo", key_factor_id: "factor-capex", company_id: "company-tsm", company_name: "台积电", stock_id: "stock-tsm", stock_code: "TSM", stock_name: "台积电", metric_name: "资本开支", expected_direction: "positive", rationale: "仅记录待验证的基本面传导，不构成市场因果。", reviewed_by: "human:reviewer", review_reason: "研究员确认传导口径", reviewed_at: now, source }, ...(this.fundamentalImpacts.get(caseId) ?? [])], market_observations: [{ id: "market-demo", key_factor_id: "factor-capex", stock_id: "stock-tsm", stock_code: "TSM", stock_name: "台积电", event_at: "2026-08-08T20:00:00Z", available_at: "2026-08-09T00:00:00Z", window_label: "公告后 1D", benchmark: "SOX", price_source: "授权行情快照", after_hours_treatment: "事件发生在盘后，窗口从下一交易日开盘开始", relative_return: -0.012, reviewed_by: "human:reviewer", review_reason: "仅核验数据窗口", reviewed_at: now }, ...(this.marketObservations.get(caseId) ?? [])], fund_exposure: [{ fund_id: "fund-demo", fund_code: "000001", fund_name: "演示成长基金", disclosed_exposure: 0.038, positions: [{ stock_id: "stock-tsm", stock_code: "TSM", stock_name: "台积电", weight: 0.038, report_period: "2026-06-30", published_at: "2026-07-20T00:00:00Z", acquired_at: "2026-07-21T00:00:00Z", source: "披露持仓快照", source_document_version_id: "doc-fund-holdings-2026q2", source_locator: { table: "前十大持仓", row: 3 }, provider_record_id: "provider-fund-2026q2", source_permission_status: "admitted", coverage_status: "complete", freshness_status: "historical_disclosure" }] }] };
  }

  async sourceStatements(_caseId: string): ReturnType<ResearchOsApi["sourceStatements"]> { return { items: [{ id: source.source_statement_id!, kind: "disclosed_fact", text: "公司上调全年资本开支指引。", document_version_id: source.document_version_id!, document_title: source.document_title!, source_url: source.source_url, locator: source.locator!, available_at: source.available_at!, permission_status: "admitted" }] }; }
  async marketInstruments(caseId: string): ReturnType<ResearchOsApi["marketInstruments"]> { return { items: [{ id: "binding-demo", company_id: "company-tsm", company_code: "TSM", company_name: "台积电", stock_id: "stock-tsm", stock_code: "TSM", stock_name: "台积电", relationship_role: "directly_affected", reviewed_by: "human:reviewer", review_reason: "研究员已核对该公司与本 Case 的适用范围。", reviewed_at: now, source }, ...(this.marketBindings.get(caseId) ?? [])] }; }
  async marketInstrumentCatalog(_query: string): ReturnType<ResearchOsApi["marketInstrumentCatalog"]> { return { items: [{ company_id: "company-tsm", company_code: "TSM", company_name: "台积电", company_type: "listed", stocks: [{ id: "stock-tsm", code: "TSM", name: "台积电", market: "NYSE" }] }] }; }
  async createMarketInstrumentBinding(caseId: string, input: Parameters<ResearchOsApi["createMarketInstrumentBinding"]>[1]): ReturnType<ResearchOsApi["createMarketInstrumentBinding"]> { const catalog = await this.marketInstrumentCatalog(""); const company = catalog.items.find((item) => item.company_id === input.company_id); const stock = company?.stocks.find((item) => item.id === input.stock_id) ?? null; const created: Schemas["MarketInstrumentBindingDTO"] = { id: `binding-${Date.now()}`, company_id: input.company_id, company_code: company?.company_code ?? "已选公司", company_name: company?.company_name ?? "已选公司", stock_id: input.stock_id ?? null, stock_code: stock?.code ?? null, stock_name: stock?.name ?? null, relationship_role: input.relationship_role, reviewed_by: input.reviewed_by, review_reason: input.review_reason, reviewed_at: now, source }; this.marketBindings.set(caseId, [...(this.marketBindings.get(caseId) ?? []), created]); return created; }
  async createFundamentalImpact(caseId: string, factorId: string, input: Parameters<ResearchOsApi["createFundamentalImpact"]>[2]): ReturnType<ResearchOsApi["createFundamentalImpact"]> { const binding = (await this.marketInstruments(caseId)).items.find((item) => item.id === input.market_instrument_binding_id); if (!binding) throw new Error("market instrument binding is not reviewed"); const created: Schemas["FundamentalImpactDTO"] = { id: `impact-${Date.now()}`, key_factor_id: factorId, company_id: binding.company_id, company_name: binding.company_name, stock_id: binding.stock_id, stock_code: binding.stock_code, stock_name: binding.stock_name, metric_name: input.metric_name, expected_direction: input.expected_direction, rationale: input.rationale, reviewed_by: input.reviewed_by, review_reason: input.review_reason, reviewed_at: now, source }; this.fundamentalImpacts.set(caseId, [...(this.fundamentalImpacts.get(caseId) ?? []), created]); return created; }
  async createMarketObservation(caseId: string, factorId: string, input: Parameters<ResearchOsApi["createMarketObservation"]>[2]): ReturnType<ResearchOsApi["createMarketObservation"]> { const binding = (await this.marketInstruments(caseId)).items.find((item) => item.id === input.market_instrument_binding_id); if (!binding?.stock_id || !binding.stock_code || !binding.stock_name) throw new Error("market observation requires a reviewed stock binding"); const created: Schemas["MarketObservationDTO"] = { id: `observation-${Date.now()}`, key_factor_id: factorId, stock_id: binding.stock_id, stock_code: binding.stock_code, stock_name: binding.stock_name, event_at: input.event_at, available_at: input.available_at, window_label: input.window_label, benchmark: input.benchmark, price_source: input.price_source, after_hours_treatment: input.after_hours_treatment, relative_return: input.relative_return ?? null, reviewed_by: input.reviewed_by, review_reason: input.review_reason, reviewed_at: now }; this.marketObservations.set(caseId, [...(this.marketObservations.get(caseId) ?? []), created]); return created; }
  async createReportClaim(caseId: string, input: Parameters<ResearchOsApi["createReportClaim"]>[1]): ReturnType<ResearchOsApi["createReportClaim"]> { const statement = (await this.sourceStatements(caseId)).items.find((item) => item.id === input.source_statement_id); if (!statement) throw new Error("source statement is not admitted"); const created: Schemas["ReportClaimDTO"] = { id: `report-claim-${Date.now()}`, text: input.text, claim_kind: input.claim_kind, asserted_period: input.asserted_period ?? null, asserted_by: input.asserted_by, reviewed_by: input.reviewed_by, review_reason: input.review_reason, reviewed_at: now, source: { source_statement_id: statement.id, document_version_id: statement.document_version_id, document_title: statement.document_title, source_url: statement.source_url, locator: statement.locator, available_at: statement.available_at, permission_status: statement.permission_status } }; this.reportClaims.set(caseId, [...(this.reportClaims.get(caseId) ?? []), created]); return created; }
  async createKeyFactor(caseId: string, input: Parameters<ResearchOsApi["createKeyFactor"]>[1]): ReturnType<ResearchOsApi["createKeyFactor"]> { const created: Schemas["KeyFactorDTO"] = { id: `key-factor-${Date.now()}`, thesis_id: input.thesis_id ?? null, report_claim_id: input.report_claim_id, name: input.name, expected_direction: input.expected_direction, metric_name: input.metric_name, allowed_source_types: input.allowed_source_types, verification_window_start: input.verification_window_start ?? null, verification_window_end: input.verification_window_end ?? null, support_condition: input.support_condition, refutation_condition: input.refutation_condition, next_verification_event: input.next_verification_event, reviewed_by: input.reviewed_by, review_reason: input.review_reason, reviewed_at: now, verification: null }; this.keyFactors.set(caseId, [...(this.keyFactors.get(caseId) ?? []), created]); return created; }
  async createClaimVerification(caseId: string, factorId: string, input: Parameters<ResearchOsApi["createClaimVerification"]>[2]): ReturnType<ResearchOsApi["createClaimVerification"]> { const statement = (await this.sourceStatements(caseId)).items.find((item) => item.id === input.source_statement_id); if (!statement) throw new Error("source statement is not admitted"); const created: Schemas["ClaimVerificationDTO"] = { outcome: input.outcome, rationale: input.rationale, reviewed_by: input.reviewed_by, reviewed_at: now, source: { source_statement_id: statement.id, document_version_id: statement.document_version_id, document_title: statement.document_title, source_url: statement.source_url, locator: statement.locator, available_at: statement.available_at, permission_status: statement.permission_status } }; this.verifications.set(`${caseId}:${factorId}`, created); return created; }

...
export function EventCreatePage() {
  ...
  async function extract() {
    ...
    try {
      const next = await researchClient.extractEventResearch({ rawInput: rawInput.trim(), sourceUrl: sourceUrl.trim() || undefined, sourceType, sourceMetadata });
      setDraft(next);
  ...
  async function loadTextFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setRawInput(text);
      setSourceMetadata({ file_name: file.name, mime_type: file.type || "text/plain", byte_size: file.size });
      setError(null);
    } catch {
    ...
    <main className="ros-page ros-create-page">
      ...
      <div className="ros-create-grid">
        <section className="ros-form-card"><div className="ros-step"><span>01</span><div><h2>提供事件材料</h2><p>可以粘贴新闻、公告、研报片段；不要把二手转述当作已确认事实。</p></div></div>
          ...
          <small className="ros-form-note">当前选择：{sourceType === "licensed_provider" ? "授权数据源快照（合同权限仍需后端配置）" : sourceType === "uploaded_file" ? "上传文本快照（原件未保存）" : "粘贴快照（需后续核验）"}。来源类型与本次输入元数据会随 Case 冻结。</small>
        </section>

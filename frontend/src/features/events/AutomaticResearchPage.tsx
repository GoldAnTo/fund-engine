...
export function AutomaticResearchPage() {
  ...
  const estimatedRemaining = view.narrative.estimatedRemainingSecondsMin !== null
    ? view.narrative.estimatedRemainingSecondsMax === null
      ? `${view.narrative.estimatedRemainingSecondsMin} 秒`
      : `${view.narrative.estimatedRemainingSecondsMin}–${view.narrative.estimatedRemainingSecondsMax} 秒`
    : null;
    ...
    >
      ...
      {liveStage && (
        <p
          ...
          aria-atomic="true"
        >
          当前动作：{view.narrative.currentAction}（{liveStage.label}，{automaticStageStatusLabel(liveStage.status)}）
          。{liveStage.summary}
        </p>
      )}

      <p className="automatic-research-process__narrative-summary">
        已完成 <strong>{view.narrative.completedCount}/{view.narrative.totalCount}</strong>，下一步：
        <strong>{view.narrative.nextAction}</strong>，
        已耗时：{formatAutomaticDuration(view.narrative.elapsedSeconds)}
        {estimatedRemaining ? `，预计还需 ${estimatedRemaining}` : ""}。
      </p>

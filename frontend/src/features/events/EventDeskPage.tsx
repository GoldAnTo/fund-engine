...
import { eventDeskRoute } from "../../domain/eventResearchPresentation";
...
export function EventDeskPage() {
  ...
  const visibleEvents = useMemo(() => attentionOnly ? events.filter((event) => event.nextHumanAction) : events, [attentionOnly, events]);
  const ordered = useMemo(() => [...visibleEvents].sort((a, b) => Number(Boolean(b.nextHumanAction)) - Number(Boolean(a.nextHumanAction)) || b.updatedAt.localeCompare(a.updatedAt)), [visibleEvents]); const highest = ordered[0]; const active = ordered.filter((event) => ["extracting", "researching", "continuing"].includes(event.status)); const review = ordered.filter((event) => event.nextHumanAction);
  ...
  return <main className="ros-page ros-desk"><header className="ros-page-head"><div><p className="ros-eyebrow">{section === "network" ? "研究资产 · Research Network" : section === "monitoring" ? "研究资产 · Monitoring" : "事件研究 · Research Desk"}</p><h1>{title}</h1><p>{subtitle}</p></div><Link className="ros-button ros-button--primary" to="/events/new">＋ 从事件开始</Link></header>{error && <section className="ros-empty ros-empty--compact" role="alert"><strong>研究调度暂不可读取</strong><p>{error}</p><button className="ros-button ros-button--secondary" type="button" onClick={loadEvents}>重试读取事件研究</button></section>}{!loading && !error && ordered.length === 0 && <section className="ros-empty ros-empty--large"><h2>{attentionOnly ? "没有待人工审核的 Case" : "还没有进行中的研究"}</h2><p>从公告、研报或新闻快照建立事件 Case；系统会先固定研究范围和可验证因素。</p><Link className="ros-button ros-button--primary" to="/events/new">从事件开始</Link></section>}{!!highest && <div className="ros-desk-layout"><section className="ros-card ros-case-queue" aria-label="研究优先队列"><header className="ros-card-head"><div><p className="ros-eyebrow">优先队列</p><h2>等待推进的判断</h2></div><span>{review.length} 项需要人工决定</span></header><Link className="ros-priority" to={eventDeskRoute(highest)}><span className="ros-eyebrow">当前优先</span><h2>{highest.nextHumanAction || highest.statusSummary}</h2><p>{highest.eventTitle} · 完成后决定当前研究是否需要调整范围或复核结论。</p><strong>{highest.nextHumanAction ? "进入审核 →" : "查看 Case →"}</strong></Link><div className="ros-queue-rows">{ordered.slice(1, 6).map((event, index) => <Link className="ros-queue-row" key={event.id} to={eventDeskRoute(event)}><span className={`ros-priority-number${event.nextHumanAction ? " is-review" : ""}`}>{index + 2}</span><div><strong>{event.eventTitle}</strong><p>{event.statusSummary}</p></div><span className={event.nextHumanAction ? "ros-need is-review" : "ros-need"}>{event.nextHumanAction || (workerUnavailable ? "等待执行器启动" : workerUnknown ? "执行器状态待确认" : "系统补证中")}</span></Link>)}</div></section><aside className="ros-desk-rail"><section><p className="ros-eyebrow">今天的工作</p><h2>先审核，再扩展</h2><p>系统不会自行采纳候选关系，也不会用同期行情自动写成因果结论。</p></section><section><p className="ros-eyebrow">研究网络</p><div className="ros-mini-row"><span>进行中 Case</span><b>{active.length || "—"}</b></div><div className="ros-mini-row"><span>待你审核</span><b>{review.length || "—"}</b></div><div className="ros-mini-row"><span>已发布结论</span><b>{ordered.filter((event) => event.status === "published").length || "—"}</b></div></section><section><p className="ros-eyebrow">系统状态</p><h2>{active.length && workerUnavailable ? "执行器未启动；已排队研究不会自动推进" : active.length && workerUnknown ? "执行器状态暂不可确认" : active.length ? "正在受控运行" : "等待下一项触发"}</h2><p>{active.length && workerUnavailable ? "范围和排队记录已保存；执行器恢复前不会领取任务，系统不会把它显示为正在执行。" : active.length && workerUnknown ? "存在进行中 Case，但执行器心跳或状态暂不可确认；请以运行档案和阶段事件为准。" : active.length ? `${active.length} 个 Case 正在按各自范围、来源许可和配置版本补证。` : "没有后台运行；所有新证据和任务状态都会在这里留下可查看的记录。"}</p></section></aside></div>}<section className="ros-all-events"><header><div><p className="ros-eyebrow">{attentionOnly ? "待审核事件" : "全部事件"}</p><h2>{loading ? "正在读取…" : `${ordered.length} 个研究 Case`}</h2></div><span>按下一人工动作与更新时间排序</span></header>{loading ? <ResearchDispatchSkeleton /> : ordered.map((event) => <Link to={eventDeskRoute(event)} key={event.id} className="ros-event-row" aria-label={event.eventTitle}><div><strong>{event.eventTitle}</strong><span>{[event.companyName, event.ticker].filter(Boolean).join(" · ") || "待确认标的"}</span></div><div><i className={event.nextHumanAction ? "ros-dot is-review" : "ros-dot"} /><span>{STATUS_LABEL[event.status]} · {event.statusSummary}</span></div><div><b>{event.nextHumanAction ? "你需要做" : workerUnavailable ? "等待执行器" : workerUnknown ? "状态待确认" : "系统正在做"}</b><span>{event.nextHumanAction || (workerUnavailable ? "执行器未启动；已排队工作不会自动推进" : workerUnknown ? "执行器状态暂不可确认" : event.statusSummary)}</span></div><time dateTime={event.updatedAt}>{timeLabel(event.updatedAt)}</time></Link>)}</section></main>;
}

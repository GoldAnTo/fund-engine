...
export function EventDeskPage() {
  const location = useLocation(); const [searchParams] = useSearchParams(); const [events, setEvents] = useState<EventResearchListItem[]>([]); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(true);
  const loadEvents = () => {
    setError(null);
    setLoading(true);

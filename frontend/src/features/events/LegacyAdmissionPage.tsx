...
export function LegacyAdmissionPage() {
  ...
  return <main className="ros-page ros-admission-page">
    ...
    {session && <p className="ros-admission-identity">当前管理员会话：<b>{session.display_name}</b> · 团队 <b>{session.tenant_id}</b>。提交会追加不可变准入记录，之后该 Case 才进入目标团队的工作台与运行档案。</p>}
    ...
    {items && items.length > 0 && <section className="ros-admission-list" aria-label="历史 Case 准入队列">{items.map((item) => <AdmissionRow key={item.case_id} item={item} tenantLabel={session?.tenant_id ?? ""} actorLabel={session?.display_name ?? ""} onAdmitted={(title) => { setMessage(`已准入「${title}」，它现在仅在当前团队的研究工作台中可见。`); void load(); }} />)}</section>}
  </main>;
}

function AdmissionRow({ item, tenantLabel, actorLabel, onAdmitted }: { item: LegacyCaseAdmissionCandidate; tenantLabel: string; actorLabel: string; onAdmitted: (title: string) => void }) {
  ...
  const missingRequirements = [
    ...
    !tenantLabel ? "确认当前团队身份" : null,
    !actorLabel ? "确认当前用户身份" : null,
  ...
  async function submit() {
    ...
    try {
      await researchOsApi.admitLegacyCase(item.case_id, { initial_document_version_id: documentId, reason: reason.trim() });
  ...
  return <article className="ros-admission-row">
    ...
    <div className="ros-admission-layout"><section><h3>选择初始冻结资料</h3><p>只能选择该 Case 在准入前已关联的资料。选择不会改写原文或重新解析内容。</p><div className="ros-admission-documents">{item.documents.map((document) => <label key={document.document_version_id} className={document.document_version_id === documentId ? "is-selected" : ""}><input type="radio" name={`source-${item.case_id}`} checked={document.document_version_id === documentId} onChange={() => setDocumentId(document.document_version_id)} /><span><strong>{document.title || "未命名冻结资料"}</strong><small>{document.source_url} · 可得于 {new Date(document.available_at).toLocaleString("zh-CN")}</small><code>{document.document_version_id}</code></span></label>)}</div></section>
      <section className="ros-admission-form"><h3>记录准入决定</h3><p className="ros-admission-identity">归属团队与操作人由当前登录会话确定：<b>{actorLabel || "用户未加载"}</b> · <b>{tenantLabel || "团队未加载"}</b></p><label>准入依据<textarea aria-label={`${item.event_title} 的准入依据`} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="说明如何核验历史迁移记录、原始资料和团队归属" /></label>{selected && <p className="ros-admission-proof">将写入：<b>{selected.title || selected.document_version_id}</b>、当前团队、当前登录用户与准入依据。</p>}{missingRequirements.length > 0 && <p className="ros-note" role="status">准入前还需：{missingRequirements.join("、")}。系统不会从创建人、来源地址或内容哈希推断团队归属。</p>}{error && <p className="ros-error" role="alert">{error}</p>}<button className="ros-button ros-button--primary" type="button" disabled={!readyToSubmit} onClick={() => void submit()}>{submitting ? "正在追加准入记录…" : "确认并准入此 Case"}</button></section></div>
  </article>;
}

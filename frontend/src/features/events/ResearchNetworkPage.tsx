...
export function ResearchNetworkPage() {
  ...
  return <main className="ros-page ros-network"><header className="ros-page-head"><div><p className="ros-eyebrow">研究资产 · Research Network</p><h1>跨 Case 研究网络</h1><p>只用于发现可继续下钻的相关研究。关联不继承证据、结论或审核状态。</p></div><Link className="ros-button ros-button--primary" to="/events/new">＋ 从事件开始</Link></header>{error && <section className="ros-empty ros-empty--compact" role="alert"><strong>跨 Case 关联暂不可读取</strong><p>{error}</p><button className="ros-button ros-button--secondary" type="button" onClick={loadNetwork}>重试读取跨 Case 关联</button></section>}{!network ? !error && <NetworkSkeleton /> : <><section className="ros-network-note"><strong>关联边界</strong><span>共享资料进入目标 Case 后，仍须重新确认来源权限、研究范围、证据角色与人工审核。</span></section><RelationLane title="已审核关联" relations={network.reviewed_relations} reviewed /><RelationLane title="AI 候选" relations={network.candidate_relations} /><RelationLane title="已处理候选" relations={network.resolved_candidates ?? []} resolved /></>}</main>;
}

...
export function CaseRelationsContent({ caseId }: { caseId: string }) {
  ...
  if (error && !relations) return <section className="ros-empty ros-page-gap" role="alert"><strong>关联研究暂不可读取</strong><p>{error}</p><button className="ros-button ros-button--secondary" type="button" onClick={() => void load()}>重试读取关联记录</button></section>;

...
function MarketObservationRegistration({ caseId, factor, bindings, onSaved }: { caseId: string; factor: MarketExpression["factors"][number] | null; bindings: MarketInstrumentBindings["items"]; onSaved: (notice: string) => void }) {
  ...
  const [open, setOpen] = useState(false); const [bindingId, setBindingId] = useState(""); const [eventAt, setEventAt] = useState("2026-08-08T20:00"); const [availableAt, setAvailableAt] = useState("2026-08-09T00:00"); const [windowLabel, setWindowLabel] = useState("公告后 1D"); const [benchmark, setBenchmark] = useState("中证全指"); const [priceSource, setPriceSource] = useState("授权行情快照"); const [relativeReturn, setRelativeReturn] = useState(""); const [reviewer, setReviewer] = useState("human:researcher"); const [reason, setReason] = useState(""); const [busy, setBusy] = useState(false); const [message, setMessage] = useState<string | null>(null);
  function begin() { if (!factor) return; setOpen(true); setMessage(null); setBindingId(stockBindings[0]?.id ?? ""); }
  async function save() { if (!factor || !bindingId || !eventAt || !availableAt || !windowLabel.trim() || !benchmark.trim() || !priceSource.trim() || !reviewer.trim() || !reason.trim()) return; setBusy(true); setMessage(null); try { await researchOsApi.createMarketObservation(caseId, factor.id, { market_instrument_binding_id: bindingId, event_at: new Date(eventAt).toISOString(), available_at: new Date(availableAt).toISOString(), window_label: windowLabel.trim(), benchmark: benchmark.trim(), price_source: priceSource.trim(), relative_return: relativeReturn.trim() ? Number(relativeReturn) : null, reviewed_by: reviewer.trim(), review_reason: reason.trim() }); const notice = "已追加已审核市场观测；它描述窗口表现，不构成因果结论。 "; setMessage(notice); onSaved(notice); } catch { setMessage("市场观测未登记。请核对股票绑定、时点、数据来源和审核理由。 "); } finally { setBusy(false); } }
  return <section className="ros-fundamental-register ros-market-observation-register"><p>市场窗口只记录发生了什么，不表示研报、事件或因素造成价格变化。</p>{!open ? <button className="ros-button ros-button--secondary" type="button" disabled={!factor || !stockBindings.length} onClick={begin}>登记市场观测</button> : <div><label>观测标的<select aria-label="观测标的" value={bindingId} onChange={(event) => setBindingId(event.target.value)}>{stockBindings.map((binding) => <option key={binding.id} value={binding.id}>{binding.company_name} · {binding.stock_code} · {binding.relationship_role}</option>)}</select></label><label>事件时间<input aria-label="事件时间" type="datetime-local" value={eventAt} onChange={(event) => setEventAt(event.target.value)} /></label><label>资料可得时间<input aria-label="资料可得时间" type="datetime-local" value={availableAt} onChange={(event) => setAvailableAt(event.target.value)} /></label><label>观测窗口<input aria-label="观测窗口" value={windowLabel} onChange={(event) => setWindowLabel(event.target.value)} /></label><label>比较基准<input aria-label="比较基准" value={benchmark} onChange={(event) => setBenchmark(event.target.value)} /></label><label>价格来源<input aria-label="价格来源" value={priceSource} onChange={(event) => setPriceSource(event.target.value)} /></label><label>相对表现<input aria-label="相对表现" type="number" step="any" value={relativeReturn} onChange={(event) => setRelativeReturn(event.target.value)} placeholder="例如 -0.012" /></label><label>审核人<input aria-label="市场观测审核人" value={reviewer} onChange={(event) => setReviewer(event.target.value)} /></label><label>审核理由<textarea aria-label="市场观测审核理由" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="说明窗口、基准和价格来源为何适用于这条观察…" /></label><button className="ros-button ros-button--primary" type="button" disabled={busy || !bindingId || !reason.trim()} onClick={() => void save()}>{busy ? "正在追加市场观测…" : "保存已审核市场观测"}</button></div>}{!stockBindings.length && <p className="ros-note">先建立包含股票的已审核标的关联，才可登记市场观测。</p>}{message && <p className={message.startsWith("已追加") ? "ros-success" : "ros-error"}>{message}</p>}</section>;
}

...
export function MarketExpressionContent({ caseId }: { caseId: string }) {
  ...
  return <section className="ros-market ros-market-expression">
    ...
    <div className="ros-market-workbench">
      ...
      <section className="ros-market-chain" aria-live="polite"><p className="ros-eyebrow">当前因素的四段链路</p><h3>{selectedFactor?.name ?? "尚未选择关键因素"}</h3><p className="ros-market-status">{transmissionStatus}</p><div className="ros-chain-row"><article><span>01 · 研报主张</span><b>{selectedClaim?.text ?? (selectedFactor?.report_claim_id ? "上游主张未在当前截点可见" : "独立关键因素")}</b><small>{selectedClaim && <><span>{claimKindLabels[selectedClaim.claim_kind] ?? selectedClaim.claim_kind}</span> · 审核 {selectedClaim.reviewed_by}</>}</small>{sourceMetadata && <><small>{sourceMetadata.source_url ? <a className="ros-source-link" href={sourceMetadata.source_url} target="_blank" rel="noreferrer">查看研报原文</a> : "原文链接未记录"}{sourceMetadata.document_title ? ` · ${sourceMetadata.document_title}` : ""}</small><small>定位 {sourceMetadata.locator ? JSON.stringify(sourceMetadata.locator) : "未记录"} · 可得 {sourceMetadata.available_at ? new Date(sourceMetadata.available_at).toLocaleString("zh-CN") : "未记录"}</small><small>许可 {sourceMetadata.permission_status}</small></>}<small>{selectedFactor ? `允许来源：${selectedFactor.allowed_source_types.join("、") || "未记录"}` : ""}</small></article><i>→</i><article><span>02 · 公司传导</span><b>{fundamentals.length ? `${fundamentals.length} 家已审核公司` : "尚未建立已审核传导"}</b><small>{fundamentals.map((impact) => `${impact.company_name}${impact.stock_code ? ` · ${impact.stock_code}` : " · 未上市/未映射"}`).join("；") || "需补关系来源与机制"}</small></article><i>→</i><article><span>03 · 股票验证</span><b>{hasMarketEvidence ? `${observations.length} 条市场观测` : "尚未取得市场验证"}</b><small>{hasMarketEvidence ? observations.map((item) => `${item.stock_code} ${item.window_label}`).join("；") : "经营数据与市场观测必须分开记录"}</small></article><i>→</i><article><span>04 · 基金披露</span><b>{relatedFunds.length ? `${relatedFunds.length} 只基金有历史披露` : "暂无可关联披露"}</b><small>{relatedFunds.length ? "仅展示已披露仓位，不表示实时仓位" : "未上市公司不进入基金暴露；披露不足不估算"}</small></article></div>

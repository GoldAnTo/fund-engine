...
export function MarketExpressionContent({
  ...
  const publishedForecastVerdict = selectedFactor?.forecast_verdict ?? null;
  return (
      ...
      <div className="ros-market-workbench">
          ...
          {expression.factors.length ? (
            expression.factors.map((factor) => (
              ...
              >
                <i
                  className={
                  (factor.forecast_verdict?.outcome ?? factor.verification?.outcome) ===
                  "contradicted"
                    ? "is-risk"
                    : factor.forecast_verdict || factor.verification
                        ? ""
                        : "is-gap"
                  }
                />
                ...
                <b>
                  {factor.forecast_verdict || factor.verification
                    ? (verificationLabels[
                        factor.forecast_verdict?.outcome ??
        ...
        <section className="ros-market-chain" aria-live="polite">
          ...
          <ForecastVerdictPanel
            caseId={caseId}
            verdict={publishedForecastVerdict}
          />
...
function ForecastVerdictPanel({
  ...
  caseId: string;
  verdict: MarketExpression["factors"][number]["forecast_verdict"];
}) {
  if (!verdict) {
    return (
      <section className="ros-expression-lane" aria-label="预测验证">
        <article className="ros-expression-card">
          <strong>预测验证待人审</strong>
          <p>机器候选不会作为正式结论展示；请在冻结预测、实际公告与判据齐备后追加人工 verdict。</p>
        </article>
      </section>
    );
  }
  const label = verificationLabels[verdict.outcome] ?? verdict.outcome;
  return (
    <section className="ros-expression-lane" aria-label="预测验证">
      <article className="ros-expression-card">
        <strong>{label}</strong>
        <small>人工已审核 · {verdict.reviewed_by}</small>
        <p>
          预测 {verdict.expected_value.toLocaleString()} {verdict.unit}
          {verdict.baseline_value !== null
            ? `；冻结基线 ${verdict.baseline_value.toLocaleString()} ${verdict.unit}`
            : ""}
          {`；实际 ${verdict.actual_value.toLocaleString()} ${verdict.unit}`}
        </p>
        <small>
          期间 {verdict.forecast_period_start} 至 {verdict.forecast_period_end} ·
          判据 {verdict.comparator}
          {verdict.relative_tolerance !== null
            ? `（容差 ${(verdict.relative_tolerance * 100).toFixed(0)}%）`
            : ""}
        ...
        <small>
          {verdict.forecast_source.document_version_id ? (
            <Link to={`/events/${caseId}/documents?document=${verdict.forecast_source.document_version_id}`}>
              预测冻结原文
            </Link>
          ) : "预测来源版本未记录"}
          {" · "}
          {verdict.actual_source.document_version_id ? (
            <Link to={`/events/${caseId}/documents?document=${verdict.actual_source.document_version_id}`}>
              实际公告原文
            </Link>
          ) : "实际来源版本未记录"}
        </small>

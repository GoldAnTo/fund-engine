...
function ScopeEditor({ caseId, initialFactors, currentVersion }: { caseId: string; initialFactors: EventWorkbench["scope"]["factors"]; currentVersion: number }) {
  const [factors, setFactors] = useState(() => initialFactors.map((factor) => factor.statement)); const [busy, setBusy] = useState(false); const [notice, setNotice] = useState<string | null>(null);
  ...
  async function save() { if (normalized.length < 3 || normalized.length > 5 || new Set(normalized).size !== normalized.length) { setNotice("请保留 3–5 个不重复的关键因素；空白项不会保存。"); return; } setBusy(true); setNotice(null); try { const updated = await researchClient.updateEventResearchScope({ caseId, factors: normalized, changedBy: "human:researcher" }); setFactors(updated.factors.map((factor) => factor.statement)); setNotice(`已创建范围版本 v${updated.version}；旧版本与其证据映射仍可回放。`); } catch { setNotice("范围未更新。已发布 Case 不能直接改写范围；请先从新材料启动后继研究。 "); } finally { setBusy(false); } }
  return <section className="ros-scope-editor"><header className="ros-section-heading"><div><p className="ros-eyebrow">范围版本 v{currentVersion}</p><h2>调整关键因素，创建新的研究范围</h2><p>范围更新会重新映射已审核证据，并只影响后续研究；过去的运行和结论仍使用各自冻结的范围版本。</p></div><Link className="ros-button ros-button--secondary" to={`/events/${caseId}/evidence`}>查看当前证据</Link></header><div className="ros-scope-editor__form">{factors.map((factor, index) => <label key={index}>关键因素 {index + 1}<textarea aria-label={`关键因素 ${index + 1}`} value={factor} onChange={(event) => edit(index, event.target.value)} /></label>)}</div><div className="ros-scope-editor__actions"><button className="ros-button ros-button--secondary" type="button" disabled={factors.length <= 3} onClick={() => setFactors((current) => current.slice(0, -1))}>移除最后一项</button><button className="ros-button ros-button--secondary" type="button" disabled={factors.length >= 5} onClick={() => setFactors((current) => [...current, ""])}>增加因素</button><button className="ros-button ros-button--primary" type="button" disabled={busy} onClick={save}>{busy ? "正在创建范围版本…" : "保存新的研究范围"}</button></div>{notice && <p className={notice.startsWith("已创建") ? "ros-success" : "ros-error"}>{notice}</p>}</section>;
}

...
import { researchOsApi, type AtomicClaimCandidate, type CaseMechanismProtocol, type MechanismTemplate, type MetricDefinition, type Monitor, type MonitorDetail, type Researchability } from "../../app/researchOsApi";
...
export function CaseDocumentsPage() { return <CaseFrame>{(data, caseId) => <CaseDocumentsContent caseId={caseId} isPublished={data.lifecycle.status === "published"} />}</CaseFrame>; }
function CaseDocumentsContent({ caseId, isPublished }: { caseId: string; isPublished: boolean }) {
  ...
  return <section className="ros-documents"><header className="ros-section-heading"><div><p className="ros-eyebrow">来源阅读</p><h2>原文资料</h2><p>只显示已关联当前 Case 的资料；正文、定位与版本来自资料记录，而非系统推测。</p></div><Link className="ros-button ros-button--secondary" to={`/events/${caseId}/evidence`}>返回命题与证据</Link></header>{isPublished && <PublishedMaterialDecisionForm caseId={caseId} onFrozen={(id) => { setPreferredDocumentId(id); setDocumentReload((value) => value + 1); }} />}{error && <p className="ros-error">{error}</p>}{extractionNotice && <p className="ros-success">{extractionNotice} <Link to={`/events/${caseId}/review`}>进入证据审核 →</Link></p>}{!documents ? <div className="ros-empty">正在读取当前 Case 的原文资料…</div> : documents.length === 0 ? <div className="ros-empty">当前 Case 暂无可读取的原文资料；这不表示其他 Case 的资料可以被复用。</div> : <div className="ros-documents-grid"><section className="ros-document-list" aria-label="当前 Case 原文资料">{documents.map((document) => <button type="button" key={document.id} className={`ros-document-row${document.id === selectedId ? " is-selected" : ""}`} onClick={() => selectDocument(document.id)}><span className={`ros-pill ${document.parse_quality === "ok" ? "ros-pill--system" : "ros-pill--human"}`}>{document.parse_quality === "ok" ? "解析完成" : document.parse_quality === "partial" ? "解析不完整" : "解析失败"}</span><strong>{document.title || "未命名资料"}</strong><small>{document.publisher || "发布方未记录"} · {document.document_type || "类型未记录"}</small><small>{document.version_label || "版本未记录"} · {document.span_count} 个定位片段</small></button>)}</section><section className="ros-document-reader" aria-live="polite">{!detail ? <div className="ros-empty ros-empty--compact">正在读取冻结内容…</div> : <DocumentReader caseId={caseId} detail={detail} isPublished={isPublished} focusSpanId={focusSpanId} extracting={extractingId === detail.document.id} onExtract={() => extractCandidates(detail.document)} onSupplementCreated={(id) => { setNoClaimDocumentId(null); setPreferredDocumentId(id); setDocumentReload((value) => value + 1); }} needsSupplement={needsSupplement} recoveryReason={currentRecoveryReason} recoveryActive={recoveryTarget?.documentId === detail.document.id && recoveryTarget.reason === currentRecoveryReason} onBeginRecovery={() => beginRecovery(detail.document.id, currentRecoveryReason)} onCancelRecovery={cancelRecovery} onStartNewMaterials={() => navigate("/events/new")} />}</section></div>}</section>;
}

function PublishedMaterialDecisionForm({ caseId, onFrozen }: { caseId: string; onFrozen: (documentId: string) => void }) {
  const navigate = useNavigate(); const [rawInput, setRawInput] = useState(""); const [sourceUrl, setSourceUrl] = useState(""); const [sourceType, setSourceType] = useState<"pasted_snapshot" | "uploaded_file" | "licensed_provider">("pasted_snapshot"); const [decision, setDecision] = useState<"reopen" | "no_change">("reopen"); const [reason, setReason] = useState(""); const [busy, setBusy] = useState(false); const [message, setMessage] = useState<string | null>(null);
  async function submit() { if (!rawInput.trim() || !reason.trim()) return; setBusy(true); setMessage(null); try { const result = await researchClient.decidePublishedMaterial({ caseId, rawInput: rawInput.trim(), sourceUrl: sourceUrl.trim() || undefined, sourceType, sourceMetadata: { authority_level: sourceType === "licensed_provider" ? "licensed_research" : "user_supplied" }, decision, reason: reason.trim(), actor: "human:researcher" }); onFrozen(result.documentVersionId); setMessage(result.decision === "reopen" ? `已冻结新材料并创建后继运行 ${result.runId}；此前发布结论未被改写。` : `已冻结新材料并记录“不改变当前判断”的人工决定 ${result.decisionEventId}；发布结论保持有效。`); if (result.decision === "reopen") navigate(`/events/${caseId}/monitor`); } catch { setMessage("新材料或人工决定未保存；此前发布结论保持不变。请检查必填信息后重试。"); } finally { setBusy(false); } }
  return <section className="ros-material-continuation"><p className="ros-eyebrow">已发布结论 · 新材料决定</p><h3>新材料是否需要改变复核范围？</h3><p>先冻结这份材料，再由研究员决定纳入重新复核或记录为不改变当前判断。两种决定都会保留原因；系统不会静默改写发布结论。</p><label>新材料正文<textarea aria-label="新增材料正文" value={rawInput} onChange={(event) => setRawInput(event.target.value)} placeholder="粘贴新研报、公告或其他已获准使用的正文…" /></label><label>来源接入方式<select aria-label="新增材料来源接入方式" value={sourceType} onChange={(event) => setSourceType(event.target.value as typeof sourceType)}><option value="pasted_snapshot">粘贴快照</option><option value="uploaded_file">上传文本快照</option><option value="licensed_provider">授权数据源快照</option></select></label><label>来源链接（可选）<input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://…" /></label><fieldset><legend>人工决定</legend><label><input type="radio" checked={decision === "reopen"} onChange={() => setDecision("reopen")} />纳入重新复核：创建后继运行，旧结论保持可回放</label><label><input type="radio" checked={decision === "no_change"} onChange={() => setDecision("no_change")} />记录为不改变当前判断：保留材料和理由，不启动运行</label></fieldset><label>决定理由<textarea aria-label="新材料决定理由" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="说明它影响或不影响哪个已发布判断、关键因素或反证条件…" /></label><button className="ros-button ros-button--primary" type="button" disabled={!rawInput.trim() || !reason.trim() || busy} onClick={() => void submit()}>{busy ? "正在冻结并记录决定…" : decision === "reopen" ? "冻结材料并纳入重新复核" : "冻结材料并记录不改变判断"}</button>{message && <p className={message.startsWith("已冻结") ? "ros-success" : "ros-error"}>{message}</p>}</section>;
}
function DocumentReader({ caseId, detail, isPublished, focusSpanId, extracting, onExtract, onSupplementCreated, needsSupplement, recoveryReason, recoveryActive, onBeginRecovery, onCancelRecovery, onStartNewMaterials }: { caseId: string; detail: { document: SourceDocumentView; spans: DocumentSpan[] }; isPublished: boolean; focusSpanId: string | null; extracting: boolean; onExtract: () => void; onSupplementCreated: (documentId: string) => void; needsSupplement: boolean; recoveryReason: "parse_failed" | "no_claims"; recoveryActive: boolean; onBeginRecovery: () => void; onCancelRecovery: () => void; onStartNewMaterials: () => void }) {
...
export function CaseConclusionPage() {
  return <CaseFrame>{(data, caseId) => <section className="ros-case-columns">
    ...
    <aside className="ros-case-rail"><section className="ros-action-card"><p className="ros-eyebrow">下一步</p><h2>{data.nextAction.label}</h2><p>{data.lifecycle.currentGap || "先完成当前人工判断，再决定是否创建新的结论或范围版本。"}</p><Link className="ros-button ros-button--primary" to={`/events/${caseId}/${data.nextAction.kind === "review_intake" ? "documents" : data.nextAction.kind === "review_evidence" || data.nextAction.kind === "review_conclusion" ? "review" : data.nextAction.kind === "view_conclusion_change" ? "history" : data.nextAction.kind === "edit_factors" ? "scope" : "monitor"}`}>{data.nextAction.kind === "review_intake" ? "核验冻结原文" : data.nextAction.kind.includes("review") ? "进入审核" : data.nextAction.kind === "view_conclusion_change" ? "查看结论版本" : data.nextAction.kind === "edit_factors" ? "调整研究范围" : "查看持续研究"}</Link></section><section className="ros-rail-section"><p className="ros-eyebrow">持续研究</p><h2>{data.lifecycle.activeRunId ? "系统正在受控补证" : "尚未授权后台运行"}</h2><p>{data.lifecycle.activeRunId ? `范围版本 v${data.scope.version} · 仅允许来源内的材料可进入后续审核。` : "完成原文核验、来源许可与研究协议后，才可配置并触发一次可回放的补证运行。"}</p><Link to={`/events/${caseId}/monitor`}>查看运行记录 →</Link></section><section className="ros-rail-section"><p className="ros-eyebrow">结论依据</p><p>{data.conclusion.citations.length} 条可回溯引用；每条都保留原文定位、可用时点与审核状态。</p></section></aside>
  </section>}</CaseFrame>;
}

export function CaseConclusionHistoryPage() { return <CaseFrame>{(_data, caseId) => <ConclusionHistoryContent caseId={caseId} />}</CaseFrame>; }
export function CaseScopePage() { return <CaseFrame>{(data, caseId) => <ScopeEditor caseId={caseId} initialFactors={data.scope.factors} currentVersion={data.scope.version} />}</CaseFrame>; }
function ScopeEditor({ caseId, initialFactors, currentVersion }: { caseId: string; initialFactors: EventWorkbench["scope"]["factors"]; currentVersion: number }) {
  const [factors, setFactors] = useState(() => initialFactors.map((factor) => factor.statement)); const [reason, setReason] = useState("调整关键因素以补足当前验证缺口"); const [history, setHistory] = useState<Awaited<ReturnType<typeof researchOsApi.scopeHistory>>["items"]>([]); const [busy, setBusy] = useState(false); const [notice, setNotice] = useState<string | null>(null);
  useEffect(() => { setFactors(initialFactors.map((factor) => factor.statement)); setNotice(null); }, [initialFactors]);
  useEffect(() => { researchOsApi.scopeHistory(caseId).then((value) => setHistory(value.items)).catch(() => setHistory([])); }, [caseId, notice]);
  const normalized = factors.map((factor) => factor.trim()).filter(Boolean);
  function edit(index: number, value: string) { setFactors((current) => current.map((factor, position) => position === index ? value : factor)); }
  async function save() { if (normalized.length < 3 || normalized.length > 5 || new Set(normalized).size !== normalized.length) { setNotice("请保留 3–5 个不重复的关键因素；空白项不会保存。"); return; } if (!reason.trim()) { setNotice("请记录本次范围调整的原因，历史版本才可复现。"); return; } setBusy(true); setNotice(null); try { const updated = await researchClient.updateEventResearchScope({ caseId, factors: normalized, changedBy: "human:researcher", changeReason: reason.trim() }); setFactors(updated.factors.map((factor) => factor.statement)); setNotice(`已创建范围版本 v${updated.version}；旧版本与其证据映射仍可回放。`); } catch { setNotice("范围未更新。已发布 Case 不能直接改写范围；请先从新材料启动后继研究。 "); } finally { setBusy(false); } }
  return <section className="ros-scope-editor"><header className="ros-section-heading"><div><p className="ros-eyebrow">范围版本 v{currentVersion}</p><h2>调整关键因素，创建新的研究范围</h2><p>范围更新会重新映射已审核证据，并只影响后续研究；过去的运行和结论仍使用各自冻结的范围版本。</p></div><Link className="ros-button ros-button--secondary" to={`/events/${caseId}/evidence`}>查看当前证据</Link></header><div className="ros-scope-editor__form">{factors.map((factor, index) => <label key={index}>关键因素 {index + 1}<textarea aria-label={`关键因素 ${index + 1}`} value={factor} onChange={(event) => edit(index, event.target.value)} /></label>)}<label>本次调整原因<textarea aria-label="本次调整原因" value={reason} onChange={(event) => setReason(event.target.value)} /></label></div><div className="ros-scope-editor__actions"><button className="ros-button ros-button--secondary" type="button" disabled={factors.length <= 3} onClick={() => setFactors((current) => current.slice(0, -1))}>移除最后一项</button><button className="ros-button ros-button--secondary" type="button" disabled={factors.length >= 5} onClick={() => setFactors((current) => [...current, ""])}>增加因素</button><button className="ros-button ros-button--primary" type="button" disabled={busy} onClick={save}>{busy ? "正在创建范围版本…" : "保存新的研究范围"}</button></div>{notice && <p className={notice.startsWith("已创建") ? "ros-success" : "ros-error"}>{notice}</p>}<section className="ros-rule-history"><p className="ros-eyebrow">范围版本历史</p><h2>谁在何时因何调整了范围</h2><ol>{history.map((item) => <li key={item.version}><strong>v{item.version}</strong><span>{item.changed_by} · {item.created_at}</span><p>{item.change_reason}</p><small>{item.factors.map((factor) => factor.statement).join("；")}</small></li>)}</ol></section></section>;
}
...
export function CaseMarketPage() { return <CaseFrame>{(data, caseId) => <MarketExpressionContent caseId={caseId} theses={data.factors.flatMap((factor) => factor.thesisId ? [{ id: factor.thesisId, statement: factor.statement }] : [])} />}</CaseFrame>; }
...
function MonitorContent({ caseId, activeRunId }: { caseId: string; activeRunId: string | null }) {
  ...
  const loadMonitor = () => researchOsApi.monitor(caseId).then((value) => { setDetail(value); setSelectedRunId(value.latest_run?.id ?? null); }).catch(() => setError("监控配置尚不可用。"));
  const loadEvents = (runId: string) => researchOsApi.runEvents(runId).then((response) => setEvents(response.items.map((event) => ({ seq: event.seq, stage: event.stage ?? null, status: event.status ?? null, message: event.message ?? null, details: event.details ?? {}, createdAt: event.created_at })))).catch(() => setError("该运行的事件记录暂不可读。"));
  useEffect(() => { void loadMonitor(); }, [caseId]);
  useEffect(() => { if (!selectedRunId) { setEvents([]); return; } void loadEvents(selectedRunId); const refresh = window.setInterval(() => void loadEvents(selectedRunId), 5_000); return () => window.clearInterval(refresh); }, [selectedRunId]);
  ...
  async function startNow() { if (!detail?.monitor) { setError("请先保存一份 CaseMonitor 配置，系统才能冻结本次补证范围。"); return; } setStarting(true); setError(null); try { const created = await researchOsApi.startMonitorRun(caseId); setSelectedRunId(created.id); await loadEvents(created.id); setDrawer(true); } catch { setError("无法按当前 CaseMonitor 版本创建立即补证运行；没有写入部分运行。请检查服务状态后重试。"); } finally { setStarting(false); } }
  async function cancelRun(changeReason: string) { if (!selectedRunId) return; setError(null); try { await researchOsApi.cancelRun(selectedRunId, changeReason); await Promise.all([loadMonitor(), loadEvents(selectedRunId)]); } catch { setError("停止运行失败；原运行状态与记录未被页面伪造修改。请刷新后重试。"); } }
  return <section className="ros-monitor"><header className="ros-section-heading"><div><p className="ros-eyebrow">监控与版本</p><h2>每一次运行均可检查、复现与配置</h2></div><div className="ros-header-actions"><button className="ros-button ros-button--primary" type="button" disabled={starting || !detail?.monitor || protocolBlockers.length > 0} onClick={startNow}>{starting ? "正在创建运行…" : "立即补证一次"}</button><button className="ros-button ros-button--secondary" type="button" disabled={!selectedRunId} onClick={() => setDrawer(true)}>打开运行详情</button></div></header>{protocolBlockers.length > 0 && <section className="ros-rulebox"><b>研究协议尚未通过，不能启动补证。</b>{protocolBlockers.map((blocker) => <p key={`${blocker.id}:${blocker.code}`}>{protocolReason[blocker.code] || blocker.code}；下一步：{blocker.nextAction}</p>)}<Link className="ros-button ros-button--primary" to={`/events/${caseId}/protocol`}>补齐研究协议</Link></section>}{error && <p className="ros-error">{error}</p>}<div className="ros-monitor-grid"><section className="ros-card ros-run-card"><header className="ros-card-head"><div><p className="ros-eyebrow">{run ? "选中的运行" : "尚未开始运行"}</p><h2>{run ? `${run.stage} · ${run.status}` : "先配置持续研究"}</h2></div><span>{run ? `更新于 ${run.updated_at}` : ""}</span></header><div className="ros-run-summary"><strong>{events.length ? "系统的每一步都在记录" : "运行将从范围冻结开始记录"}</strong><p>{events.length ? "查看每个阶段的输入、允许来源、排除理由和输出；系统不会在后台悄悄改写结论。" : "立即运行或定时任务都会绑定有效的 CaseMonitor 配置版本。"}</p></div><ol className="ros-run-log">{events.length ? events.map((event) => <li key={event.seq}><span>{event.seq}</span><div><strong>{event.stage || "运行阶段"} · {event.status || "已记录"}</strong><p>{event.message || "无文字摘要"}</p><small>{Object.entries(event.details).map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join("、") : String(value)}`).join(" · ")}</small></div></li>) : <li><span>1</span><div><strong>等待运行</strong><p>尚无结构化运行事件；新运行会从读取并冻结配置开始逐条记录。</p></div></li>}</ol></section><aside className="ros-monitor-config"><section className="ros-panel"><p className="ros-eyebrow">当前 CaseMonitor · {detail?.monitor ? `配置 v${detail.monitor.version}` : "未配置"}</p><h2>用于未来运行的配置</h2><p>它只决定下一次立即补证与定时任务；历史运行必须以它自己的冻结事件回放。</p>{detail?.monitor ? <dl className="ros-definition"><div><dt>运行频率</dt><dd>{detail.monitor.frequency} · 中国标准时间</dd></div><div><dt>最近一次运行</dt><dd>{detail.latest_run ? new Date(detail.latest_run.updated_at).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" }) : "尚无运行记录"}</dd></div><div><dt>下次定时检查</dt><dd>{detail.next_scheduled_at ? new Date(detail.next_scheduled_at).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" }) : detail.monitor.status === "paused" ? "已暂停" : "当前频率未被调度器支持"}</dd></div><div><dt>允许来源</dt><dd>{detail.monitor.allowed_source_types.join("、")}</dd></div><div><dt>下一验证</dt><dd>{detail.monitor.next_verification_event}</dd></div><div><dt>单次上限</dt><dd>{detail.monitor.budget} 份候选资料</dd></div></dl> : <div className="ros-empty ros-empty--compact">尚未设置监控条件。</div>}<Link className="ros-button ros-button--primary" to={`/events/${caseId}/monitor/config`}>调整配置</Link></section><p className="ros-rulebox"><b>可复现要求：</b>触发人/原因、配置版本、查询口径、来源许可、资料版本、阶段输出、排除理由和失败原因均要可查看。</p></aside></div>{drawer && selectedRunId && <RunDrawer run={run} detail={detail} events={events} onCancel={cancelRun} onClose={() => setDrawer(false)} />}</section>;
}

function RunDrawer({ run, detail, events, onCancel, onClose }: { run: MonitorDetail["latest_run"] | { id: string; status: string; stage: string; updated_at: string } | null; detail: MonitorDetail | null; events: RunEvent[]; onCancel: (reason: string) => Promise<void>; onClose: () => void }) { const [reason, setReason] = useState(""); const [stopping, setStopping] = useState(false); const scope = events.find((event) => event.stage === "scope")?.details; const value = (key: string) => scope?.[key]; const list = (key: string) => { const current = value(key); return Array.isArray(current) ? current.join("、") : current ? String(current) : null; }; const canStop = ["queued", "running", "waiting_for_review"].includes(run?.status || ""); async function stop() { if (!reason.trim()) return; setStopping(true); try { await onCancel(reason.trim()); setReason(""); } finally { setStopping(false); } } return <aside className="ros-run-drawer" aria-label="运行详情"><header><div><p className="ros-eyebrow">ResearchRun · {run?.id || "尚无运行"}</p><h2>{run ? `${run.stage} · ${run.status}` : "运行详情"}</h2><p>{scope ? "以下口径来自本次运行的冻结事件。" : "冻结口径尚未在事件记录中提供，不能用当前配置替代。"}</p></div><button aria-label="关闭运行详情" type="button" onClick={onClose}>×</button></header><div className="ros-drawer-body"><section className="ros-run-scope"><p className="ros-eyebrow">本次运行口径</p><dl><div><dt>触发方式</dt><dd>{list("trigger") || "未记录"}</dd></div><div><dt>配置版本</dt><dd>{list("monitor_version_id") || "未记录"}</dd></div><div><dt>范围因素</dt><dd>{list("factor_statements") || list("factor_ids") || "未记录"}</dd></div><div><dt>允许来源</dt><dd>{list("allowed_source_types") || "未记录"}</dd></div><div><dt>检索预算</dt><dd>{list("budget") || "未记录"}</dd></div><div><dt>运行事件</dt><dd>{events.length} 条</dd></div></dl>{!scope && detail?.monitor && <p className="ros-note">当前配置为 v{detail.monitor.version}，仅供下一次运行使用。</p>}</section>{canStop && <section className="ros-run-control"><p className="ros-eyebrow">人工停止</p><p>停止会取消尚未完成的任务，但不覆盖本次冻结范围、已产生的阶段记录或历史结论。</p><label>停止原因<textarea aria-label="停止原因" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="说明为什么现在应停止这次运行" /></label><button className="ros-button ros-button--secondary" type="button" disabled={!reason.trim() || stopping} onClick={() => void stop()}>{stopping ? "正在停止…" : "停止本次运行"}</button></section>}<ol className="ros-run-log">{events.map((event) => <li key={event.seq}><span>{event.seq}</span><div><strong>{event.stage || "阶段"}</strong><p>{event.message || "无文字摘要"}</p></div></li>)}</ol></div></aside>; }

export function MonitorConfigPage() { return <CaseFrame>{(_data, caseId) => <><MonitorConfigForm caseId={caseId} /><MonitorHistory caseId={caseId} /></>}</CaseFrame>; }
function MonitorHistory({ caseId }: { caseId: string }) { const [history, setHistory] = useState<Monitor[]>([]); useEffect(() => { researchOsApi.monitor(caseId).then((value) => setHistory(value.history ?? [])).catch(() => setHistory([])); }, [caseId]); return <section className="ros-rule-history"><p className="ros-eyebrow">CaseMonitor 版本历史</p><h2>配置由谁、为何变更</h2>{history.length ? <ol>{history.map((monitor) => <li key={monitor.id}><strong>v{monitor.version} · {monitor.status}</strong><span>{monitor.changed_by} · {monitor.created_at}</span><p>{monitor.change_reason}</p><small>{monitor.frequency} · {monitor.allowed_source_types.join("、")} · 预算 {monitor.budget}</small></li>)}</ol> : <div className="ros-empty ros-empty--compact">尚无可展示的监控配置版本。</div>}</section>; }

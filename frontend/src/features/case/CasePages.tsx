...
function CaseDocumentsContent({ caseId }: { caseId: string }) {
  ...
  useEffect(() => { let active = true; setDocuments(null); setDetail(null); setError(null); researchClient.getDocuments({ caseId }).then((value) => { if (!active) return; setDocuments(value); const requested = searchParams.get("document"); setSelectedId(value.some((item) => item.id === requested) ? requested : value[0]?.id ?? null); }).catch(() => active && setError("无法读取这个 Case 的原文资料；系统没有展示其他 Case 的替代内容。")); return () => { active = false; }; }, [caseId, searchParams]);
...
function DocumentReader({ caseId, detail, extracting, onExtract }: { caseId: string; detail: { document: SourceDocumentView; spans: DocumentSpan[] }; extracting: boolean; onExtract: () => void }) {
  ...
  return <><header><span className={`ros-pill ${contract?.status === "admitted" ? "ros-pill--system" : "ros-pill--human"}`}>{contract?.status === "admitted" ? "来源已准入" : "许可未完整记录"}</span><h3>{document.title || "未命名资料"}</h3><p>内容快照（当前 V1 未提供原件文件）</p></header><dl className="ros-definition"><div><dt>冻结资料 ID</dt><dd><code>{document.id}</code></dd></div><div><dt>来源类型</dt><dd>{contract?.source_type || document.document_type || "未记录"}</dd></div><div><dt>来源权威性</dt><dd>{document.source_authority || "unknown"}</dd></div><div><dt>发布方</dt><dd>{contract?.provider_or_tenant || document.publisher || "未记录"}</dd></div><div><dt>发布日期</dt><dd>{document.publish_date || "未记录"}</dd></div><div><dt>可用 / 采集时点</dt><dd>{document.available_at} / {document.acquired_at}</dd></div><div><dt>解析版本</dt><dd>{document.parser_version} · {document.parse_quality}</dd></div><div><dt>许可 / 展示范围</dt><dd>{permissionText}</dd></div>{contract && <><div><dt>保留策略</dt><dd>{contract.retention_policy} · {contract.deletion_policy}</dd></div><div><dt>下游限制</dt><dd>{contract.downstream_restrictions.join("；") || "无额外记录"}</dd></div></>}</dl>{document.parse_quality === "failed" ? <SupplementRecovery caseId={caseId} documentId={document.id} failureStage={document.parse_failure_stage || "未记录阶段"} /> : <><section className="ros-document-extract"><p><b>下一步：从冻结原文提取候选。</b>只会创建待人工审核的原子陈述；不会发布 SourceStatement、不会启动补证、监控或市场任务。</p><button className="ros-button ros-button--primary" type="button" disabled={!extractionAllowed || extracting} onClick={onExtract}>{extracting ? "正在提取候选…" : "从冻结资料提取候选"}</button>{!extractionAllowed && <small>此版本解析失败或来源合同禁止 AI 处理，不能请求候选抽取。</small>}</section><section className="ros-source-spans"><p className="ros-eyebrow">可定位正文片段</p>{spans.length ? spans.map((span) => <article key={span.id}><code>{JSON.stringify(span.locator)}</code><blockquote>{span.verbatim_text}</blockquote><small>{span.cited_by.length ? `已被 ${span.cited_by.length} 条证据关系引用` : "尚未被证据关系引用"}</small></article>) : <div className="ros-empty ros-empty--compact">当前资料没有返回可定位正文片段。</div>}</section></>}</>;
}

function SupplementRecovery({ caseId, documentId, failureStage }: { caseId: string; documentId: string; failureStage: string }) {
  const [open, setOpen] = useState(false); const [rawText, setRawText] = useState(""); const [page, setPage] = useState(""); const [busy, setBusy] = useState(false); const [message, setMessage] = useState<string | null>(null);
  async function submit() { if (!rawText.trim() || !page.trim()) return; setBusy(true); setMessage(null); try { const result = await researchOsApi.createDocumentSupplement(documentId, { case_id: caseId, raw_text: rawText.trim(), claimed_page_reference: page.trim(), created_by: "human:researcher", source_metadata: { authority_level: "user_supplied" } }); setMessage(result.extraction_allowed ? "补充正文已独立冻结；下一步可从新版本提取待审候选。" : "补充正文已独立冻结，但继承后的许可不允许 AI 处理；可保留供人工核验。"); setOpen(false); } catch { setMessage("补充正文未保存；原件保持不变。请检查 Case 权限和必填项后重试。"); } finally { setBusy(false); } }
  return <section className="ros-recovery"><p className="ros-rulebox">该资料在 {failureStage} 解析失败；保留资料记录，暂不能把它写成证据。补充正文会作为独立版本关联原件，页码仅作为你的声明。</p><p>不会改写原件，也不会自动启动研究、发布正式陈述或市场任务。</p><button className="ros-button ros-button--primary" type="button" onClick={() => setOpen((value) => !value)}>{open ? "取消补充" : "补充正文并标注页码"}</button>{open && <div className="ros-recovery__form"><label>补充正文<textarea aria-label="补充正文" value={rawText} onChange={(event) => setRawText(event.target.value)} placeholder="粘贴可核验的原文内容…" /></label><label>声称页码或位置<input aria-label="声称页码或位置" value={page} onChange={(event) => setPage(event.target.value)} placeholder="例如：第 3 页" /></label><button className="ros-button ros-button--primary" type="button" disabled={!rawText.trim() || !page.trim() || busy} onClick={submit}>{busy ? "正在冻结补充正文…" : "冻结补充正文"}</button></div>}{message && <p className={message.startsWith("补充正文已") ? "ros-success" : "ros-error"}>{message}</p>}</section>;
}

...
function ScopeEditor({ caseId, initialFactors, currentVersion }: { caseId: string; initialFactors: EventWorkbench["scope"]["factors"]; currentVersion: number }) {
  const [factors, setFactors] = useState(() => initialFactors.map((factor) => factor.statement)); const [reason, setReason] = useState("调整关键因素以补足当前验证缺口"); const [busy, setBusy] = useState(false); const [notice, setNotice] = useState<string | null>(null);

...
import { isActiveWorkflowState } from "../../domain/eventWorkflow";
import {
...
import { EventWorkflowOverview } from "./EventWorkflowOverview";
...
function workflowDocumentDrilldownFromState(state: unknown): string | null {
  if (!state || typeof state !== "object") return null;
  const drilldown = (state as { documentDrilldown?: unknown }).documentDrilldown;
  return typeof drilldown === "string" && drilldown.startsWith("/events/") ? drilldown : null;
}
...
function EventSwitcher({
  ...
  caseId: string;
  currentTitle: string;
  caseOptions: EventResearchListItem[];
  locationSearch: string;
  onSelect: (caseId: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const triggerRef = useRef<HTMLButtonElement>(null);
  const menuId = `event-switcher-${useId().replace(/:/g, "")}`;
  ...
  function choose(nextCaseId: string) {
    setOpen(false);
    triggerRef.current?.focus();
    onSelect(nextCaseId);
  }
  ...
  return (
    <div className="ros-event-context">
      ...
      <button
        ...
        aria-expanded={open}
        aria-label={`当前事件研究：${currentTitle}，切换事件`}
      ...
      {open && (
        <div className="ros-event-menu" id={menuId}>
          <header>
            <div><span className="ros-eyebrow">事件研究</span><strong>切换当前研究事件</strong></div>
            <Link to={preserveLocationSearch("/events/new", locationSearch)}>＋ 新建事件研究</Link>
          </header>
          ...
          <Link className="ros-event-menu__all" to={preserveLocationSearch("/events", locationSearch)}>查看全部事件研究 →</Link>
        </div>
      )}
    </div>
  );
...
function CaseFrame({
  ...
  const isWorkflowOverview = location.pathname.replace(/\/$/, "") === `/events/${caseId}`;
  if (!data)
    return (
  ...
  return (
    <main className="ros-page ros-case-page">
      <header className="ros-case-header">
        <EventSwitcher caseId={caseId} currentTitle={data.event.eventTitle} caseOptions={caseOptions} locationSearch={location.search} onSelect={switchCase} />
        {!isWorkflowOverview && <p className="ros-eyebrow">
          {[data.event.companyName, data.event.ticker, `范围版本 v${data.scope.version}`].filter(Boolean).join(" · ")}
        </p>}
        {!isWorkflowOverview && <h1>{data.event.eventTitle}</h1>}
        {!isWorkflowOverview && <p>{data.lifecycle.summary}</p>}
        {!isWorkflowOverview && <ol className="ros-event-progress" aria-label="当前事件研究进展">
        ...
        {!isWorkflowOverview && <div className="ros-case-header__facts">
          <span>人工已审核 {data.progress.reviewedCount}</span>
          <span>系统自动采纳，未经人工审核 {data.progress.automaticallyAdmittedCount}</span>
      ...
      {isWorkflowOverview && <EventWorkflowOverview caseId={caseId} />}
...
export function CaseWorkflowOverviewPage() {
  const { caseId = "" } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const [caseOptions, setCaseOptions] = useState<EventResearchListItem[]>([]);
  const workflowNotice = workflowNoticeFromState(location.state);
  const documentDrilldown = workflowDocumentDrilldownFromState(location.state);

  useEffect(() => {
    let active = true;
    researchClient.listEventResearch()
      .then((items) => { if (active) setCaseOptions(items); })
      .catch(() => { if (active) setCaseOptions([]); });
    return () => { active = false; };
  }, []);

  const currentCase = caseOptions.find((item) => item.id === caseId);
  const currentTitle = currentCase?.eventTitle ?? "当前事件";

  return (
    <main className="ros-page ros-case-page">
      <header className="ros-case-header ros-case-header--workflow">
        <EventSwitcher
          caseId={caseId}
          currentTitle={currentTitle}
          caseOptions={caseOptions}
          locationSearch={location.search}
          onSelect={(nextCaseId) => navigate(preserveLocationSearch(`/events/${nextCaseId}`, location.search))}
        />
      </header>
      {workflowNotice && (
        <p className="ros-success" role="status">
          {workflowNotice}
          {documentDrilldown && <> <Link to={preserveLocationSearch(documentDrilldown, location.search)}>查看冻结原文</Link></>}
        </p>
      )}
      <EventWorkflowOverview caseId={caseId} />
      <nav className="ros-case-primary-tabs" aria-label="事件研究工作区">
        {caseSections.map((section) => (
          <Link
            aria-current={section.id === "conclusion" ? "page" : undefined}
            className={section.id === "conclusion" ? "active" : ""}
            key={section.id}
            to={preserveLocationSearch(`/events/${caseId}${section.to ? `/${section.to}` : ""}`, location.search)}
          >
            {section.label}
      ...
      <details className="ros-case-nav-more">
        ...
        <div role="group" aria-label="更多研究内容">
          {caseSections
            .filter((section) => section.id !== "conclusion")
            .map((section) => (
              <Link
                key={section.id}
                to={preserveLocationSearch(`/events/${caseId}${section.to ? `/${section.to}` : ""}`, location.search)}
    ...
    <CaseFrame>
      {(data, caseId) => {
        ...
        return <>
          ...
          <section className="ros-case-columns ros-case-columns--supporting">
            <div>
            ...
            <aside className="ros-case-rail">
              <section className="ros-action-card ros-action-card--supporting">
              ...
              <section className="ros-rail-section">
                ...
                <p>{data.conclusion.citations.length} 条可回溯引用；每条都保留原文定位、可用时点与审核状态。</p>
              </section>
...
function ScopeEditor({
  ...
  const [busy, setBusy] = useState(false);
  const [scopeWriteCommitted, setScopeWriteCommitted] = useState(false);
  ...
  const scopeSaveReady = !busy && !scopeWriteCommitted && scopeMissingRequirements.length === 0;
  ...
  async function save() {
    ...
    try {
      try {
        ...
        if (requestIsCurrent()) {
          setNotice(
            "范围未更新。请确认当前流程仍允许调整范围，并检查服务状态后重试。",
          );
        }
        return;
      }
      if (requestIsCurrent()) setScopeWriteCommitted(true);
      window.dispatchEvent(new Event("research-os-workflow-refresh"));
      if (!requestIsCurrent()) return;
      let nextWorkflow: Awaited<ReturnType<typeof researchClient.getEventWorkflow>>;
      try {
        nextWorkflow = await researchClient.getEventWorkflow(caseId);
      } catch {
        if (requestIsCurrent()) {
          setNotice(
            "范围已更新，最新状态暂不可读。系统不会重复创建范围版本；请只重新读取最新流程。",
          );
        }
        return;
      }
      if (!requestIsCurrent()) return;
      const needsProtocol = nextWorkflow.userAction?.recommendation.kind
        === "complete_research_protocol";
      navigate(
        preserveLocationSearch(
          needsProtocol
            ? `/events/${request.caseId}/protocol?focus=required`
            : `/events/${request.caseId}`,
          request.search,
        ),
        {
          state: {
            workflowNotice: needsProtocol
              ? "范围已更新，需完成研究协议。"
              : "研究范围已更新，系统已按新范围继续补证。",
          },
        },
  ...
  async function rereadCommittedScope() {
    if (!scopeWriteCommitted || busy) return;
    const request = {
      caseId,
      generation: generationRef.current,
      search: location.search,
    ...
    setBusy(true);
    try {
      const nextWorkflow = await researchClient.getEventWorkflow(request.caseId);
      if (!requestIsCurrent()) return;
      const needsProtocol = nextWorkflow.userAction?.recommendation.kind
        === "complete_research_protocol";
      navigate(
      ...
      if (requestIsCurrent()) {
        setNotice("范围已更新，最新状态仍暂不可读。请稍后仅重试读取，不要重复保存范围。");
      }
    } finally {
      if (requestIsCurrent()) setBusy(false);
    }
    ...
    <section className="ros-scope-editor">
      ...
      {scopeWriteCommitted && (
        <button
          className="ros-button ros-button--secondary"
          disabled={busy}
          onClick={() => void rereadCommittedScope()}
          type="button"
        >
          {busy ? "正在读取最新流程…" : "重新读取最新流程"}
        </button>
      )}
...
export function ProtocolContent({
  ...
  const [notice, setNotice] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [writeReceipts, setWriteReceipts] = useState<Record<string, true>>({});
  const [protocolWorkflow, setProtocolWorkflow] = useState<Awaited<ReturnType<typeof researchClient.getEventWorkflow>> | null>(null);
  ...
  const currentRequest = () => ({ caseId, generation: generationRef.current });
  const requestIsCurrent = (request: { caseId: string; generation: number }) =>
    mounted.current
    && caseIdRef.current === request.caseId
    && generationRef.current === request.generation;
  const selected =
  ...
  const bindingReceiptKey = selected?.thesisId
    ? `${caseId}:${selected.thesisId}:outcome-binding`
    : "";
  const approvalReceiptKey = selected?.thesisId
    ? `${caseId}:${selected.thesisId}:outcome-binding-approval`
    : "";
  const bindingWriteCommitted = Boolean(
    bindingReceiptKey && writeReceipts[bindingReceiptKey],
  );
  const approvalWriteCommitted = Boolean(
    approvalReceiptKey && writeReceipts[approvalReceiptKey],
  );
  const approvalReady =
    !busy
    && !approvalWriteCommitted
    && Boolean(state?.effective_binding_id && approvalReason.trim());
  async function refresh(request = currentRequest()) {
    ...
    try {
      const workflow = await researchClient.getEventWorkflow(request.caseId);
      if (!requestIsCurrent(request)) return null;
      setProtocolWorkflow(workflow);
      return { states: nextStates, workflow };
    } catch {
      if (!requestIsCurrent(request)) return null;
      setProtocolWorkflow(null);
      return { states: nextStates, workflow: null };
    }
  }
  useEffect(() => {
    mounted.current = true;
  ...
  }, [caseId, data.event.id]);
  useEffect(() => {
    const refreshAfterProtocolWrite = () => {
      const request = currentRequest();
      void refresh(request).catch(() => {
        if (!requestIsCurrent(request)) return;
        setNotice("协议已保存，但最新可研究性状态暂不可读；系统不会据此继续补证。");
      });
    };
    window.addEventListener("research-os-protocol-refresh", refreshAfterProtocolWrite);
    return () => window.removeEventListener("research-os-protocol-refresh", refreshAfterProtocolWrite);
  }, [caseId, data.event.id]);
  useEffect(() => {
    let active = true;
  ...
  async function saveBinding() {
    ...
    setBusy(true);
    setNotice(null);
    let binding: Awaited<ReturnType<typeof researchOsApi.createOutcomeBinding>>;
    try {
      binding = await researchOsApi.createOutcomeBinding(
        selected.thesisId,
        {
      ...
      if (requestIsCurrent(request)) setBusy(false);
      return;
    }
    if (!requestIsCurrent(request)) return;
    setWriteReceipts((current) => ({
      ...current,
      [`${request.caseId}:${selected.thesisId}:outcome-binding`]: true,
    }));
    try {
      const latest = await refresh(request);
      if (!requestIsCurrent(request)) return;
      setNotice(latest?.workflow
        ? `结果绑定已登记为草案 ${binding.id}；仍需独立审核后才生效。`
        : "结果绑定已登记，最新状态暂不可读。请只重新读取协议状态，不要重复登记。");
    } catch {
      if (requestIsCurrent(request)) {
        setNotice("结果绑定已登记，最新状态暂不可读。请只重新读取协议状态，不要重复登记。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  ...
  async function approveBinding() {
    ...
    setWriteReceipts((current) => ({
      ...current,
      [`${request.caseId}:${selected.thesisId}:outcome-binding-approval`]: true,
    }));
    try {
      const latest = await refresh(request);
      if (!requestIsCurrent(request)) return;
      setNotice(latest?.workflow
        ...
        : "结果绑定已审核，最新状态暂不可读。请只重新读取协议状态，不要重复审核。");
    } catch {
      if (requestIsCurrent(request)) {
        setNotice("结果绑定已审核，最新状态暂不可读。请只重新读取协议状态，不要重复审核。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  }
  async function rereadProtocolAfterWrite() {
    const request = currentRequest();
    if (busy || (!bindingWriteCommitted && !approvalWriteCommitted)) return;
    setBusy(true);
    try {
      const latest = await refresh(request);
      if (!requestIsCurrent(request)) return;
      setNotice(latest?.workflow
        ? "已读取最新协议状态；已收到回执的写入不会再次提交。"
        : "协议写入已保存，最新流程仍暂不可读。请稍后只重试读取。");
    } catch {
      if (requestIsCurrent(request)) {
        setNotice("协议写入已保存，最新状态仍暂不可读。请稍后只重试读取。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  }
  const protocolCompletionRequired = protocolWorkflow?.userAction?.recommendation.kind
    === "complete_research_protocol";
  const protocolFactors = data.factors;
  const incompleteProtocolFactors = protocolFactors.filter((factor) => {
    const status = factor.thesisId ? states[factor.thesisId]?.status : undefined;
    return status !== "ready" && status !== "not_applicable";
  });
  const protocolReady = protocolCompletionRequired
    && protocolFactors.length > 0
    && incompleteProtocolFactors.length === 0;
  async function resumeAfterProtocol() {
    if (busy) return;
    const request = currentRequest();
    setBusy(true);
    setNotice(null);
    ...
    try {
      const latest = await refresh(request);
      if (!latest || !requestIsCurrent(request)) return;
      const remaining = protocolFactors.filter((factor) => {
        const status = factor.thesisId ? latest.states[factor.thesisId]?.status : undefined;
        return status !== "ready" && status !== "not_applicable";
      });
      if (remaining.length > 0) {
        setNotice(`还有 ${remaining.length} 项研究协议未完成：${remaining.map((factor) => factor.statement).join("、")}。`);
        return;
      }
      if (!latest.workflow) {
        setResumeFailed(true);
        setNotice("研究协议状态已确认，但统一流程暂时无法读取。系统不会声称已继续补证，请重新尝试。");
        return;
      }
      if (latest.workflow.userAction?.recommendation.kind !== "complete_research_protocol") {
        setNotice("当前统一流程已不再等待研究协议；请返回事件总览确认最新状态。");
        return;
      }
      const result = await researchClient.resumeProtocolWorkflow(
        request.caseId,
        latest.workflow.scope.id,
        {
          scopeVersion: latest.workflow.scope.version,
          idempotencyKey: `workflow:resume-protocol:${request.caseId}:${latest.workflow.scope.id}`,
        },
      );
      if (!requestIsCurrent(request)) return;
      if (!isActiveWorkflowState(result.state)) {
        setNotice(`协议恢复请求已受理，但流程当前为 ${result.state}；请返回事件总览确认后续动作。`);
        return;
      }
      window.dispatchEvent(new Event("research-os-workflow-refresh"));
      navigate(preserveLocationSearch(`/events/${request.caseId}`, location.search), {
        state: { workflowNotice: "系统已按新范围继续补证。" },
      });
    } catch {
      if (!requestIsCurrent(request)) return;
      setResumeFailed(true);
      setNotice("协议已满足，但系统尚未恢复补证。当前页面保留完成状态，可重新尝试继续自动补证。");
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  }
  return (
    <section className="ros-protocol" id="required-research-protocol">
      <header className="ros-section-heading">
        <div>
      ...
      {protocolCompletionRequired && (
        <section className="ros-protocol-resume" aria-label="协议完成后的流程动作">
          <div>
            <strong>{protocolReady ? "所有必要研究协议已满足" : "完成所有必要研究协议后才能继续"}</strong>
            <p>
              {protocolReady
                ? "系统仍处于协议阻塞状态；继续后才会创建真实补证运行。"
                : `当前还有 ${incompleteProtocolFactors.length} 项未完成：${incompleteProtocolFactors.map((factor) => factor.statement).join("、") || "正在读取协议状态"}。`}
            </p>
          </div>
          {protocolReady && (
            <button className="ros-button ros-button--primary" type="button" disabled={busy} onClick={() => void resumeAfterProtocol()}>
              {busy ? "正在继续…" : resumeFailed ? "重新尝试继续自动补证" : "继续自动补证"}
            </button>
          )}
        </section>
      )}
            ...
            <>
              ...
              {showForm && (
                <ProtocolBindingForm
                  ...
                  setReason={setReason}
                  busy={busy}
                  writeCommitted={bindingWriteCommitted}
                  onSave={saveBinding}
      ...
      {notice && (
        <p
          role={resumeFailed ? "alert" : "status"}
          className={
            notice.startsWith("结果绑定已") || notice.startsWith("系统已") ? "ros-success" : "ros-error"
          }
        >
          {notice}
        </p>
      )}
      {(bindingWriteCommitted || approvalWriteCommitted) && (
        <button
          className="ros-button ros-button--secondary"
          disabled={busy}
          onClick={() => void rereadProtocolAfterWrite()}
          type="button"
        >
          {busy ? "正在重新读取协议状态…" : "重新读取协议状态"}
        </button>
      )}
    </section>
  );
...
function ProtocolBindingForm(props: {
  ...
  const saveReady = !props.busy && !props.writeCommitted && missingSaveRequirements.length === 0;
...
export function MechanismProtocolPanel({ caseId }: { caseId: string }) {
  const [templateState, setTemplateState] = useState<{ caseId: string; value: MechanismTemplate[] } | null>(null);
  const [protocolState, setProtocolState] = useState<{ caseId: string; value: CaseMechanismProtocol } | null>(null);
  const [messageState, setMessageState] = useState<{ caseId: string; value: string } | null>(null);
  const [busyState, setBusyState] = useState<{ caseId: string; value: boolean }>({ caseId, value: false });
  const [selectionState, setSelectionState] = useState<{ caseId: string; value: boolean }>({ caseId, value: false });
  const mounted = useRef(true);
  const caseIdRef = useRef(caseId);
  const generationRef = useRef(0);
  ...
  const requestIsCurrent = (request: { caseId: string; generation: number }) =>
    ...
    && caseIdRef.current === request.caseId
    && generationRef.current === request.generation;
  const message = messageState?.caseId === caseId ? messageState.value : null;
  const busy = busyState.caseId === caseId && busyState.value;
  const selectionWriteCommitted = selectionState.caseId === caseId && selectionState.value;
  const setMessage = (value: string | null) => setMessageState(value === null ? null : { caseId, value });
  const setBusy = (value: boolean) => setBusyState({ caseId, value });
  const setSelectionWriteCommitted = (value: boolean) => setSelectionState({ caseId, value });
  async function refresh(request = currentRequest(), signal?: AbortSignal) {
    const [templateItems, protocolItem] = await Promise.all([
      researchOsApi.mechanismTemplates({ signal }),
      researchOsApi.caseMechanismProtocol(request.caseId, { signal }),
    ]);
    if (!requestIsCurrent(request)) return false;
    setTemplateState({ caseId: request.caseId, value: templateItems });
    setProtocolState({ caseId: request.caseId, value: protocolItem });
    return true;
  }
  useEffect(() => {
    mounted.current = true;
  ...
  useEffect(() => {
    ...
    setSelectionWriteCommitted(false);
    setBusy(false);
    setMessage(null);
    void refresh(request, controller.signal).catch(() => {
      if (!controller.signal.aborted && requestIsCurrent(request)) {
  ...
  async function select(templateId: string) {
    if (selectionWriteCommitted) return;
    const request = currentRequest();
    setBusy(true);
    setMessage(null);
    try {
      await researchOsApi.selectMechanismTemplate(request.caseId, {
    ...
    if (!requestIsCurrent(request)) return;
    setSelectionWriteCommitted(true);
    try {
      readControllerRef.current?.abort();
      const controller = new AbortController();
      readControllerRef.current = controller;
      await refresh(request, controller.signal);
      if (!requestIsCurrent(request)) return;
      window.dispatchEvent(new Event("research-os-protocol-refresh"));
      setMessage("机制模板已作为新的 Case 协议版本保存；旧选择仍可回放。");
    } catch {
      if (requestIsCurrent(request)) {
        setMessage("机制模板已保存，最新状态暂不可读。请只重新读取机制协议，不要重复选择。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  ...
  async function rereadSelection() {
    if (busy || !selectionWriteCommitted) return;
    const request = currentRequest();
    setBusy(true);
    try {
      readControllerRef.current?.abort();
      ...
      await refresh(request, controller.signal);
      if (!requestIsCurrent(request)) return;
      setMessage("已读取最新机制协议；已保存的模板选择不会再次提交。");
      window.dispatchEvent(new Event("research-os-protocol-refresh"));
    } catch {
      if (requestIsCurrent(request)) {
        setMessage("机制模板已保存，最新状态仍暂不可读。请稍后只重试读取。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  }
  const templates = templateState?.caseId === caseId ? templateState.value : [];
  const protocol = protocolState?.caseId === caseId ? protocolState.value : null;
  ...
  return (
    <section className="ros-mechanism">
      ...
      {selectionWriteCommitted && (
        <button className="ros-button ros-button--secondary" disabled={busy} onClick={() => void rereadSelection()} type="button">
          {busy ? "正在读取机制协议…" : "重新读取机制协议"}
        </button>
      )}
      {!protocol ? (
        <div className="ros-empty ros-empty--compact">
          ...
          {templates.map((item) => (
            <article key={item.id}>
              ...
              <button
                className="ros-button ros-button--primary"
                type="button"
                disabled={busy || selectionWriteCommitted}
...
export function MechanismRuleConfig({ caseId }: { caseId: string }) {
  const [protocolState, setProtocolState] = useState<{ caseId: string; value: CaseMechanismProtocol } | null>(null);
  const [metricState, setMetricState] = useState<{ caseId: string; value: MetricDefinition[] } | null>(null);
  ...
  const [messageState, setMessageState] = useState<{ caseId: string; value: string } | null>(null);
  const [busyState, setBusyState] = useState<{ caseId: string; value: boolean }>({ caseId, value: false });
  const [ruleWriteState, setRuleWriteState] = useState<{ caseId: string; value: boolean }>({ caseId, value: false });
  const mounted = useRef(true);
  const caseIdRef = useRef(caseId);
  const generationRef = useRef(0);
  const readControllerRef = useRef<AbortController | null>(null);
  ...
  const message = messageState?.caseId === caseId ? messageState.value : null;
  const busy = busyState.caseId === caseId && busyState.value;
  const ruleWriteCommitted = ruleWriteState.caseId === caseId && ruleWriteState.value;
  const setMessage = (value: string | null) => setMessageState(value === null ? null : { caseId, value });
  const setBusy = (value: boolean) => setBusyState({ caseId, value });
  const setRuleWriteCommitted = (value: boolean) => setRuleWriteState({ caseId, value });
  const protocol = protocolState?.caseId === caseId ? protocolState.value : null;
  const metrics = metricState?.caseId === caseId ? metricState.value : [];
  async function load(request = currentRequest(), signal?: AbortSignal) {
    const [current, metricItems] = await Promise.all([
      researchOsApi.caseMechanismProtocol(request.caseId, { signal }),
      researchOsApi.metrics({ signal }),
    ]);
    if (!requestIsCurrent(request)) return false;
    setProtocolState({ caseId: request.caseId, value: current });
    setMetricState({ caseId: request.caseId, value: metricItems });
    setEdgeId((value) =>
      current.template?.edges.some((edge) => edge.id === value)
        ? value
        : current.template?.edges[0]?.id || "",
    );
    setMetricId((value) =>
      metricItems.some((metric) => metric.id === value)
        ? value
        : metricItems[0]?.id || "",
    );
    return true;
  ...
  useEffect(() => {
    ...
    setSourceRoles("primary_disclosure");
    setPeriodStart("");
    setPeriodEnd("");
    setAvailableDeadline("");
    setNextEvent("");
    setReason("");
    setRuleWriteCommitted(false);
    setBusy(false);
    setMessage(null);
    void load(request, controller.signal).catch(() => {
      if (!controller.signal.aborted && requestIsCurrent(request)) {
  ...
  const ruleSaveReady = !busy && !ruleWriteCommitted && missingRuleRequirements.length === 0;
  async function save() {
    if (!ruleSaveReady) {
      setMessage(
    ...
    setMessage(null);
    try {
      await researchOsApi.createVerificationRule(request.caseId, edgeId, {
    ...
    if (!requestIsCurrent(request)) return;
    setRuleWriteCommitted(true);
    try {
      readControllerRef.current?.abort();
      const controller = new AbortController();
      readControllerRef.current = controller;
      await load(request, controller.signal);
      if (!requestIsCurrent(request)) return;
      window.dispatchEvent(new Event("research-os-protocol-refresh"));
      setMessage("验证规则已保存为新版本；上一规则仍可回放。");
    } catch {
      if (requestIsCurrent(request)) {
        setMessage("验证规则已保存，最新状态暂不可读。请只重新读取规则状态，不要重复保存。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
  }
  async function rereadRule() {
    if (busy || !ruleWriteCommitted) return;
    const request = currentRequest();
    setBusy(true);
    try {
      readControllerRef.current?.abort();
      ...
      await load(request, controller.signal);
      if (!requestIsCurrent(request)) return;
      setMessage("已读取最新验证规则；已收到回执的规则不会再次提交。");
      window.dispatchEvent(new Event("research-os-protocol-refresh"));
    } catch {
      if (requestIsCurrent(request)) {
        setMessage("验证规则已保存，最新状态仍暂不可读。请稍后只重试读取。");
      }
    } finally {
      if (requestIsCurrent(request)) setBusy(false);
    }
    ...
    <section className="ros-rule-config">
      ...
      {ruleWriteCommitted && (
        <button
          ...
          type="button"
          disabled={busy}
          onClick={() => void rereadRule()}
        >
          {busy ? "正在读取规则状态…" : "重新读取规则状态"}
        </button>
      )}
    </section>
  );
...
export function MechanismRuleHistory({ caseId }: { caseId: string }) {
  const [protocolState, setProtocolState] = useState<{ caseId: string; value: CaseMechanismProtocol } | null>(null);
  const [loadState, setLoadState] = useState<{ caseId: string; status: "loading" | "ready" | "error" }>({ caseId, status: "loading" });
  const [reload, setReload] = useState(0);
  const mounted = useRef(true);
  const caseIdRef = useRef(caseId);
  const generationRef = useRef(0);
  ...
  useEffect(() => {
    const request = { caseId, generation: ++generationRef.current };
    readControllerRef.current?.abort();
    const controller = new AbortController();
    readControllerRef.current = controller;
    setProtocolState(null);
    setLoadState({ caseId, status: "loading" });
    researchOsApi
      .caseMechanismProtocol(caseId, { signal: controller.signal })
      .then((value) => {
        if (!requestIsCurrent(request)) return;
        setProtocolState({ caseId: request.caseId, value });
        setLoadState({ caseId: request.caseId, status: "ready" });
      })
      .catch(() => {
        if (controller.signal.aborted || !requestIsCurrent(request)) return;
        setProtocolState(null);
        setLoadState({ caseId: request.caseId, status: "error" });
      });
    return () => controller.abort();
  }, [caseId, reload]);
  const protocol = protocolState?.caseId === caseId ? protocolState.value : null;
  const status = loadState.caseId === caseId ? loadState.status : "loading";
  if (status === "error") {
    return (
      <section className="ros-rule-history">
        <header>

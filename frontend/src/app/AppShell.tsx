...
import { parseSelectedEventRoute } from "./eventRoute";
import {
  researchOsApi,
...
export function AppShell() {
  ...
  const eventRoute = parseSelectedEventRoute(location.pathname);
  const selectedEventCaseId = eventRoute?.caseId ?? null;
  const isEventWorkflowOverview = eventRoute?.isOverview ?? false;
  ...
  useEffect(() => {
    closeDrawer();
  }, [selectedEventCaseId, location.pathname]);
  useEffect(() => {
    let live = true;
    let latestRequest = 0;
    const load = () => {
  ...
  async function openDrawer(run: ActiveResearchRun) {
    const requestGeneration = ++drawerGeneration.current;
    setDrawerRun(run);
    setDrawerEvents(null);
    setDrawerError(null);
    ...
    try {
      const response = await researchOsApi.runEvents(run.run_id);
      if (drawerGeneration.current !== requestGeneration) return;
      setDrawerEvents(
      ...
      if (drawerGeneration.current === requestGeneration) setDrawerLoading(false);
    }
  }
  const canManageLegacyCases =
    session?.roles?.includes("case_administrator") ?? false;
  const visibleActiveRuns = selectedEventCaseId
    ? isEventWorkflowOverview
      ? []
      : activeRuns.filter((run) => run.case_id === selectedEventCaseId)
    : activeRuns;
  const visibleFundRuns = selectedEventCaseId
    ? isEventWorkflowOverview
      ? []
      : activeFundRuns.filter((run) => run.case_id === selectedEventCaseId)
    : activeFundRuns;
  return (
    <div className="ros-shell">
      ...
      <div className="ros-main">
        ...
        {runLoadError && !isEventWorkflowOverview && (
          <section
        ...
        {fundRunLoadError && !isEventWorkflowOverview && (
          <section
            className="ros-run-strip ros-run-strip--error"

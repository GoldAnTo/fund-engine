...
import { researchOsApi, type MarketExpression } from "./researchOsApi";
      ...
      <section className="ros-instrument-profile__section">
        ...
        {fund.positions.map((position) => (
          <article className="ros-expression-card" key={position.stock_id}>
            <strong>
              <Link reloadDocument to={`/events/${caseId}/stocks/${position.stock_id}`}>
                {position.stock_name}
              </Link>{" "}
              · {position.stock_code}
            </strong>
...
function InstrumentMissing({
  ...
  return (
    <div className="ros-empty">
      ...
      <p>
        <Link reloadDocument to={`/events/${caseId}/market`}>返回市场与表达</Link>
      </p>
    </div>
  );
}

import { useMarketActor } from "./MarketActor";
...
export function MarketExpressionContent({
  ...
  return (
    <section className="ros-market ros-market-expression">
      ...
      <section aria-label="已审核主张" className="ros-market-register">
        <h3>已审核主张</h3>
        {!expression.claims.length && <p>尚未登记已审核主张。</p>}
        {expression.claims.map((claim) => <article key={claim.id}>
          {claim.source.permission_status === "admitted" ? <><h4>{claim.text}</h4><p>{claimKindLabels[claim.claim_kind] ?? claim.claim_kind} · {claim.asserted_by}</p><p>审核人：{claim.reviewed_by} · {claim.review_reason}</p></> : <p>来源受限，暂不展示主张内容。</p>}
        </article>)}
      </section>
      ...
      {actionNotice && (
        <p className="ros-success" role="status">
          ...
          {startedRunId && (
            <>
              {" "}
              <Link reloadDocument to={`/?caseId=${caseId}`}>查看运行记录</Link>
            </>
          )}
        </p>
      )}
            ...
            relatedFunds.map((fund) => {
              ...
              return (
                <article className="ros-fund-row" key={fund.fund_id}>
                  <strong>
                    <Link reloadDocument to={`/events/${caseId}/funds/${fund.fund_id}`}>
                      {fund.fund_name}
                    </Link>{" "}
  ...
  async function saveTarget() {
    ...
    try {
      const target = await researchOsApi.createForecastTarget(caseId, {
        ...
        reviewed_by: actor, review_reason: reason.trim(),
      });
  ...
  async function saveActual() {
    ...
    try {
      const actual = await researchOsApi.recordActualMetricObservation(caseId, {
        ...
        available_at: actualSource.available_at, recorded_by: actor, record_reason: reason.trim(),
      });
    ...
    try {
      await researchOsApi.createForecastVerdict(candidate.id, {
        ...
        outcome: decision === "modified"
          ? modifiedOutcome
          : null,
        reason: reason.trim(), reviewed_by: actor,
      });
  ...
  return (
    <p>
      {label}：{source.document_version_id ? (
        <Link reloadDocument className="ros-source-link" to={`/events/${caseId}/documents?document=${source.document_version_id}`}>
          定位到冻结原文
        </Link>
      ) : "冻结版本未记录"}
        ...
        {loading ? (
          ...
          bindings.map((binding) => (
            <article key={binding.id}>
              <strong>
                {binding.stock_id ? (
                  <Link reloadDocument to={`/events/${caseId}/stocks/${binding.stock_id}`}>
...
function MarketExpressionRegistration({
  ...
  const [claimReviewer, setClaimReviewer] = useState(actor);
  ...
  const [factorReviewer, setFactorReviewer] = useState(actor);

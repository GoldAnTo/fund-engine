...
export interface CreateReportResearchInput {
  ...
  sourceUrl?: string;
  content: string;
  /** An approved, active source-contract version. The backend rejects unknown ids. */
...
export type ReportIntakeState =
  | "needs_supplement"
  | "pending_candidate_extraction"
  | "artifact_extraction_unavailable";

export type ReportIntakeActionKind =
  | "supplement_text"
  | "extract_candidates"
  | "view_saved_snapshot";

export type ReportResearchDocumentInputKind =
  | "pdf_upload"
  | "pasted_text"
  | "web_content"
  | "recovery_text";

export interface ReportIntakeNextAction {
  kind: ReportIntakeActionKind;
  label: string;
  unlockMessage: string;
}

export interface ReportIntakeDocument {
  id: string;
  title: string;
  inputKind: ReportResearchDocumentInputKind;
}

export interface ReportIntakeSupplementArtifact {
  id: string;
  claimedPageReference: string | null;
}

export interface ReportResearchIntake {
  caseId: string;
  caseTitle: string;
  primaryDocument: ReportIntakeDocument;
  supplementDocuments: ReportIntakeDocument[];
  supplementArtifacts: ReportIntakeSupplementArtifact[];
  state: ReportIntakeState;
  blockingReason: string | null;
  nextAction: ReportIntakeNextAction;
}
...
export interface CreatedReportResearch {
  ...
  blockingReason: string | null;
  nextAction: ReportIntakeNextAction;
  recoverySnapshot: ReportIntakeSupplementArtifact | null;
}
...
export interface EventResearchClient {
  ...
  getReportResearchIntake?(caseId: string): Promise<ReportResearchIntake>;

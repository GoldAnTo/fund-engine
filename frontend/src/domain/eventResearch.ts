import type { EventWorkflowClient } from "./eventWorkflow";
...
export interface EventFactor {
  ...
  automaticallyAdmittedSupportCount?: number;
  automaticallyAdmittedContradictionCount?: number;
...
export interface WorkbenchProgress {
  ...
  automaticallyAdmittedCount: number;
  pending: number;
...
export interface EventResearchClient extends EventWorkflowClient {
  ...
  attachEventMaterial(input: { caseId: string; rawInput: string; sourceUrl?: string; sourceType: EventSourceType; sourceMetadata: Record<string, unknown> }): Promise<{ documentVersionId: string }>;
  uploadEventMaterial(input: { caseId: string; file: File; sourceMetadata: Record<string, unknown> }): Promise<{ documentVersionId: string; parseState: "parsed" | "partial" | "failed"; nextAction: "review_original" | "supplement_original" }>;
  ...
  continueEventResearch(input: { caseId: string; documentVersionId: string; reason: string }): Promise<EventResearchContinuation>;
  decidePublishedMaterial(input: { caseId: string; rawInput: string; sourceUrl?: string; sourceType: EventSourceType; sourceMetadata: Record<string, unknown>; decision: "reopen" | "no_change"; reason: string }): Promise<PublishedMaterialDecision>;
  decidePublishedUploadedMaterial(input: { caseId: string; file: File; sourceMetadata: Record<string, unknown>; decision: "reopen" | "no_change"; reason: string }): Promise<PublishedMaterialDecision>;
  updateEventResearchScope(input: { caseId: string; factors: EventResearchScopeFactorInput[]; changeReason: string }): Promise<EventResearchScope & { reclassifiedEvidenceCount: number }>;
  getEventReviewQueue(caseId: string): Promise<EventReviewQueue>;
  publishEventConclusion(input: { caseId: string; text: string }): Promise<{ conclusionId: string; state: "published" }>;
}

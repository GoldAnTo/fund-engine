...
export interface StartAutomaticResearchUpload {
  file: File;
  input?: string;
}
...
export interface AutomaticResearchNarrative {
  currentAction: string;
  completedCount: number;
  totalCount: number;
  nextAction: string;
  elapsedSeconds: number;
  estimatedRemainingSecondsMin: number | null;
  estimatedRemainingSecondsMax: number | null;
}

export interface AutomaticResearchActivityDetail {
  occurredAt: string;
  workItem: string;
  internalStatus: string;
}

export interface AutomaticResearchActivity {
  label: string;
  count: number;
  technicalDetails: AutomaticResearchActivityDetail[];
}

export type AutomaticResearchFactorClassification =
  | "key"
  | "secondary"
  | "pending"
  | "excluded";

export interface AutomaticResearchFactor {
  statement: string;
  classification: AutomaticResearchFactorClassification;
...
export interface AutomaticResearchView {
  ...
  narrative: AutomaticResearchNarrative;
  activities: AutomaticResearchActivity[];
  exceptions: AutomaticResearchException[];
  factors: AutomaticResearchFactor[];
...
export interface AutomaticResearchClient {
  ...
  startAutomaticResearchFromUpload(input: StartAutomaticResearchUpload): Promise<AutomaticResearchStart>;

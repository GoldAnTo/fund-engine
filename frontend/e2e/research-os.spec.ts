...
test.describe("Event-first Research OS", () => {
  test("workflow overview stays usable at 390px with one full-size primary action", async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto("/events/event-tsm?client=mock");

    const primaryAction = page.locator(".event-workflow-primary-action");
    await expect(primaryAction).toHaveCount(1);
    await expect(primaryAction).toBeVisible();
    await expect(primaryAction).toHaveText("调整研究范围");
    ...
    const layout = await page.evaluate(() => {
      const selectors = [
        ...
        ".event-workflow-workspaces",
        ".event-workflow-ledger",
      ];
      return {
        ...
        boxes: selectors.map((selector) => {
          ...
          return { selector, missing: false, left: box.left, right: box.right, width: box.width };
        }),
      };
    });
    expect(layout.scrollWidth).toBeLessThanOrEqual(layout.viewportWidth);
    for (const box of layout.boxes) {
      expect(box.missing, `${box.selector} should exist`).toBe(false);
      expect(box.left, `${box.selector} should not escape left`).toBeGreaterThanOrEqual(0);
      expect(box.right, `${box.selector} should not escape right`).toBeLessThanOrEqual(390);
    }
  ...
  test("human review advances one event from evidence to a published conclusion", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "调整研究范围" })).toBeVisible();

    // Human review remains available as an optional governed sub-workflow; the
    // automatic overview no longer pretends it is the one required next step.
    await page.getByRole("link", { name: "证据工作台" }).click();
    await page.getByRole("navigation", { name: "证据工作台页面" }).getByRole("link", { name: "证据审核" }).click();
    ...
    await expect(page.getByRole("list", { name: "事件研究六阶段" }).locator('[aria-current="step"]')).toContainText("报告与监测");
    await expect(page.getByRole("heading", { name: "当前无需操作" })).toBeVisible();
    await expect(page.getByRole("region", { name: "系统正在运行" })).toHaveCount(0);
  });
  ...
  test("event workbench switches the current event without leaving its workspace", async ({ page }) => {
    ...
    await expect(page.getByRole("region", { name: "系统正在运行" })).toHaveCount(0);
    await expect(page.getByRole("link", { name: /查看本次运行/ })).toHaveCount(0);
  });

  test("event switching clears the previous Case run drawer and top strip", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/events/event-tsm/documents?client=mock");

    await expect(page.getByRole("region", { name: "系统正在运行" })).toBeVisible();
    await page.getByRole("button", { name: "展开运行详情" }).click();
    await expect(page.getByRole("complementary", { name: "全局运行详情" })).toBeVisible();

    await page.getByRole("button", { name: /当前事件研究/ }).click();
    await page.getByRole("group", { name: "切换事件研究" }).getByRole("button", { name: /经营数据披露后的变动/ }).click();

    await expect(page).toHaveURL(/\/events\/event-published\/documents\?client=mock/);
    await expect(page.getByRole("complementary", { name: "全局运行详情" })).toHaveCount(0);
    await expect(page.getByRole("region", { name: "系统正在运行" })).toHaveCount(0);
    await expect(page.getByText("台积电 Case")).toHaveCount(0);
  });
  ...
  test("event switcher stays usable on a narrow screen", async ({ page }) => {
    ...
    await expect(page.getByRole("list", { name: "事件研究六阶段" })).toBeVisible();
  });
  ...
  test("Case navigation groups contextual pages on a narrow workbench", async ({ page }) => {
    ...
    await expect(page.getByRole("region", { name: "你现在应该做什么" })).toBeVisible();
    const action = page.getByRole("link", { name: "调整研究范围" });
    await expect(action).toHaveCount(1);
    await expect(action).toBeVisible();
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);

    await page.getByText("更多研究内容").click();
    await page.getByRole("group", { name: "更多研究内容" }).getByRole("link", { name: "证据工作台" }).click();
    await expect(page).toHaveURL(/\/events\/event-tsm\/evidence/);
    await page.getByRole("navigation", { name: "证据工作台页面" }).getByRole("link", { name: "证据审核" }).click();
  ...
  test("licensed-provider intake requires a reproducible record and makes use permissions explicit", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "证据工作台" }).click();
    await page.getByRole("navigation", { name: "证据工作台页面" }).getByRole("link", { name: "原文资料" }).click();
  ...
  test("new strict Case opens its own intake workbench before any run starts", async ({ page }) => {
    ...
    await expect(page.getByText("统一研究流程")).toBeVisible();
    await page.getByRole("link", { name: "证据工作台" }).click();
    const inspectSource = page.getByRole("navigation", { name: "证据工作台页面" }).getByRole("link", { name: "原文资料" });
  ...
  test("inbox freezes a material into a chosen active Case without starting a new Case", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "查看冻结原文" }).click();
  ...
  test("freezes an uploaded original into an existing Case and exposes its immutable file record", async ({ page }) => {
    ...
    await expect(page.getByText(/user:mock-researcher · case_retained/)).toBeVisible();
  });

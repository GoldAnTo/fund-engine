...
**状态：已确认；原件冻结与可恢复接入已实现，后续研究门禁仍按本规格推进**
**日期：2026-08-08**
...
用户上传的二进制原件另以不可变上传工件保存，并与对应的 `DocumentVersion` 一对一关联。读取端只展示文件名、MIME、大小、对象版本、上传人和保留策略，绝不把原始字节混入资料摘要。当前接入支持 PDF 文本层及 TXT、Markdown、CSV：损坏或无法解析的 PDF 仍保留原件并进入可补充状态；Office 格式尚未接入解析器，也不在界面中宣称可上传。

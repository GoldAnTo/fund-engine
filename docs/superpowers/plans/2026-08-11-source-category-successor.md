...
- `backend/alembic/versions/0052_source_contract_research_type.py`：为不可变合同新增列并以已有接入方式回填历史数据。
...
- Create: `backend/alembic/versions/0052_source_contract_research_type.py`
...
Extend the SQLite bootstrap assertion to expect Alembic revision `0052`, inspect `source_contracts`, and assert `research_source_type` is non-null. Add a small upgrade fixture at revision `0051`, insert a contract with `source_type='licensed_provider'`, upgrade to head, and assert its new value is `licensed_provider`.
...
Expected: the contract has no `research_source_type`, the HTTP(S) validation is absent, and the expected head revision is not `0052`.
...
```bash
git add backend/alembic/versions/0052_source_contract_research_type.py backend/app/models/source_governance.py backend/app/services/source_governance.py backend/app/schemas/v1/documents.py backend/app/queries/documents.py backend/tests/test_event_source_governance.py backend/tests/test_sqlite_migration_bootstrap.py

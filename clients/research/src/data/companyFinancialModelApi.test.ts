...
describe("company financial model API boundaries", () => {
  ...
  it("preserves scientific Decimal results across save, history, and workspace reads", async () => {
    const value = modelRecord();
    value.inputs.terminal_growth = "0.00000000";
    value.result.terminal_growth = "0E-8";
    value.result.scenarios[0]!.rows[0]!.fcff = "1.25E-8";
    value.result.scenarios[0]!.enterprise_value_usd_million = "1E+3";
    const body = { parent_revision_id: modelIds.revision, expected_latest_id: null, baseline_content_hash: modelHash, inputs: value.inputs };
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(value, 201)));
    expect(await api.saveCompanyFinancialModel(modelIds.project, body, "decimal-save")).toEqual(value);
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(value)));
    expect(await api.companyFinancialModelDraft(modelIds.project, value.id)).toEqual(value);
    const workspace = modelWorkspace();
    workspace.latest = value;
    workspace.history = [{ id: value.id, sequence: value.sequence, created_at: value.created_at, input_hash: value.input_hash, content_hash: value.content_hash }];
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(workspace)));
    expect((await api.companyFinancialModel(modelIds.project)).latest).toEqual(value);
  });
  it.each(["NaN", "Infinity", "1E9999", "1E", "0x10", " 1E-8"])("rejects invalid Decimal result %s", async (decimal) => {
    const value = modelRecord();
    value.result.terminal_growth = decimal;
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(value)));
    await expect(api.companyFinancialModelDraft(modelIds.project, value.id)).rejects.toMatchObject({ code: "invalid_response" });
  });
  it("keeps input validation in the backend's fixed decimal format", async () => {
    const value = modelRecord();
    value.inputs.terminal_growth = "1E-8";
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response(value)));
    await expect(api.companyFinancialModelDraft(modelIds.project, value.id)).rejects.toMatchObject({ code: "invalid_response" });
  });

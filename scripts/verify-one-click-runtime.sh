...
capture_snapshot() {
  ...
  if ! docker inspect "${ids[@]}" | python3 "$STABILITY_HELPER" snapshot --expect postgres=1 --expect api=1 --expect research-worker=1 --expect "acquisition-worker=${ACQUISITION_REPLICAS}" --expect company-research-worker=1 --expect api-proxy=1 > "$destination"; then die 'container snapshot is invalid'; fi
}
http_checks() {
  ...
  curl --fail --silent --show-error --connect-timeout 2 --max-time 5 "$API_PROXY_URL/health" >/dev/null || die 'api-proxy health endpoint check failed'
  [[ "$(curl --silent --show-error --connect-timeout 2 --max-time 5 --output /dev/null --write-out '%{http_code}' "$API_PROXY_URL/")" == 404 ]] || die 'API proxy must not serve pages'

...
def test_case_protocol_api_lists_templates_and_appends_selection(cmd_client, cmd_session) -> None:
    ...
    selection = cmd_client.post(
        ...
        json={"template_version_id": templates.json()[0]["id"], "reason": "适用范围已核对"},
    )
    ...
    metric = cmd_client.post("/api/v1/metric-definitions", json={
        ...
        "allowed_source_roles": ["primary_disclosure"], "role_eligibility": ["driver"], "reason": "规则指标",
    })
    ...
    rule = cmd_client.post(
        ...
        json={"metric_definition_id": metric.json()["id"], "expected_direction": "increase", "support_predicate": "披露的 CapEx 同比增长", "contradiction_predicate": "CapEx 下调", "allowed_source_roles": ["primary_disclosure"], "observed_period_start": "2026-01-01", "observed_period_end": "2026-03-31", "available_at_deadline": "2026-05-31", "next_verification_event": "季度财报", "reason": "定义反证规则"},
    )

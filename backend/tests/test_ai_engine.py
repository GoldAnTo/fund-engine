...
def test_extraction_derives_offsets_for_a_unique_verbatim_quote(session, span):
    """Provider offsets are advisory; the exact source slice remains binding."""
    client = LLMClient(model_version="mock-test", mock=True)
    ...
    assert len(candidates) == 1
    assert candidates[0].quote_start == 0
    assert candidates[0].quote_end == len("original")


def test_extraction_rejects_ambiguous_quote_when_provider_offsets_are_wrong(
    session, document_service
):
    document = document_service.freeze(
        raw=b"repeated repeated", source_url="https://example.test/repeated"
    )
    span = document_service.add_span(
        document.id, {"paragraph": 1}, "repeated repeated"
    )
    client = LLMClient(model_version="mock-test", mock=True)

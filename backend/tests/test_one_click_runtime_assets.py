...
def test_gateway_delivery_uses_isolated_api_database_and_loopback_publication() -> None:
    ...
    assert api["ports"] == ["127.0.0.1:${GATEWAY_API_PORT:-8100}:8000"]
    assert compose.count("ports:") == 1
    assert "ACQUISITION_ENABLED_ADAPTERS: gildata,sse,szse" in compose
    assert 'command: ["alembic", "upgrade", "head"]' in compose
    assert api["environment"]["RESEARCH_TENANT_TOKENS"] == "${RESEARCH_TENANT_TOKENS:?}"
    assert "frontend" not in services
    assert "context: ./frontend" not in compose
    assert "GATEWAY_PROXY_TOKEN" not in compose
    assert "GATEWAY_API_BASE" not in compose

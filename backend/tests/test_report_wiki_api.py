...
def _append_explicit_reviewed_scope(
...
) -> tuple[list[ReportClaim], list[ReportRelation]]:
    """Model the post-intake human workflow required before Wiki reads.

    Report intake deliberately freezes only source material.  Wiki tests that
    need a graph create candidates and an explicit selected scope themselves,
    rather than relying on the removed intake-time automation.
    """
    ...
    relations = list(
        cmd_session.scalars(
            select(ReportRelation).where(ReportRelation.claim_id.in_(claim_ids))
        )
    )
...
def _create_reviewed_report(
    cmd_client, cmd_session, *, title: str, content: str
) -> tuple[uuid.UUID, uuid.UUID]:
    case_id = _create_report(cmd_client, title=title, content=content)
...
def test_wiki_graph_returns_only_the_current_report_scope_and_source_locators(
    ...
    case_id, _document_id = _create_reviewed_report(
        cmd_client, cmd_session,
        title="供应链关系图",
...
def test_wiki_defaults_to_latest_report_document_but_can_read_one_history_scope(
    ...
    _append_explicit_reviewed_scope(
        cmd_session, case_id=case_id, document_id=old_document_id
    )
...
def test_same_report_document_can_have_multiple_question_scopes(cmd_client, cmd_session) -> None:
    ...
    claims, _relations = _append_explicit_reviewed_scope(
        cmd_session, case_id=case_id, document_id=document_id
    )
    claim = claims[0]
...
def test_same_document_scopes_freeze_distinct_claim_and_relation_paths(
    ...
    case_id, document_id = _create_reviewed_report(
        cmd_client, cmd_session,
        title="同一研报不同路径",
...
def test_scope_selection_rejects_foreign_claim_before_appending_scope(
    ...
    other_case_id, _other_document_id = _create_reviewed_report(
        cmd_client, cmd_session,
        title="其他研报",
...
def test_unadmitted_confounder_cannot_satisfy_factor_gate_or_leak_into_wiki(
    ...
    assert relation is not None
    _later_statement = _append_independent_statement(
        cmd_session,
        case_id=case_id,
...
def test_postgres_append_scope_serializes_same_case_versions(
    ...
    try:
        ...
        now = datetime.now(timezone.utc)
        contract = SourceContractRepository(bootstrap).append_contract(
            provider_name="postgres scope concurrency fixture",
            tenant_id="tenant-a",
            may_display=True,
            may_search=True,
            may_ai_process=True,
            ...
            deletion_policy="fixture",
            downstream_restrictions="internal only",
            approved_by="test",
            reason="postgres scope concurrency fixture",
            created_at=now,
        )
        bootstrap.commit()
        created = ReportResearchService(bootstrap).create_text(
        ...
        claims = ReportClaimExtractor(bootstrap).extract(created.case.id)
        relation_ids = list(
            bootstrap.scalars(
        ...
        scope = ReportResearchService(bootstrap).append_scope(
            created.case.id,
            created.document.id,
            changed_by="postgres-fixture-reviewer",
            change_summary="显式创建并发测试范围",
        ...
        claim = claims[0]
        relation = bootstrap.get(ReportRelation, relation_ids[0])
        assert relation is not None
    finally:
        bootstrap.close()

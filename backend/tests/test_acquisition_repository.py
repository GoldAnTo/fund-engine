...
def test_lease_renewal_extends_only_the_current_unexpired_claim(
    repo, session, clock, research_case, thesis
):
    queued = _create_job(repo, research_case, thesis)
    ...
    assert expiry == clock.now + timedelta(seconds=30)
    persisted = session.get(AcquisitionJob, queued.id)
    assert persisted is not None and persisted.lease_expires_at is not None
    assert persisted.lease_expires_at.replace(tzinfo=UTC) == expiry
    with pytest.raises(StaleLeaseError):
        repo.renew_lease(
            queued.id,
            lease_token="rotated-token",
            lease_for=timedelta(seconds=30),
        )
    session.rollback()

...
def cmd_client(cmd_session, monkeypatch):
    ...
    contract = SourceContractRepository(cmd_session).append_contract(
        provider_name="test report intake contract",
        tenant_id="tenant-a",
        may_display=True,
        may_search=True,
        may_ai_process=True,
    ...
    # Each request shares this session in the test client.  Persist the fixture
    # contract before exercising validation failures, whose route-level
    # rollback must not erase an authorized fixture needed by later requests.
    cmd_session.commit()
    # Report-dispatch reads resolve their tenant from a server-configured
    # bearer credential.  Individual tests still pass that header explicitly
    # so missing-credential behavior remains observable.
    monkeypatch.setenv(
        "REPORT_RESEARCH_TENANT_TOKENS",
        '{"test-tenant-a-token":"tenant-a"}',
    )
    ...
    try:
        ...
        client.report_source_contract_id = str(contract.id)
        client.report_tenant_headers = {"Authorization": "Bearer test-tenant-a-token"}
        original_post = client.post

        def _report_contract_post(url, *args, **kwargs):
            """Keep legacy report fixtures explicit about their seeded contract."""
            headers = kwargs.get("headers") or {}
            if (
                isinstance(url, str)
                and (
                    url in {"/api/v1/report-research", "/api/v1/report-research/pdf"}
                    or url.endswith("/supplement")
                )
                and headers.get("x-test-omit-source-contract") != "1"
            ):
                ...
                elif url == "/api/v1/report-research/pdf":
                    kwargs["params"] = {
                        "source_contract_id": str(contract.id),
        ...
        client.post = _report_contract_post
        yield client
    finally:
        app.dependency_overrides.pop(get_db, None)

...
def test_sqlite_upgrade_downgrade_and_reupgrade_0053(tmp_path: Path) -> None:
    database_path = tmp_path / "acquisition-0052.db"
...
def test_sqlite_upgrade_downgrade_and_reupgrade_0054(tmp_path: Path) -> None:
    database_path = tmp_path / "automatic-admission-0054.db"
    backend = Path(__file__).parents[1]
    environment = {
        **os.environ,
        "DATABASE_URL": f"sqlite:///{database_path}",

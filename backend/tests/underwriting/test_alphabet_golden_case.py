                ...
                "driver_overrides": [
                    {
                        ...
                        **_assumption_number(
                            "1"
                            if scenario == "base"
                            else ("1.1" if scenario == "bull" else "0.9"),
...
def test_bundled_manifest_is_pinned_by_its_exact_utf8_bytes() -> None:
    ...
    ).hexdigest() == (BUNDLED_MANIFEST_CONTENT_SHA256)
...
def test_custom_fixture_read_failure_is_a_fixture_error(
    ...
    with pytest.raises(
        AlphabetGoldenCaseFixtureError, match="source_facts.json is unreadable"
    ):
    ...
    [
        (
            lambda root: (root / "source_facts.json").write_text(
                '{"schema_version":"alphabet.golden-case.source-facts.v1","schema_version":"duplicate"}',
                encoding="utf-8",
            ),
            "duplicate key",
        ),
...
def test_custom_fixture_rejects_unhashable_security_external_key(
    tmp_path: Path,
) -> None:
    root = _copy_fixture(tmp_path)
    facts = _read_json(root / "source_facts.json")
...
def test_loaded_fixture_keeps_official_lineage_modules_gaps_and_distinct_securities() -> (
    None
):
    ...
    assert all(
        fact.published_at.time().isoformat() == "23:59:59" for fact in fixture.facts
    )
...
def test_adapter_accepts_only_the_fixture_business_module_vocabulary() -> None:
    ...
    assert (
        AlphabetCompanyResearchAdapter().validate_source_modules(
...
def test_strategy_adapter_preserves_candidate_digest_and_all_assumption_metadata() -> (
    None
):
    fixture = load_alphabet_golden_case_fixture()
    assert fixture.strategy_assumptions is not None
    ...
    assert all(
        path.assumption_key and path.assumption_rationale and path.assumption_equation
    ...
    assert (
        assumptions.calculate_content_hash(
...
def test_bundled_market_inputs_pin_exact_abc_capital_and_raw_lineage() -> None:
    ...
    assert tuple(
        (item.security_external_key, item.value) for item in market.prices
    ) == (
...
def test_optional_governed_files_load_closed_market_and_strategy_contracts(
    ...
    with pytest.raises(ValidationError, match="content hash does not match"):
        StrategyAssumptionSet(
            strategy_version=assumptions.strategy_version,
            content_hash=assumptions.content_hash,
            first_fiscal_year=assumptions.first_fiscal_year,
            driver_paths=assumptions.driver_paths,
            scenario_overrides=assumptions.scenario_overrides,
            terminal_growth=assumptions.terminal_growth,
...
def test_capture_script_enforces_streamed_download_byte_limit() -> None:
    source = (_FIXTURE_ROOT / "_capture_verified_raw.py").read_text(encoding="utf-8")
...
def test_alphabet_template_declares_every_minimum_operating_driver_and_baseline() -> (
    None
):
    template = AlphabetCompanyResearchAdapter().model_template()
    expected = {
        "search_query_intensity",
        "search_ad_monetization",
    ...
    assert {
        item.requirement_key for item in template.operating_baseline_requirements
    } == expected
    assert {
    ...
    assert all(
        ...
        for descriptor in (
            *module.revenue_sources,
            *module.cost_structure,
            *module.capital_needs,
        )
    )
...
def test_prepare_evidence_index_writes_only_index_and_gaps_then_queues_model(
    session,
) -> None:
    ...
    assert outcome.status == "building_model"
...
def test_prepare_evidence_index_records_recoverable_failure_without_artifact_head(
    ...
    service = CompanyResearchSourceService(
        session,
        now=lambda: NOW,
        fixture_loaders={
            "US:ALPHABET:COMPANY": (unavailable if failure == "unavailable" else parser)
        },
    )
    ...
    assert outcome.error["code"] == "company_research_source_unavailable"
    assert (
        session.scalar(
            select(func.count())
...
def test_prepare_evidence_index_recovers_from_unhashable_fixture_security_key(
    ...
    service = CompanyResearchSourceService(
        ...
        now=lambda: NOW,
        fixture_loaders={
            "US:ALPHABET:COMPANY": lambda: load_alphabet_golden_case_fixture(root)
        },
    )

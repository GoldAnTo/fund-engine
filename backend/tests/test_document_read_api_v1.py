...
from app.models.ledger import CaseTenantAdmission, DocumentVersion, EvidenceLink
...
def test_documents_list_can_be_scoped_to_one_case_without_leaking_other_case_material(
    api_client, session, document_service, research_service, document
):
    ...
    session.add_all(
        [
            ...
            CaseTenantAdmission(
                ...
                tenant_id="test-team",
                initial_document_version_id=other_document.id,
                admitted_by="test-fixture",
                admitted_at=now,
            ),
        ]
...
def test_document_detail_counts_citations(
    api_client, session, document_service, document, span, research_service
):

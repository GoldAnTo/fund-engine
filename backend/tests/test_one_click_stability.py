...
SERVICES = ["api", "worker", "postgres", "redis", "neo4j", "frontend"]
...
def test_validate_exact_healthy_six_service_snapshot():
    expected = {service: 1 for service in SERVICES}
    actual = validate_snapshot(snapshot_payload(), expected)
    assert set(actual) == set(SERVICES)
    assert all(len(items) == 1 and set(items[0]) == {"id", "restart_count"} for items in actual.values())
...
def test_connection_cap_examples_and_invalid_inputs():
    ...
    with pytest.raises(ValueError, match="connection-cap inputs are invalid"): connection_cap(0, 1, 0)

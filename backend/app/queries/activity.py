...
class ActivityQueries:
    ...
    def _page(
        ...
        *,
        ...
        visible_ids = (
            {str(case_id)} if case_id is not None else authorized_case_ids
        )
        if not visible_ids:
            return []
        query = query.where(
            or_(
                and_(
                    DomainEvent.ref_type == "research_case",
                    DomainEvent.ref_id.in_(visible_ids),
                ),
                and_(
                    DomainEvent.aggregate_type == "research_case",
                    DomainEvent.aggregate_id.in_(visible_ids),
                ),
            )
        )

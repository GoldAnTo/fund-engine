...
class EventResearchScopeService:
    ...
    def update(
        ...
        # reviewed-evidence assignments. Orchestrated Cases lock in stable
        # Case -> orchestration -> lifecycle order before any scope read.
        orchestration_service = ResearchOrchestrationService(self._session)
        orchestration = orchestration_service.lock_scope_revision_target(
            case_id
        )
        ...
        if lifecycle is not None:
            if orchestration is not None:
                orchestration_service.adopt_revised_scope(
                    case_id,
                    scope.id,
                    actor=changed_by.strip(),
                )
            else:
                self._continue_research_if_needed(
                    lifecycle,
                    active_theses,
                    now,
                )

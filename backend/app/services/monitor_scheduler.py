...
ACCEPTANCE_FREQUENCY = "acceptance_every_5_minutes"
ACCEPTANCE_FREQUENCY_FLAG = "LIVE_ACCEPTANCE_FAST_SCHEDULER"


def acceptance_frequency_enabled() -> bool:
    return os.environ.get(ACCEPTANCE_FREQUENCY_FLAG, "").strip().lower() == "true"
...
class MonitorScheduler:
    ...
    def dispatch_due(self, *, now: datetime | None = None) -> list[ResearchRun]:
        ...
        for monitor in latest.values():
            ...
            if monitor.frequency == ACCEPTANCE_FREQUENCY:
                # The short acceptance bucket uses the scheduler's explicit
                # clock so its deduplication proof is deterministic.
                run.created_at = now
                run.updated_at = now
            created.append(run)
        return created
    ...
    def _is_due(cls, frequency: str, now: datetime) -> bool:
        if frequency == ACCEPTANCE_FREQUENCY:
            return acceptance_frequency_enabled()
    ...
    def next_due_at(cls, frequency: str, *, now: datetime | None = None) -> datetime | None:
        if frequency == ACCEPTANCE_FREQUENCY:
            if not acceptance_frequency_enabled():
                return None
            now = now or datetime.now(timezone.utc)
            return (
                now.replace(

...
@app.exception_handler(WorkflowNotInitializedError)
async def workflow_not_initialized_error_handler(
    request: Request, exc: WorkflowNotInitializedError
):
    request_id = getattr(request.state, "request_id", "")
    return _v1_error_response(
        "workflow_not_initialized",
        str(exc) or "research workflow is not initialized",
        request_id,
        details={"safe_action": exc.safe_action},
        status_code=409,
    )

...
router = APIRouter(tags=["jobs-v1"], dependencies=[Depends(require_research_actor)])
Actor = Annotated[ResearchActor, Depends(require_research_actor)]
Database = Annotated[Session, Depends(get_db)]


def _job_dto(job, *, gateway_owned: bool = False) -> JobDTO:
    return JobDTO(
        id=str(job.id),
        kind=job.kind,
        status=job.status,
        ...
        attempt=job.attempt,
        step=None if gateway_owned else job.step,
        error=("native_execution_failed" if job.error else None)
        if gateway_owned
        else job.error,
...
def get_job(job_id: uuid.UUID, actor: Actor, db: Database):
    job, gateway_owned = OperationalAccess(db).require_job(actor, job_id, summary=True)
    return _job_dto(job, gateway_owned=gateway_owned)
...
def get_job_events(
    ...
    OperationalAccess(db).require_job(actor, job_id)
...
@router.post(
    "/jobs/{job_id}/cancel", response_model=JobDTO, status_code=status.HTTP_200_OK
)
def cancel_job(job_id: uuid.UUID, actor: Actor, db: Database):
    job, _ = OperationalAccess(db).require_job(actor, job_id)
...
@router.post(
    "/jobs/{job_id}/retries", response_model=JobDTO, status_code=status.HTTP_200_OK
)
def retry_job(job_id: uuid.UUID, actor: Actor, db: Database):
    repo = JobRepository(db)
    job, _ = OperationalAccess(db).require_job(actor, job_id)
    if job.status not in {"failed", "cancelled"}:
        raise NotFoundError("job is not retryable")
    job.status = "queued"
    job.attempt += 1
    job.error = None

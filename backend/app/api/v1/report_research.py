...
from app.api.v1.report_research_tenant import require_report_research_tenant
...
def _response(created: CreatedReportResearch) -> ReportResearchCreatedResponse:
    ...
    return ReportResearchCreatedResponse(
        ...
        state=created.state,
        next_action=created.outcome.next_action,
        blocking_reason=created.outcome.blocking_reason,
        needs_text_or_pages=created.state == "needs_supplement",
        recovery_snapshot=(
            ReportRecoverySnapshotDTO(
                id=created.recovery_snapshot.id,
                source_identity=created.recovery_snapshot.source_identity,
                content_sha256=created.recovery_snapshot.content_sha256,
                claimed_page_reference=created.recovery_snapshot.claimed_page_reference,
                parser=created.recovery_snapshot.parser_version,
            )
            if created.recovery_snapshot is not None
            else None
        ),
    )
...
def upload_pdf_report(
    ...
    source_contract_id: uuid.UUID = Query(),
...
@router.get(
    "",
    response_model=ReportResearchListResponse,
    responses={
        401: {"model": ErrorEnvelope, "description": "Authentication required"},
        403: {"model": ErrorEnvelope, "description": "Permission denied"},
    },
)
def list_report_research(
    tenant_id: str = Depends(require_report_research_tenant),
    limit: int = Query(default=50, ge=1, le=100),
    db: Session = Depends(get_db),
) -> ReportResearchListResponse:
    """List report Cases as intake summaries for the research dispatch."""
    return ReportWikiQueries(db).list_intakes(tenant_id=tenant_id, limit=limit)


@router.get("/{case_id}/intake", response_model=ReportIntakeResponse)
def get_report_research_intake(
    case_id: uuid.UUID, db: Session = Depends(get_db)
) -> ReportIntakeResponse:
    """Return the current safe handoff before a report Wiki may be read."""
    return ReportWikiQueries(db).intake(case_id)
...
@router.head("/{case_id}/embed/wiki", include_in_schema=False)

...
class CompanyResearchWorkbench:
    ...
    def _artifact(
        ...
        duplicate_refs = len({canonical_hash(ref) for ref in row.source_refs}) != len(
            row.source_refs
        )
        legacy_evidence_refs = (
            row.kind in {"evidence_index", "research_gaps"}
            and row.payload.get("fixture_content_hash")
            == LEGACY_EVIDENCE_MANIFEST_CONTENT_SHA256
        )
        if duplicate_refs and not legacy_evidence_refs:
            raise ValidationError(
    ...
    def review_evidence(
        ...
        *,
        ...
        project = self._product_repository.lock_project(project_id)
        ...
        research_gaps = self._company.current_artifact(
            project_id, "research_gaps", lock=True
        )
        company = self._product_repository.object(project.primary_company_id)
        if research_gaps is None or company is None:
            raise ValidationError("company research evidence lineage is invalid")
        evidence_chain = self._company.artifact_chain(head.id, lock=True)
        self._validate_evidence_review_chain(
            evidence_chain,
            research_gaps=research_gaps,
            preparation=preparation,

...
_VERSIONED_ASSUMPTION = re.compile(r"[a-z][a-z0-9_.-]*\.v[1-9][0-9]*:[a-z0-9_.:-]+\Z")
_SCENARIO_ORDER = ("base", "bull", "bear")
_STRATEGY_ASSUMPTIONS_SCHEMA = "company-research.strategy-assumptions.v2"
_LEGACY_ALPHABET_STRATEGY_ASSUMPTIONS_SCHEMA = (
...
class CompanyResearchModelTemplate:
    ...
    def __post_init__(self) -> None:
        ...
        security_keys = _text_tuple(
            self.security_external_keys, "company model template security identity"
        )
...
class SensitivityAssumption:
    """Two governed assumption points for one model sensitivity variable."""
    ...
    def __post_init__(self) -> None:
        if self.variable_key not in {"required_return", "terminal_growth"}:
            raise ValidationError("sensitivity variable is unsupported")
        if (
            type(self.low) is not StrategyAssumptionValue
            or type(self.high) is not StrategyAssumptionValue
            or self.low.value >= self.high.value
        ):
            raise ValidationError("sensitivity assumption range is invalid")

    def canonical_payload(self) -> dict[str, object]:
        return {
            "variable_key": self.variable_key,
            "low": self.low.canonical_payload(),
            "high": self.high.canonical_payload(),
        }
...
class StrategyAssumptionSet:
    ...
    sensitivity_assumptions: tuple[SensitivityAssumption, ...] = ()
    _allow_legacy_alphabet_hash: InitVar[bool] = False

    def __post_init__(self, _allow_legacy_alphabet_hash: bool) -> None:
        if (
        ...
        if (
            tuple(item.driver_key for item in self.driver_paths)
            != SCENARIO_FINANCIAL_DRIVER_KEYS
        ):
            raise ValidationError(
        ...
        if not self.terminal_growth.assumption_key.startswith(
            f"{self.strategy_version}:"
        ):
            raise ValidationError("strategy terminal growth key must be versioned")
        if (
            not isinstance(self.sensitivity_assumptions, tuple)
            or not all(
                type(item) is SensitivityAssumption
                for item in self.sensitivity_assumptions
            )
            or len({item.variable_key for item in self.sensitivity_assumptions})
            != len(self.sensitivity_assumptions)
            or any(
                not item.low.assumption_key.startswith(f"{self.strategy_version}:")
                or not item.high.assumption_key.startswith(f"{self.strategy_version}:")
                for item in self.sensitivity_assumptions
            )
        ):
            raise ValidationError("strategy sensitivity assumptions are invalid")
        if (
            not isinstance(self.content_hash, str)
            or _SHA256.fullmatch(self.content_hash) is None
        ):
            raise ValidationError(
                "strategy assumptions content hash must be a SHA-256 content hash"
            )
        expected_hash = self.calculate_content_hash(
            ...
            sensitivity_assumptions=self.sensitivity_assumptions,
        )
        legacy_alphabet_hash = self.calculate_legacy_alphabet_content_hash(
            strategy_version=self.strategy_version,
            first_fiscal_year=self.first_fiscal_year,
            driver_paths=self.driver_paths,
            scenario_overrides=self.scenario_overrides,
        ...
        legacy_hash_is_readable = (
            _allow_legacy_alphabet_hash
            and (
                self.strategy_version.startswith("alphabet.")
                or self.strategy_version.startswith("alphabet-")
            )
            and self.content_hash == legacy_alphabet_hash
        )
        if self.content_hash != expected_hash and not legacy_hash_is_readable:
            raise ValidationError("strategy assumptions content hash does not match")

    @classmethod
    def from_legacy_alphabet(
        cls,
        *,
        strategy_version: str,
        content_hash: str,
        ...
        """Read an authenticated Alphabet v1 hash without enabling legacy writes."""
        return cls(
            ...
            _allow_legacy_alphabet_hash=True,
        )
    ...
    def calculate_content_hash(
        ...
        *,
        ...
        return cls._calculate_content_hash_for_schema(
            schema_version=_STRATEGY_ASSUMPTIONS_SCHEMA,
            strategy_version=strategy_version,
            first_fiscal_year=first_fiscal_year,
            driver_paths=driver_paths,
            scenario_overrides=scenario_overrides,
    ...
    @classmethod
    def calculate_legacy_alphabet_content_hash(
        cls,
        *,
        strategy_version: str,
        first_fiscal_year: int,
        ...
        """Verify an already-frozen Alphabet v1 hash; never use for new writes."""
        return cls._calculate_content_hash_for_schema(
            schema_version=_LEGACY_ALPHABET_STRATEGY_ASSUMPTIONS_SCHEMA,
            strategy_version=strategy_version,
            first_fiscal_year=first_fiscal_year,
            driver_paths=driver_paths,
            scenario_overrides=scenario_overrides,
    ...
    @staticmethod
    def _calculate_content_hash_for_schema(
        *,
        schema_version: str,
        strategy_version: str,
        first_fiscal_year: int,
        ...
        if sensitivity_assumptions:
            payload["sensitivity_assumptions"] = tuple(
                item.canonical_payload() for item in sensitivity_assumptions
            )
